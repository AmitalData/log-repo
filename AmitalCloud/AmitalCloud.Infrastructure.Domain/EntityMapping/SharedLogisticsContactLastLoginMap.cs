//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a Amital
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated> AmitalClassesGenerator.tt
//---
using System;
using System.Threading.Tasks;
using System.Data.Entity.ModelConfiguration;
using POCO = AmitalCloud.Infrastructure.Domain.EntityPOCOs;
 
namespace AmitalCloud.Infrastructure.Domain.EntityMapping
{
 
    public class SharedLogisticsContactLastLoginMap : EntityTypeConfiguration<POCO.SharedLogisticsContactLastLogin>
    {
	    string dbms;
        public SharedLogisticsContactLastLoginMap()
        { 
		
      dbms = System.Configuration.ConfigurationManager.AppSettings.Get("DBMS");
      if (dbms == "oracle")
      {
		this.ToTable("SharedLogisticsContactLastLogi");
      }
	  else
	  {
	    this.ToTable("SharedLogisticsContactLastLogins");
	  }

		
		    this.HasKey(t => new { t.ContactId, t.CardId, t.PartnerTypeId, t.Via });
	 
            this.Property(t => t.Tenant).HasColumnName("Tenant").IsRequired();

            this.Property(t => t.ContactId).HasColumnName("ContactId").IsRequired().HasMaxLength(15).IsUnicode(false);

            this.Property(t => t.CardId).HasColumnName("CardId").IsRequired().HasMaxLength(15).IsUnicode(false);

            this.Property(t => t.PartnerTypeId).HasColumnName("PartnerTypeId").IsRequired().HasMaxLength(2).IsUnicode(false);

            this.Property(t => t.Via).HasColumnName("Via").IsRequired().HasMaxLength(20).IsUnicode(false);

            this.Property(t => t.LoginDateTime).HasColumnName("LoginDateTime");
        }
    }
}
	 