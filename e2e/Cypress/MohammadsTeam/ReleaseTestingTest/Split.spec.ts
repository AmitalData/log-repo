...
// create new customer 
	...
	// cy.get('#GLAccount_IsMultiCurrency').click(); 
        cy.get('#Ok-AddGLAccount').click();
       cy.get('#DispalyTransactions').click({ force: true }); 
        cy.get('li[id=GLAccountTHAdditional]').click();
        cy.get('#AddSplittedbyCurrency').click(); 
        cy.get('input[id=GLAccount_CurrencyId_1]').type('NIS');
        cy.get('ul[id=mydatalist_GLAccount_CurrencyId_1]').contains("NIS").then(a => {
        ...
        cy.get('button[id=SaveSplittedbyCurrency]').click({ force: true }); 
		 cy.wait(4000);
       cy.get('button[id=GLAccount-SaveClose]').click({ force: true }); 
