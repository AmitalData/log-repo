...
exports.ChequeDepositSpec = void 0;
...
describe('New Deposit ', function () {
    ...
    it('New Cheque Deposit was Created Successfully', function () {
        ...
        cy.get('ul[id=mydatalist_ARPayment_BillToId]').contains("ARpayment Reco").then(function (a) {
            a[0].click();
        });
        ...
        cy.get('ul[id=mydatalist_ARPayment_AccountingPaymentMethodId]').contains("Cheque").then(function (a) {
            a[0].click();
        });
        cy.get('input[id=ARPayment_BranchId]').type("Main Office").should("have.value", "Main Office");
        cy.get('ul[id=mydatalist_ARPayment_BranchId]').contains("Main Office").then(function (a) {
            a[0].click();
        });
        ...
        cy.get('ul[id=mydatalist_BankDeposit_CashBookId]').contains("cheque").then(function (a) {
            a[0].click();
        });
        cy.get('input[id=BankDeposit_DepositBankAccountId]').type("BankNO822998ZA");
        cy.get('ul[id=mydatalist_BankDeposit_DepositBankAccountId]').contains("Bank").then(function (a) {
            a[0].click();
        });
        ...
        cy.get('input[id=SearchFieldsId_0_1]').should('be.visible').then(function (a) {
        ...
        var closeBalance = cy.get('input[id=textboxdiv_ReconcileExternalPage_CloseBalance]');
        cy.log(closeBalance + '');
        //cy.get('#Add').click();
    });
});
