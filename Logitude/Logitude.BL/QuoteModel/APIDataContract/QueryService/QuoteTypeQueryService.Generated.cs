...
{ 
   public partial class QuoteTypeQueryService
   {
		...
		public QuoteType GetQuoteTypeByCode(string Code,int Tenant)
        { 
		    try
            {
				...
				return QuoteTypeDataMapping(temp,Tenant);
			}
            catch (Exception ex)
            {
		...
		public QuoteTypePM QuoteTypeDataMappingAndValidatin(QuoteType MyEntity,int Tenant,string ComputingPartnerName = "")
        {
		    try
            {
