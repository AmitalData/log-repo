...
namespace Logitude.BL.ShipmentsModel.CustomFilters
{
    public static class DigitalPortalCustomFilter
    {
            ...
            foreach (QueryFilterItem item in queryFilters)
            {
                if (item.IsCustom)
                {
                    if (item.FieldName == "InProgress")
                    {
                        ...
                        queryableData = queryableData.Where(a => a.IsOperationalClosed == false && a.IsAccountingClosed == false && a.IsCustomerArchived == false
                        &&
                    ...
                    if (item.FieldName == "InOrigin")
                    {
                        ...
                        && a.IsOperationalClosed == false && a.IsAccountingClosed == false && a.IsCustomerArchived == false
                        &&
                        !(a.MainCarriageFinalDestinationATA > afterNinetyDaysDate
                        || System.Data.Entity.DbFunctions.TruncateTime(a.CreateDateTime) > afterOneYearDate));
                    }
                    ...
                    if (item.FieldName == "AllShipments")
                    {
                        ...
                        var afterNinetyDaysDate = currentDateTime.AddDays(90);
                        isAllShipments = true;
                        queryableData = queryableData.Where(a => a.ShipmentLevelCode != "C" && a.IsCancelled == false
                         &&
                        (a.MainCarriageFinalDestinationATA > afterNinetyDaysDate)
                        || System.Data.Entity.DbFunctions.TruncateTime(a.CreateDateTime) > afterOneYearDate || a.IsCustomerArchived == true);
        ...
        public static IQueryable<ShipmentDataView> GetDigtalOldFilteredQuery(QueryOperations operations, IQueryable<ShipmentDataView> queryableData, ShipmentRepository shipmentRepository = null, int _tenant = 0)
        {
            List<QueryFilterItem> queryFilters = operations.QueryFilterItems;
            ...
            foreach (QueryFilterItem item in queryFilters)
            {
                if (item.IsCustom)
                {
                    ...
                    if (item.FieldName == "DigitalPortalSearchFields")
                    {
                        queryableData = ApplyOldDigitalPortalSearchFilter(item, queryableData);
                    }

                    if (item.FieldName == "ConsigneeShipperIds")
                    {
                        queryableData = ApplyOldShipperConsigneeFilter(item, queryableData);
                    }
        ...
        public static IQueryable<ShipmentDataView> ApplyOldShipperConsigneeFilter(QueryFilterItem item, IQueryable<ShipmentDataView> queryableData)
        {
            string[] values = item.FieldValue == null ? null : item.FieldValue.ToString().Split(',');
        ...
        public static IQueryable<ShipmentDataView> ApplyOldDigitalPortalSearchFilter(QueryFilterItem item, IQueryable<ShipmentDataView> queryableData)
        {
            string digitalPortalSearchFields = item.FieldValue as string;
            digitalPortalSearchFields = digitalPortalSearchFields.ToLower().Trim();
