		    ...
		    try
            {
					...
					if(MyEntity.AirPackages != null && MyEntity.AirPackages.Count > 0)
					{
						...
						{								//throw new ApplicationException("AirPackages Can't be update"); 
								temp.ShipmentPackages = AirPackageService33.AirPackageCustomDataMappingAndValidatin(MyEntity,MyEntity.AirPackages,Tenant,ComputingPartnerName);
					...
					if(MyEntity.OceanOrInlandPackages != null && MyEntity.OceanOrInlandPackages.Count > 0)
					{
						...
						{								//throw new ApplicationException("OceanOrInlandPackages Can't be update"); 
								temp.ShipmentPackages = OceanOrInlandPackageService33.OceanOrInlandPackageCustomDataMappingAndValidatin(MyEntity,MyEntity.OceanOrInlandPackages,Tenant,ComputingPartnerName);
					...
					if(MyEntity.Containers != null && MyEntity.Containers.Count > 0)
					{
						...
						{								//throw new ApplicationException("Containers Can't be update"); 
								temp.ShipmentPackages = ContainerService33.ContainerCustomDataMappingAndValidatin(MyEntity,MyEntity.Containers,Tenant,ComputingPartnerName);
