...
namespace Logitude.BL.GlobalModel.EntityQueries
{
    public class RoleQuery
    {
        ...
        public List<RolePM> GetRolesByUser(string userId, int tenant)
        {
            ...
            if (userId != null)
            {
                ContactTenant contacttenant = (from a in commonDataContext.ContactTenants
                                               where a.ContactId == userId && (a.TenantId == tenant)
                                               select a).FirstOrDefault();
                if (contacttenant == null)
                {
                    contacttenant = (from a in commonDataContext.ContactTenants
                ...
                if (contacttenant != null)
                {
                    ...
                    List<ContactTenantRole> contactTenantRoles = (from a in commonDataContext.ContactTenantRoles
        ...
        public List<RolePM> GetCustomRolesByIds(List<string> myRolesIds)
        {
            ...
            if (myRolesIds.Count > 0)
            {
                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("GetCustomRolesByIds myRolesIds: " +  myRolesIds?.Count());
                ...
                NetCommonHelper.Logger.DevLog.Instance.WriteInfo($"GetCustomRolesByIds myResult: {myResult?.Count()}");
