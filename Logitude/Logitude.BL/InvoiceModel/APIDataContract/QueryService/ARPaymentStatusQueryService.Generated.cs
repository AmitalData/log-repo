...
{ 
   public partial class ARPaymentStatusQueryService
   {
		...
		public ARPaymentStatus GetARPaymentStatusByCode(string Code,int Tenant)
        { 
		    try
            {
				...
				return ARPaymentStatusDataMapping(temp,Tenant);
			}
            catch (Exception ex)
            {
		...
		public ARPaymentStatusPM ARPaymentStatusDataMappingAndValidatin(ARPaymentStatus MyEntity,int Tenant,string ComputingPartnerName = "")
        {
		    try
            {
