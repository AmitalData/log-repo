...
{ 
   public partial class ARInvoiceStatusQueryService
   {
		...
		public ARInvoiceStatus GetARInvoiceStatusByCode(string Code,int Tenant)
        { 
		    try
            {
				...
				return ARInvoiceStatusDataMapping(temp,Tenant);
			}
            catch (Exception ex)
            {
		...
		public ARInvoiceStatusPM ARInvoiceStatusDataMappingAndValidatin(ARInvoiceStatus MyEntity,int Tenant,string ComputingPartnerName = "")
        {
		    try
            {
