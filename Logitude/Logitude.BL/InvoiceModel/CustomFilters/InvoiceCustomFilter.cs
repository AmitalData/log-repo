        ...
        public IQueryable<ARInvoice> GetFilteredQuery(QueryOperations operations, IQueryable<ARInvoice> queryableData)
        {
            ...
            foreach (QueryFilterItem item in queryFilters)
            {
                if (item.IsCustom)
                {
                    ...
                    {
                        queryableData = queryableData.Where(d => d.IsClosed == false && d.IsCancelled == false && d.IsGeneralInvoice == true && (d.StatusCode != "DR" && d.StatusCode != "VD" && d.StatusCode != "LL" && d.IsAutoCredit == false));
                    }
