...
namespace Logitude.BL.InvoiceModel.Tools.DataMapping
{
    public partial class ARInvoiceMapping
    {
        ...
        {
            var transferStatusCode = GetConcurrencyFieldValue_String(entityPM.TransferStatusCode_Original, entityPM.TransferStatusCode, entity.TransferStatusCode);
            entity.TransferStatusCode = transferStatusCode;

            var isTransferStarted = GetConcurrencyFieldValue_Bool(entityPM.IsTransferStarted_Original, entityPM.IsTransferStarted, entity.IsTransferStarted);
            entity.IsTransferStarted = isTransferStarted;

            var transferError = GetConcurrencyFieldValue_String(entityPM.TransferError_Original, entityPM.TransferError, entity.TransferError);
            if (!string.IsNullOrEmpty(transferError) && transferError.Length > 250)
                transferError = transferError.Substring(0, 250);
        ...
        private static bool GetConcurrencyFieldValue_Bool(bool oiginalValue, bool newValue, bool serverValue)
        {
