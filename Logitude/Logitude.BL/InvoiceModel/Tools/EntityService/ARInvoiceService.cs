...
namespace Logitude.BL.InvoiceModel.Tools.EntityService
{
    public class ARInvoiceService
    {
                ...
                if (!entityPM.IsInvoiceNumberManuallySet)
                {
                    if (!entityPM.IsInvoiceNumberFromStock)
                    {
                        ...
                        if (string.IsNullOrEmpty(entityPM.InvoiceNumber) || entityPM.InvoiceNumber == entityPM.Id)
                        {
                            ...
                            {
                                CounterDefinitionRepository counterDefinitionRepository = new CounterDefinitionRepository();
        ...
        private GLAccountPM GetSplittedAccountByInvoiceLineCurrency(GLAccountPM debitGLAcount, string invoiceLineCurrency, int tenant)
        {
            IGLAccountQueryServiceExt glAccountQuery = ContainerAccessor.Container.Resolve(typeof(IGLAccountQueryServiceExt), "GLAccountQueryServiceExt", new ParameterOverride("", 1)) as IGLAccountQueryServiceExt;
            GLAccountPM splittedAccount = glAccountQuery.GetSplittedGLAccount(debitGLAcount.Id, tenant, invoiceLineCurrency);
            return splittedAccount;
        }
