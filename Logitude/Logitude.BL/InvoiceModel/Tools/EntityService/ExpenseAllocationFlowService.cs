...
namespace Logitude.BL.InvoiceModel.Tools.EntityService
{
    public class ExpenseAllocationFlowService
    {
        ...
        public void Create(ExpenseAllocationFlowPM theEntityPm)
        {
            ...
            if (theEntityPm.RunDate != null && theEntityPm.RunDate.Date == DateTime.Today.Date && theEntityPm.JournalId == null)
            {
                RunTaskNow(Poco);
            }
        ...
        private string AddJournalAndJournalLines(APInvoicePM theEntityPm, ExpenseAllocationSettingPM settingPM, string expenseAllocationFlowId)
        {
            ...
            if (tenantPOCO.AccountingActivated)
            {
                ...
                List<JournalLinePM> journalLines = (from d in theEntityPm.InvoiceLines?.Where(a => a.ChangeSetOp != ChangeSetOperation.Delete && a.IsPrepaidExpenses == true)
                ...
                const decimal roundingTolerance = 0.06m;
                Func<decimal, decimal> round2 = x => Math.Round(x, 2, MidpointRounding.AwayFromZero);
                if (ShouldCreateAnotherTask(settingPM.Id, 1))
                {
                    if (journalLines != null && journalLines.Count > 0)
                    {
                        var debitLinesByCurrency = journalLines
                            .GroupBy(l => l.CurrencyId ?? string.Empty);

                        foreach (var grp in debitLinesByCurrency)
                        {
                            string currency = grp.Key;
                            var linesInCurrency = grp.ToList();

                            decimal totalOriginalForThisCurrency = theEntityPm.InvoiceLines
                                .Where(il => il.ChangeSetOp != ChangeSetOperation.Delete
                                             && il.IsPrepaidExpenses == true
                                             && (il.ForiegnCurrencyId ?? string.Empty) == currency)
                                .Sum(il => (Math.Round(il.VatRecognizedPercentage == null ? (decimal)il.LocalCurrencyAmount.Value : (decimal)il.LocalAmountWithVatRecognized.Value, 2)));

                            decimal perInstallmentRaw = totalOriginalForThisCurrency / settingPM.NumberOfPayments;
                            decimal totalAfterRounding = 0m;
                            for (int i = 0; i < settingPM.NumberOfPayments; i++)
                            {
                                totalAfterRounding += round2(perInstallmentRaw);
                            }
                            decimal diff = round2(totalOriginalForThisCurrency - totalAfterRounding);
                            if (diff != 0m && Math.Abs(diff) <= roundingTolerance)
                            {
                                ...
                                journal.JournalLines.Where(d => d.Line == largestJournalAmount.Line).ToList().ForEach(d => { d.LocalAmount = d.LocalAmount + diff; d.ForeignAmount = d.ForeignAmount + diff; });
                ...
                if (!differentCurrencies)
                {
                    ...
                    journalLine.LocalAmount = ((decimal)journalDebitLines.Sum(ln => ln.LocalAmount));
                    journalLine.CurrencyId = theEntityPm.InvoiceCurrencyId;
                    journalLine.ForeignAmount = ((decimal)journalDebitLines.Sum(ln => ln.ForeignAmount));
                ...
                else
                {
                    foreach (var curr in currencies)
                    {
                        ...
                        journalLine.LocalAmount = ((decimal)theEntityPm.InvoiceLines.Where(ln => ln.ForiegnCurrencyId == curr && ln.IsPrepaidExpenses == true).Sum(ln => ln.LocalCurrencyAmount)) / settingPM.NumberOfPayments;
                        journalLine.CurrencyId = curr;
                        journalLine.ForeignAmount = ((decimal)theEntityPm.InvoiceLines.Where(ln => ln.ForiegnCurrencyId == curr && ln.IsPrepaidExpenses == true).Sum(ln => ln.ForiegnCurrencyAmount)) / settingPM.NumberOfPayments;
                        journalLine.ExchangeRate = (decimal)theEntityPm.InvoiceLines.Where(ln => ln.ForiegnCurrencyId == curr && ln.IsPrepaidExpenses == true).FirstOrDefault().ForiegnExchangeRate;
