...
namespace Logitude.BL.CommonDataModel.Tools.EntityService
{
    public class CustomAgentService
    {
        bool isNewEntity;
        private int tenant;
        public CustomAgent entityPOCO { get; set; }
        private Card entityCard;
        private CustomAgentPM entityPM;
        private Contact loggedContact;
        private CardRepository cardRepository;
        private CustomAgentRepository entityRepository;
        ...
        public void Create(CustomAgentPM entityPM)
        {
            this.entityPM = entityPM;
            ...
            {
                CustomAgentTracing.Trace(entityPM, entityPOCO, isNewEntity);
            }
            ...
            new EntityCustomFieldService(new EntityCustomFieldServiceArgs() { ObjectTableName = "Card", EntityId = entityPM.Id, Tenant = entityPM.Tenant, Type = "PM", Entities = new List<CustomAgentPM> { entityPM }.Cast<object>().ToList() }).Update();
        ...
        public void Update(CustomAgentPM entityPM, bool mapComposition = false)
        {
            ...
            this.entityPOCO = entityRepository.GetSingleCustomAgent(entityPM.Tenant, entityPM.Id);
                    ...
                    if (!string.IsNullOrEmpty(itemContactPM.Email))
                    {
                        using (TransactionScope scope = TransactionFactory.GetNewTransaction())
                        {
                            ...
                            bool globalContactExists = (from a in globalContactRep.GetGlobalContactByTenant(tenant)
                                                        where a.Email == itemContactPM.Email.ToLower()
                                                        select a).Any();
