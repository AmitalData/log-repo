...
{
    public partial class CargoTrackingValidationClass : ICargoTrackingValidateContext
    {
        bool ready;
        int tenant = 0;
        ...
        public CargoTrackingValidationClass(string objectTableName, int tenant)
        {
            this.tenant = tenant;
        ...
        public string GetErrorMessage(object value, object instance, string propertyName)
        {
            ...
            if (field.IsRequiered)
            {
                ...
                if (value == null)
                {
                    error = CargoTrackingTranslateTextsClass.GetTranslation("General.M.FieldIsRequired", field.FullNameTextCode.Code, null, null, field.Tenant);
                }
            ...
            if (field.DataTypeCode == "Text")
            {
                ...
                if (FieldValueValidator.IsNotValidMinMaxValue(field, valueString))
                {
                    error = CargoTrackingTranslateTextsClass.GetTranslation("General.M.MinMax", field.FullNameTextCode.Code, field.MinLength.ToString(), field.MaxLength.ToString(), field.Tenant);
                }
            }
