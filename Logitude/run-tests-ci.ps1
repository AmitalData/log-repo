...
if ($useCoverlet -and $coverlet) {
    ...
    # If coverlet.msbuild is installed, build (not rebuild) to instrument, then run vstest directly
    # Only build if DLL doesn't exist to avoid unnecessary dependency rebuilds
    if ($useCoverlet -and $coverletMsbuild) {
        $testDllCheck = 'Library\Bin\Logitude.UnitTest.dll'
        if (-not (Test-Path $testDllCheck)) {
            $testDllCheck = "Logitude.UnitTest\bin\$Configuration\Logitude.UnitTest.dll"
        }
        
        if (-not (Test-Path $testDllCheck)) {
            Write-Host 'Building test project with coverlet.msbuild instrumentation...' -ForegroundColor Cyan
            $testProjectPath = 'Logitude.UnitTest\Logitude.UnitTest.csproj'
            $coverageOutputPath = Join-Path $OutputDir 'coverage'
            
            # Build (not rebuild) with coverlet properties to instrument assemblies
            # Use Build instead of Rebuild to avoid rebuilding dependencies unnecessarily
            ...
            if ($buildExitCode -ne 0) {
                ...
                Write-Host '[OK] Project built successfully with coverlet instrumentation' -ForegroundColor Green
            }
        } else {
            Write-Host 'Test DLL already exists, skipping build to avoid dependency rebuilds' -ForegroundColor Green
        }
        
        # Update test DLL path (should be in Library\Bin based on project OutputPath)
        $testDll = 'Library\Bin\Logitude.UnitTest.dll'
        if (-not (Test-Path $testDll)) {
            $testDll = "Logitude.UnitTest\bin\$Configuration\Logitude.UnitTest.dll"
        }
        if (-not (Test-Path $testDll)) {
            Write-Error "Test DLL not found: $testDll. Please build the test project first."
            exit 1
        }
