...
{
   public class TimeManagementContext: DbContextBase, ITimeManagementContext
    {
        public TimeManagementContext()
        {
            Database.SetInitializer<TimeManagementContext>(null); 
			Database.CommandTimeout = ApplicationAppInfo.GetDataBaseTimeOut();
			
        }

        public TimeManagementContext(DbConnection conn)
            : base(conn,true)
        {
        ...
        public static ITimeManagementContext GetContext(int tenant)
        {           
            GlobalDB currentDb;
