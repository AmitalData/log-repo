   ...
   public partial class ShipmentOrderValidationClass : IShipmentOrderValidateContext
    {
        ...
        public ShipmentOrderValidationClass(string objectTableName, int tenant)
        {
            this.tenant = tenant;
        ...
        public string GetErrorMessage(object value, object instance, string propertyName)
        {
            ...
            if (field.IsRequiered)
            {
                ...
                if (value == null)
                {
                    error = ShipmentOrderTranslateTextsClass.GetTranslation("General.M.FieldIsRequired", field.FullNameTextCode.Code, null, null, field.Tenant);
                }
            ...
            if (field.DataTypeCode == "Text")
            {
                ...
                if (!string.IsNullOrEmpty(valueString))
                {
                    if (!field.IsMaxLength)
                    {
                        if (valueString.Length > field.MaxLength || valueString.Length < field.MinLength)
                        {
                            error = ShipmentOrderTranslateTextsClass.GetTranslation("General.M.MinMax", field.FullNameTextCode.Code, field.MinLength.ToString(), field.MaxLength.ToString(), field.Tenant);
