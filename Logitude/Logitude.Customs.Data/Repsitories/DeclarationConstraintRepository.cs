            ...
            {    
                ...
                if (consignment != null)
                {
                    declarationConstraintConsignments = (from c in context.Consignments.Include("Declaration")
                                                         ...
                                                         select c).ToList();
                    consignmentParames = declarationConstraintConsignments.FindAll(x => !ArrayDeclartiosId.Contains(x.DeclarationId)).Select(s => s.Declaration.CustomFileNo).DistinctBy().ToList();
