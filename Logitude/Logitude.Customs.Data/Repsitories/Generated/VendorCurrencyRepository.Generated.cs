...
namespace Logitude.Customs.Data.Repsitories
{
   public partial class VendorCurrencyRepository:IRepository<VendorCurrency>
   {
		...
		public  VendorCurrency GetSingle(string vendorid, int tenant, int linenumber, string currencytype, int tenant)
        {
            return (from a in context.VendorCurrencies
                    where a.VendorId == vendorid && a.Tenant == tenant && a.LineNumber == linenumber && a.CurrencyType == currencytype && a.Tenant == tenant
        ...
        public VendorCurrency GetSingle(EntityKeyFields entityKeys)
        {
            ...
            return (from a in context.VendorCurrencies
                    where a.VendorId == keys.VendorId && a.Tenant == keys.Tenant && a.LineNumber == keys.LineNumber && a.CurrencyType == keys.CurrencyType
                    select a).FirstOrDefault();
        }
