...
namespace Logitude.CRM.BL.Validators
{
    public partial class CRMValidationClass : ICRMValidateContext
    {
        ...
        public CRMValidationClass(string objectTableName, int tenant)
        {
            this.tenant = tenant;
        ...
        public string GetErrorMessage(object value, object instance, string propertyName)
        {
            ...
            if (field.IsRequiered)
            {
                ...
                if (value == null)
                {
                    error = CRMTranslateTextsClass.GetTranslation("General.M.FieldIsRequired", field.FullNameTextCode.Code, null, null, field.Tenant);
                }
            ...
            if (field.DataTypeCode == "Text")
            {
                ...
                if (FieldValueValidator.IsNotValidMinMaxValue(field, valueString))
                {
                    error = CRMTranslateTextsClass.GetTranslation("General.M.MinMax", field.FullNameTextCode.Code, field.MinLength.ToString(), field.MaxLength.ToString(), field.Tenant);
                }
            }
