...
namespace Simplog.Server.Infrastructure.Helpers
{
    public static class ObjectQueryExtension
    {
        ...
        public static IQueryable<TSource> IncludeMultipleEntities<TSource>(this IQueryable<TSource> source, string path) where TSource : class
        {
            IQueryable<TSource> objectQuery=null;
            string[] paths = path.Split(',');
            foreach (string p in paths)
            {
                objectQuery = QueryableExtensions.Include(source, p);
            }
            return objectQuery;
            ...
            //    foreach (string p in paths)
            //    {
            //        objectQuery = objectQuery.Include(p);
            //    }
