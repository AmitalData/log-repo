...
namespace Amital.QuoteOPM.BL.Tools.Validating
{
    public class QuoteOPValidating
    {
        public static void Validate(QuoteOPPM entityPM, QuoteOP entityPoco, bool isNewEntity, ICommonDataContext myCommonContext)
        {
            ...
            {
                AddressValidating.ValidateQuotePickupDelivery(new QuoteDTO()
                {
                    Tenant = entityPM.Tenant,
                    DeliveryAddressId = entityPM.DeliveryAddressId,
                    FromAddressCity = entityPM.FromAddressCity,
                    FromAddressCountryId = entityPM.FromAddressCountryId,
                    IncludeDelivery = entityPM.IncludeDelivery,
                    IncludePickUp = entityPM.IncludePickUp,
                    PickUpAddressId = entityPM.PickUpAddressId,
                    ToAddressCity = entityPM.ToAddressCity,
                    ToAddressCountryId = entityPM.ToAddressCountryId,
                });
        ...
        private static void ValidateConvertQuote(QuoteOPPM entityPM)
        {
            if (IsConvertingQuoteTypeOrTransportMode(entityPM))
            {
                ...
                if (existConnectedShipments)
                {
                    ...
                    {
                        throw new ApplicationException("Cannot change QuoteOP transport mode when connected to shipments");
                    }
                    else
                    {
                        throw new ApplicationException("Cannot change QuoteOP type when connected to shipments");
                    }
                }
            }
        }
        private static bool IsConvertingQuoteTypeOrTransportMode(QuoteOPPM entityPM)
        {
        ...
        private static void ValidateAirlineRestriction(QuoteOPPM entityPM)
        {
        ...
        private static void ValidateFromPort(QuoteOPPM entityPM)
        {
                            ...
                            case "A":
                                {
                                    if (!myPort.IsAir)
                                    {
                                        throw new ApplicationException("From port transport mode is different than QuoteOP transport mode");
                                    }
        ...
        private static void ValidateToPort(QuoteOPPM entityPM)
        {
                            ...
                            case "A":
                                {
                                    if (!myPort.IsAir)
                                    {
                                        throw new ApplicationException("To port transport mode is different than QuoteOP transport mode");
                                    }
        ...
        private static void ValidateMultiVatPercentages(QuoteOPPM entityPM, ICommonDataContext myCommonContext)
        {
        ...
        private static void ValidateFCLDuplicatedPackages(QuoteOPPM entityPM)
        {
        ...
        private static void ValidateDomesticQuote(QuoteOPPM entityPM)
        {
        ...
        private static void ValidateQuoteCharges(QuoteOPPM entityPM)
        {
            ValidateQuoteCharges_SaleCurrencyMode(entityPM);

            string freightLineCostCurrencyId = null;
            string freightLineSaleCurrencyId = null;
            QuoteOPChargePM freightCharge = entityPM.QuoteCharges.Where(d => d.ChangeSetOp != Simplog.Server.Infrastructure.ChangeSetOperation.Delete && d.ChargesGroupCode == "FRT").FirstOrDefault();
        ...
        private static void ValidateQuoteCharges_SaleCurrencyMode(QuoteOPPM entityPM)
        {
            List<QuoteOPChargePM> lines = entityPM.QuoteCharges.Where(d => d.ChangeSetOp != ChangeSetOperation.Delete).ToList();

            QuoteOPChargePM freightCharge = lines.Where(d => d.ChargesGroupCode == "FRT").FirstOrDefault();

            if (lines.Count > 0)
            {
                foreach (QuoteOPChargePM item in lines)
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw new ApplicationException("All charges sale currency must be fixed to QuoteOP sale currency");
                        }
                    }
                }
            }
        ...
        private static void ValidateShipmentSubType(QuoteOPPM entityPM)
        {
            if (!entityPM.IsHybrid)
            {
        ...
        private static void ValidateRegionalTax(QuoteOPPM entityPM)
        {
            List<QuoteOPChargePM> allRegionalTaxLines = entityPM.QuoteCharges.Where(d => d.ChangeSetOp != ChangeSetOperation.Delete && d.IsRegionalTax).ToList();
