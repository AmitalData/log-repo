...
namespace Amital.QuoteOPM.BL.EntityUpdateServices
{
    public partial class QuoteOPUpdateService //: EntityUpdateService<QuoteOP, QuoteOPPM, EntityPM>
    {
                ...
                if ((entityPM.QuoteCharges.Count() == 0 && !entityPM.IsHybrid))
                {
                    ...
                    if (list_ChargeTypes.Count > 0)
                    {
                        ...
                        if (this.isLCLQuote)
                        {
                            ...
                            {
                                var QuoteOPChargePM = new QuoteOPChargePM()
                                {
                                    Tenant = tenant,
                                    ...
                                    UpdatedByUserId = _Initializer.LoggedContactId,
                                ...
                                {
                                    QuoteOPChargePM.CostMeasurementId = chargesType.MeasurementId;
                                    QuoteOPChargePM.SaleMeasurementId = chargesType.MeasurementId;
                                    ...
                                    {
                                        QuoteOPChargePM.CostMeasurementCode = iMeasurement.Code;
                                        QuoteOPChargePM.SaleMeasurementCode = iMeasurement.Code;
                                        QuoteOPChargePM.CostMeasurementShortName = iMeasurement.ShortName;
                                        QuoteOPChargePM.SaleMeasurementShortName = iMeasurement.ShortName;
                                    }
                                }
                                ...
                                {
                                    QuoteOPChargePM.CostCurrencyId = chargesType.PayablesDefaultCurrencyId;
                                }
                                ...
                                else
                                {
                                    ...
                                    {
                                        QuoteOPChargePM.CostCurrencyId = loggedTenant.FreightCurrencyId;
                                    }

                                    else
                                    {
                                        QuoteOPChargePM.CostCurrencyId = loggedTenant.OtherChargesCurrencyId;
                                    }
                                }

                                if (QuoteOPChargePM.CostCurrencyId != null)
                                {
                                    CurrencyRate iCurrencyRate = AllRates.Where(d => d.Id == QuoteOPChargePM.CostCurrencyId).FirstOrDefault();
                                    if (iCurrencyRate == null)
                                    {
                                        iCurrencyRate = this.GetCurrencyRate(loggedTenant, QuoteOPChargePM.CostCurrencyId, myQuery);
                                        AllRates.Add(iCurrencyRate);
                                    }

                                    if (iCurrencyRate != null)
                                    {
                                        QuoteOPChargePM.CostExchangeRate = MethodHelper.Round(iCurrencyRate.Rate, 5);
                                    }
                                }

                                if (entityPM.IsSaleCurrencySameAsCost)
                                {
                                    QuoteOPChargePM.SaleCurrencyId = QuoteOPChargePM.CostCurrencyId;
                                    QuoteOPChargePM.SaleExchangeRate = QuoteOPChargePM.CostExchangeRate;
                                }
                                ...
                                else if (entityPM.IsMultiCurrency)
                                {
                                    ...
                                    {
                                        QuoteOPChargePM.SaleCurrencyId = chargesType.ReceivablesDefaultCurrencyId;
                                    }
                                    ...
                                    else
                                    {
                                        if (chargesType.ChargesGroupCode == "FRT" || chargesType.ChargesGroupCode == "SCH")
                                        {
                                            QuoteOPChargePM.SaleCurrencyId = loggedTenant.FreightCurrencyId;
                                        }

                                        else
                                        {
                                            QuoteOPChargePM.SaleCurrencyId = loggedTenant.OtherChargesCurrencyId;
                                        }
                                    }

                                    // Get Rate
                                    if (QuoteOPChargePM.SaleCurrencyId != null)
                                    {
                                        CurrencyRate iCurrencyRate = AllRates.Where(d => d.Id == QuoteOPChargePM.SaleCurrencyId).FirstOrDefault();
                                        if (iCurrencyRate == null)
                                        {
                                            iCurrencyRate = this.GetCurrencyRate(loggedTenant, QuoteOPChargePM.SaleCurrencyId, myQuery);
                                            AllRates.Add(iCurrencyRate);
                                        }

                                        if (iCurrencyRate != null)
                                        {
                                            QuoteOPChargePM.SaleExchangeRate = MethodHelper.Round(iCurrencyRate.Rate, 5);
                                        }
                                    }
                                }
                                ...
                                {
                                    QuoteOPChargePM.SaleCurrencyId = entityPM.SaleCurrencyId;
                                    QuoteOPChargePM.SaleExchangeRate = entityPM.ExchangeRate;
                                }

                                if (entityPM.QuoteTypeCode == "A")
                                {
                                    switch (QuoteOPChargePM.CostMeasurementCode)
                                    {
                                        case "GRWT": { QuoteOPChargePM.CostQuantity = entityPM.GrossWeight; break; }
                                        case "CHWT": { QuoteOPChargePM.CostQuantity = entityPM.ChargeableWeight; break; }
                                        case "VOLU": { QuoteOPChargePM.CostQuantity = entityPM.Volume; break; }
                                        case "FIXD": { QuoteOPChargePM.CostQuantity = 1; break; }
                                        case "BCNT": { QuoteOPChargePM.CostQuantity = null; break; }
                                        case "BTEU": { QuoteOPChargePM.CostQuantity = entityPM.TEU; break; }
                                        case "PRVL": { QuoteOPChargePM.CostQuantity = entityPM.ValueOfGoods; break; }
                                        case "QTY": { QuoteOPChargePM.CostQuantity = entityPM.NumberOfPackages; break; }
                                        case "CWKG": { QuoteOPChargePM.CostQuantity = entityPM.ChargeableWeightInKG; break; }
                                        case "GWKG": { QuoteOPChargePM.CostQuantity = entityPM.GrossWeightInKG; break; }
                                        case "PDCW": { QuoteOPChargePM.CostQuantity = entityPM.PickupDeliveryChargeableWeight; break; }
                                        default: { break; }
                                    }

                                    switch (QuoteOPChargePM.SaleMeasurementCode)
                                    {
                                        case "GRWT": { QuoteOPChargePM.SaleQuantity = entityPM.GrossWeight; break; }
                                        case "CHWT": { QuoteOPChargePM.SaleQuantity = entityPM.ChargeableWeight; break; }
                                        case "VOLU": { QuoteOPChargePM.SaleQuantity = entityPM.Volume; break; }
                                        case "FIXD": { QuoteOPChargePM.SaleQuantity = 1; break; }
                                        case "BCNT": { QuoteOPChargePM.SaleQuantity = null; break; }
                                        case "BTEU": { QuoteOPChargePM.SaleQuantity = entityPM.TEU; break; }
                                        case "PRVL": { QuoteOPChargePM.SaleQuantity = entityPM.ValueOfGoods; break; }
                                        case "QTY": { QuoteOPChargePM.SaleQuantity = entityPM.NumberOfPackages; break; }
                                        case "CWKG": { QuoteOPChargePM.SaleQuantity = entityPM.ChargeableWeightInKG; break; }
                                        case "GWKG": { QuoteOPChargePM.SaleQuantity = entityPM.GrossWeightInKG; break; }
                                        case "PDCW": { QuoteOPChargePM.SaleQuantity = entityPM.PickupDeliveryChargeableWeight; break; }
                                        default: { break; }
                                    }

                                    if (entityPM.IsChargesByVAT)
                                    {
                                        QuoteOPChargePM.VatTypeId = chargesType.VatTypeId;

                                        if (QuoteOPChargePM.VatTypeId != null)
                                        {
                                            VatType myVatType = this.allVatTypes.Where(d => d.Id == QuoteOPChargePM.VatTypeId).FirstOrDefault();
                                            if (myVatType != null)
                                            {
                                                QuoteOPChargePM.VatTypeName = myVatType.EnglishName;
                                                QuoteOPChargePM.VatIsMultiPercentage = myVatType.IsMultiPercentage;
                                                ...
                                                {
                                                    VatTypePercentagePM myPercentagePM = allVatPercentages.Where(d => d.VatTypeId == QuoteOPChargePM.VatTypeId).FirstOrDefault();
                                                    if (myPercentagePM != null)
                                                    {
                                                        QuoteOPChargePM.VatPercentage = myPercentagePM.Percentage;
                                                    }
                                                }
                                            }
                                        }
                                ...
                                entityPM.QuoteCharges.Add(QuoteOPChargePM);
                            }
                            #endregion
                        }
                        ...
                        else
                        {
                            ...
                            {
                                var itemPM = new QuoteOPChargePM()
                                {
                                    Tenant = tenant,
                                    QuoteOPId = entityPM.Id,
                                    ...
                                    UpdatedByUserId = this._Initializer.LoggedContactId,
        ...
        public void ComputeChargesAmounts()
        {
            ...
            if (isNewEntity)
            {
                if (entityPM.QuoteCharges.Count > 0 && !entityPM.IsHybrid)
                {
                    ...
                    foreach (var item in entityPM.QuoteCharges.Where(d => d.CostMeasurementCode != "PRFR" && d.SaleMeasurementCode != "PRFR"))
                    {
                    ...
                    foreach (QuoteOPChargePM item in entityPM.QuoteCharges.Where(d => d.CostMeasurementCode == "PRFR"))
                    {
                        this.ComputeLineCostQuantity(item);
                        this.ComputeLineCostUnitPrice(item);
                        this.ComputeLineCostTotalAmounts(item);
                    ...
                    foreach (QuoteOPChargePM item in entityPM.QuoteCharges.Where(d => d.SaleMeasurementCode == "PRFR"))
                    {
                        this.ComputeLineSaleQuantity(item);
                        this.ComputeLineSaleUnitPrice(item);
                        this.ComputeLineSaleTotalAmounts(item);
                    ...
                    QuoteOPChargePM itemFreight = entityPM.QuoteCharges.Where(d => d.ChargesGroupCode == "FRT").FirstOrDefault();
                    if (itemFreight != null)
                    {
                        foreach (QuoteOPChargePM item in entityPM.QuoteCharges.Where(d => d.ChargesGroupCode != "FRT" && d.IsAllIN))
                        {
        ...
        private void ComputeLineCostQuantity(QuoteOPChargePM item)
        {
        ...
        private void ComputeLineSaleQuantity(QuoteOPChargePM item)
        {
            double? myResult = null;
        ...
        private void ComputeLineCostUnitPrice(QuoteOPChargePM item)
        {
            if (this.entityPM.IsSaleCurrencySameAsCost)
            {
                item.CostUnitPriceInSaleCurrency = item.CostUnitPrice.GetValueOrDefault();
                item.CostUnitPrice1InSaleCurrency = item.CostContainerType1UnitPrice.GetValueOrDefault();
                item.CostUnitPrice2InSaleCurrency = item.CostContainerType2UnitPrice.GetValueOrDefault();
                item.CostUnitPrice3InSaleCurrency = item.CostContainerType3UnitPrice.GetValueOrDefault();
                item.CostUnitPrice4InSaleCurrency = item.CostContainerType4UnitPrice.GetValueOrDefault();
                item.CostUnitPrice5InSaleCurrency = item.CostContainerType5UnitPrice.GetValueOrDefault();
            }
            ...
            else
            {
                ...
                item.CostUnitPriceInSaleCurrency = (double)MethodHelper.Round(item.CostUnitPrice * item.CostExchangeRate / saleExchangeRate, 3);
                item.CostUnitPrice1InSaleCurrency = (double)MethodHelper.Round(item.CostContainerType1UnitPrice * item.CostExchangeRate / saleExchangeRate, 3);
                item.CostUnitPrice2InSaleCurrency = (double)MethodHelper.Round(item.CostContainerType2UnitPrice * item.CostExchangeRate / saleExchangeRate, 3);
                item.CostUnitPrice3InSaleCurrency = (double)MethodHelper.Round(item.CostContainerType3UnitPrice * item.CostExchangeRate / saleExchangeRate, 3);
                item.CostUnitPrice4InSaleCurrency = (double)MethodHelper.Round(item.CostContainerType4UnitPrice * item.CostExchangeRate / saleExchangeRate, 3);
                item.CostUnitPrice5InSaleCurrency = (double)MethodHelper.Round(item.CostContainerType5UnitPrice * item.CostExchangeRate / saleExchangeRate, 3);
            }
        }
        private void ComputeLineSaleUnitPrice(QuoteOPChargePM item)
        {
        ...
        private void ComputeLineCostTotalAmounts(QuoteOPChargePM item)
        {
        ...
        private void ComputeLineSaleTotalAmounts(QuoteOPChargePM item)
        {
            double? myTotalAmount = null;
        ...
        private void ComputeLineCostAmountInSaleCurrency(QuoteOPChargePM item)
        {
            double? myResult = null;
        ...
        private void ComputeLineAllInAmount(QuoteOPChargePM item, QuoteOPChargePM itemFreight)
        {
        ...
        private void ComputeLineAllInAmount_LCL(QuoteOPChargePM item, QuoteOPChargePM itemFreight)
        {
        ...
        private void ComputeLineAllInAmount_FCL(QuoteOPChargePM item, QuoteOPChargePM itemFreight)
        {
            if (item.CostMeasurementCode == "BCNT")
            {
                double? itemTotalAmount = null;
        ...
        {
            List<QuoteOPChargePM> allCharges = new List<QuoteOPChargePM>();
            ...
            else
            {
                allCharges = //this.quoteChargesChangeSet.Where(d => d.ChangeSetOp != ChangeSetOperation.Delete).ToList();
                    this.entityPM.DeletedQuoteCharges.Union(this.entityPM.QuoteCharges.Where(d => d.ChangeSetOp != ChangeSetOperation.Delete).ToList()).ToList();
            }

            foreach (QuoteOPChargePM item in allCharges)
            {
