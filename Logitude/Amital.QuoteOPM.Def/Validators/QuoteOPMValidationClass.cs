...
namespace Amital.QuoteOPM.Def.Validators
{
    public partial class QuoteOPMValidationClass : IQuoteOPMValidateContext
    {
        ...
        public QuoteOPMValidationClass(string objectTableName, int tenant)
        {
            this.tenant = tenant;
        ...
        public string GetErrorMessage(object value, object instance, string propertyName)
        {
            ...
            if (field.IsRequiered)
            {
                ...
                if (value == null)
                {
                    error = QuoteOPMTranslateTextsClass.GetTranslation("General.M.FieldIsRequired", field.FullNameTextCode.Code, null, null, field.Tenant);
                }
            ...
            if (field.DataTypeCode == "Text")
            {
                ...
                if (!string.IsNullOrEmpty(valueString))
                {
                    if (!field.IsMaxLength)
                    {
                        if (valueString.Length > field.MaxLength || valueString.Length < field.MinLength)
                        {
                            error = QuoteOPMTranslateTextsClass.GetTranslation("General.M.MinMax", field.FullNameTextCode.Code, field.MinLength.ToString(), field.MaxLength.ToString(), field.Tenant);
