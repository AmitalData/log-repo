namespace Logitude.DatabaseMigration.Migrations
{
    ...
    public partial class AddFeatureUniqeCode_Rabaia : DbMigration
    {
        public override void Up()
        {
            ...
            Sql("update Queries set FeatureUniqeCode = (select FeatureUniqeCode from Features where Id = FeatureId)");
            AddColumn("dbo.MenuButtons", "FeatureUniqeCode", c => c.String(maxLength: 120, unicode: false));
            Sql("update MenuButtons set FeatureUniqeCode = (select FeatureUniqeCode from Features where Id = FeatureId)");
            AddColumn("dbo.MenusTables", "FeatureUniqeCode", c => c.String(maxLength: 120, unicode: false));
            Sql("update MenusTables set FeatureUniqeCode = (select FeatureUniqeCode from Features where Id = FeatureId)");
            AddColumn("dbo.ObjectTableHelperControls", "FeatureUniqeCode", c => c.String(maxLength: 120, unicode: false));
            Sql("update ObjectTableHelperControls set FeatureUniqeCode = (select FeatureUniqeCode from Features where Id = FeatureId)");
            AddColumn("dbo.ObjectTableTabs", "FeatureUniqeCode", c => c.String(maxLength: 120, unicode: false));
            Sql("update ObjectTableTabs set FeatureUniqeCode = (select FeatureUniqeCode from Features where Id = FeatureId)");
