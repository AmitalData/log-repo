...
namespace Logitude.CustomsMessaging.MessagingServices
{
	...
	: IMessagingServiceBase<TRequestParams, TResponseData, TCustomsRequest, TCustomsResponse, TRequestHeader>
	{
		...
		private string UploadOrCheckSFTP(int tenant, byte[] filedata, string fileName, out string messageOut, bool isSend)
		{
			...
			byte[] EncryptPrivateKey = pmCustomsPartnerFtp.MyFtpDetail.PrivateKey;
			string PrivateKeyTempPath = DecryptFromKeyVaultAsync(EncryptPrivateKey).GetAwaiter().GetResult();
			string ftpPrivateKeyPath = PrivateKeyTempPath ?? Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "PRK.PPK");
			string ftpFolderName = pmCustomsPartnerFtp.MyFtpDetail.Folder;
			string ftpPort = pmCustomsPartnerFtp.MyFtpDetail.Port ?? "2023";
			string p_message = "";
			string p_status = "";
		...
		public  async Task<string> DecryptFromKeyVaultAsync(byte[] encryptedBytes)
		{
			...
			var filePath = Path.Combine(tempPath, Guid.NewGuid().ToString() + ".PPK");
