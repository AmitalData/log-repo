...
//        public FeatureQuery(IRepository<IAmitalCloudContext,Feature, string> repository) => this.repository = repository;
        ...
        private List<RoleFeature> GetallRoleFeatures(Role myRole, List<FeaturePM> allFeatures)
        {
            List<RoleFeature> allRoleFeatures = new List<RoleFeature>();
            if (myRole.IsCustomRole)
            {
                Role myParentRole = (from d in context.Roles where d.Id == myRole.ParentRoleId select d).FirstOrDefault();
                if (myParentRole != null)
                {
                    allRoleFeatures = (from d in context.RoleFeatures
                                       where
        ...
        {
            List<PackageFeature> packageFeature = (from a in context.PackageFeatures
        ...
        LoggedUserFeatures GetAllowedFeaturesForLoggedUserBL(string loggedUserId, int tenant)
        {
            ...
            IRepository<ContactTenant> contactTenantsRepository = new Repository<ContactTenant>(context);
            IRepository<ContactTenantRole> contactTenantRolesRepository = new Repository<ContactTenantRole>(context);
            ...
            if (contactTenant == null)
            {
                ...
                {
                    contactTenantsRepository = new Repository<ContactTenant>(context);
                    contactTenantRolesRepository = new Repository<ContactTenantRole>(context);
        ...
        public bool CheckIfFeatureshasAccess(string myRoleId, List<string> allowedPackages, int tenant, string FeatureCode, string ObjectTableId)
        {
            ...
            if (!string.IsNullOrEmpty(myRoleId))
            {
                ...
                IQueryable<RoleFeature> iQueryable =
                    (from a in context.RoleFeatures
                     where a.Tenant == tenant || a.Tenant == 0
                     select a);
                ...
                #region allPackageFeatures
                allPackageFeatures = (from a in context.PackageFeatures where allowedPackages.Contains(a.PackageCode) && (a.Tenant == tenant || a.Tenant == 0) && a.Feature.Code == FeatureCode && a.Feature.ObjectTableId == ObjectTableId select a).ToList();
                #endregion
