...
IF OBJECT_ID('[dbo].[usp_DeleteCustomerRecords]', 'P') IS NOT NULL
drop PROCEDURE [dbo].usp_DeleteCustomerRecords
GO
...
BEGIN
		...
		BEGIN
			...
			delete from ContactTenantRoleSet where ContactTenantId = @ContactId
			delete from ContactTenants where ContactId = @ContactId

			delete from Users where Id = @ContactId
