...
{
    [TestClass]
    public class TruckerTests
    {
        [TestMethod]
        public void GetSingleTruckerPMTest()
        {
            ...
            TruckerPM truckerPM = partnerDomainService.GetTruckerById("1-1", 1);
            Assert.AreEqual("1-1", truckerPM.Id);
        }


        [TestMethod]
        [ExpectedException(typeof(AutenticationException))]
        public void GetSingleTruckerPMWithAutenticationOnTenantTest()
        {
        ...
        [TestMethod]
        [ExpectedException(typeof(AutenticationException))]
        public void GetSingleTruckerPMFromAnoterTenantTest()
        {
            ...
            Mock.Arrange(() => SecurityUtility.CheckContactFeature(Arg.IsAny<string>(), Arg.IsAny<string>(), Arg.IsAny<int>())).DoNothing();
            TruckerPM truckerPM = partnerDomainService.GetTruckerById("1-2", 2);
          
        }

        [TestMethod]
        public void CreateTruckerTest()
        {
            ...
            CacheManager.CacheWrapper = new MockCacheWrapper();
            TruckerPM newTrucker = new TruckerPM()
            {
                Id = "1-9",
                Tenant = tenant,
            ...
            partnerDomainService.InsertTrucker(newTrucker);
            Trucker trucker = commonContext.Truckers.Where(d => d.Id == "1-9").FirstOrDefault();
            Assert.AreNotEqual(trucker, null);
        }


        [TestMethod]
        public void UpdateTruckerTest()
        {
            GeneralMocking.MockHttpContext("user1@fnarsoft.com");
            CacheManager.CacheWrapper = new MockCacheWrapper();
            MockCommonContext commonContext = new MockCommonContext();
            ...
            truckerPM.AddedManually = true;
            partnerDomainService.UpdateTrucker(truckerPM);

            Trucker trucker = commonContext.Truckers.Where(d => d.Id == "1-1").FirstOrDefault();

            Assert.AreEqual(true, trucker.AddedManually);

        }

        [TestMethod]
        public void GetTruckerFiltersTest()
        {
            ...
            queryoperations.ObjectTableName = "Trucker";
            ...
            IQueryable<TruckerList> list = partnerDomainService.GetTruckerFilters(bytearray, 1);
