...
{
    [TestClass]
    public class ARInvoiceStatusTests
    {
        [TestMethod]
        public void GetSingleARInvoiceStatusPMTest()
        {
            ...
            ARInvoiceStatusPM aRInvoiceStatuspm = invoiceDomainService.GetSingleInvoiceStatus("AD", 1);
            Assert.AreEqual("AD", aRInvoiceStatuspm.Code);
        }

        [TestMethod]
        [ExpectedException(typeof(AutenticationException))]
        public void GetSingleARInvoiceStatusPMWithAuthenticationOnTenantTest()
        {
        ...
        [TestMethod]
        [ExpectedException(typeof(AutenticationException))]
        public void GetSingleARInvoiceStatusPMFromAnoterTenantTest()
        {
            ...
            Mock.Arrange(() => SecurityUtility.CheckContactFeature(Arg.IsAny<string>(), Arg.IsAny<string>(), Arg.IsAny<int>())).DoNothing();
            ARInvoiceStatusPM aRInvoiceStatuspm = invoiceDomainService.GetSingleInvoiceStatus("AD",2);
       
        }


        [TestMethod]
        public void GetARInvoiceStatusFiltersTest()
        {
            ...
            queryoperations.ObjectTableName = "ARInvoiceStatus";
            queryoperations.PageIndex = 0;
            queryoperations.PageSize = 10;
            queryoperations.QueryFilterItems = new List<QueryFilterItem>();
            queryoperations.QueryFilterItems.Add(new QueryFilterItem() { DisplayInList = true, FieldName = "Code" });
            ...
            IQueryable<ARInvoiceStatusList> list = invoiceDomainService.GetInvoiceStatusFilters(bytearray, 1);
