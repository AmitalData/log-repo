...
namespace CustomsWorkerRole.L2U
{
    abstract class ReceivedDbMessageAction
    {
        ...
        bool Parse()
        {
            ...
            _CommunicationLogId = _ReceivedCustomDBQueueResponse.Properties["CommunicationLogId"].ToString();

            int.TryParse(_ReceivedCustomDBQueueResponse.Properties["Tenant"].ToString(), out _Tenant);
        ...
        public void ProccessReceivedMessage()
        {
            ...
            var Retries = _ReceivedCustomDBQueueResponse.GetProperty<int>( QueueExt.QueuePropertyNames.Retries, 0);
            ...
            catch (Exception exc)
            {
                ...
                var i=_ReceivedCustomDBQueueResponse.GetProperty<int>(QueueExt.QueuePropertyNames.Retries, 0);

                _ReceivedCustomDBQueueResponse.SetProperty<int>(QueueExt.QueuePropertyNames.Retries, ++i);
