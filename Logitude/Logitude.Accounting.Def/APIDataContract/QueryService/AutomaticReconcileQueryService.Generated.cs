...
{ 
   public partial class AutomaticReconcileQueryService
   {
		...
		public AutomaticReconcile GetAutomaticReconcileByCode(string Code,int Tenant)
        { 
		    try
            {
				...
				var temp = query.GetSinglePM(Code,Tenant);				
				 if (temp == null)
                    throw new ApplicationException("AutomaticReconcile with Code " + Code + " doesn't exist");
