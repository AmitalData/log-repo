 ...
 set @MaxAutomaticLastUpdateDate = (select  MAX( AutomaticLastUpdateDate) AutomaticLastUpdateDate from dw_Partners )
 ...
 begin
   ...
   declare @LastShipmentDate as datetime
	...
	if(@Key is  null) begin insert into DIM_Partners (Id,Code,Name,[Local Name],City,Country,[State Name],[Zip Code],[Primary Contact],[Account Manager],[Customer Team],Salesman,[Customer Rank],[Partner Type],[Source Tenant],[Parent Tenant] ,[Country Code],[Primary Contact Email] , [Receivables Accounting Card]  ,[Address1],[Address2],[Phone] ,[Region],[Customer Size],[Industry],[Vat Number] , [Credit Limit Amount (Local)] , [Open Balance (Local)] , [Lead Source], [Automatic Last Update Date], [InActive], [Customer First Shipment Date], [Customer Last Shipment Date] ,[Bill To]) values(@Id,@Code, @Name,@LocalName ,@City,@Country, @State, @ZipCode , @PrimaryContact , @AccountManager, @CustomerTeam , @Salesman ,@Rank , @PartnerType,  @SourceTenant , @ParentTenant,@CountryCode,@PrimaryContactEmail, @ReceivablesAccountingCard,@address1, @address2,@Phone,@Region,@CustomerSize,@Industry ,@VatNumber,@CreditLimitAmount , @CreditLimitOpenBalance , @LeadSource, @AutomaticLastUpdateDate, @InActive, @FirstShipmentDate, @LastShipmentDate , @BillTo)end
	else begin update   DIM_Partners set Name =@Name,Code= @Code, [Local Name] =@LocalName ,  City = @City , Country = @Country,  [State Name] = @State, [Zip Code] = @ZipCode ,  [Primary Contact] = @PrimaryContact,[Account Manager] = @AccountManager,[Customer Team] = @CustomerTeam,Salesman = @Salesman, [Customer Rank] = @Rank,[Partner Type] = @PartnerType , [Source Tenant] = @SourceTenant , [Parent Tenant] = @ParentTenant ,[Country Code] = @CountryCode,[Primary Contact Email] = @PrimaryContactEmail , [Receivables Accounting Card] = @ReceivablesAccountingCard , [Address1] = @address1, [Address2] = @address2, [Phone] = @Phone ,[Region] =@Region ,[Customer Size]=@CustomerSize ,[Industry]=@Industry,[Vat Number]=@VatNumber ,[Credit Limit Amount (Local)]=@CreditLimitAmount , [Open Balance (Local)]=@CreditLimitOpenBalance , [Lead Source] =@LeadSource, [Automatic Last Update Date] = @AutomaticLastUpdateDate, [InActive] = @InActive, [Customer First Shipment Date] = @FirstShipmentDate, [Customer Last Shipment Date] = @LastShipmentDate , [Bill To]=@BillTo where Id = @Id; end
