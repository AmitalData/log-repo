 ...
 begin
   ...
   declare @InActive as bit
	...
	else begin update   DIM_Partners set Name =@Name,Code= @Code, [Local Name] =@LocalName ,  City = @City , Country = @Country,  [State Name] = @State, [Zip Code] = @ZipCode ,  [Primary Contact] = @PrimaryContact,[Account Manager] = @AccountManager,Salesman = @Salesman, [Customer Rank] = @Rank,[Partner Type] = @PartnerType , [Source Tenant] = @SourceTenant , [Parent Tenant] = @ParentTenant ,[Country Code] = @CountryCode,[Primary Contact Email] = @PrimaryContactEmail , [Receivables Accounting Card] = @ReceivablesAccountingCard , [Address1] = @address1, [Address2] = @address2, [Phone] = @Phone ,[Region] =@Region ,[Customer Size]=@CustomerSize ,[Industry]=@Industry,[Vat Number]=@VatNumber ,[Credit Limit Amount (Local)]=@CreditLimitAmount , [Open Balance (Local)]=@CreditLimitOpenBalance , [Lead Source] =@LeadSource, [Automatic Last Update Date] = @AutomaticLastUpdateDate, [InActive] = @InActive  where Id = @Id; end
