  ...
  declare @MAWBOBLDate as datetime
   declare @Transshipment1AdditionalMAWBOBLBL as nvarchar(20)
	...
	@IncludesCustoms,@DeclarationNumber,@DeclarationDate,@CustomsClearanceDate,@TerminalAvailable,@WarehouseLegLastFreeDate,@WarehouseLegActualEntryDate,@WarehouseLegExpectedEntryDate,@WarehouseLegActualReleaseDate,@WarehouseLegExpectedReleaseDate,@VolumetricWeight,@ChargeableWeightUnitCode,@Ratio,@FirstPickupATD, 
    ...
    @MAWBOBLDate,@Transshipment1AdditionalMAWBOBLBL,@FirstPickupLocation,@ContainersNumbers																								 
	...
	----------------------------------------------
	 ...
	 -------------------VolumetricWeight---------------
	 SET @FinalVolumetricWeight = @VolumetricWeight +'('+ @ChargeableWeightUnitCode +')';
	 --------------------------------------------------


	 -------------------Ratio---------------
	 SET @FinalRatio = '1:' + @Ratio;
	 --------------------------------------------------
	 ...
	 ------------------------------------------------------
      ...
      insert into #Fact_ShipmentsTemp ([Id],[Source Tenant],[Parent Tenant],[Direction],[Transport Mode],[DirectHouse],[Type],[Department],[Branch],[Shipment Number],[House],[Master],[Shipper],[Consignee],[Agent],[Customer],[Incoterm],[Gross Weight (KG)],[Chargeable Weight (KG)],[Total Volume (CBM)],[Number of Packages],[Number of Containers],[Salesman],[Account Manager],[Profit ( Local )],[Profit],[Local Currency ],[Profit Currency],[Operationally Closed],[Accounting Closed],[Status],
									   ...
									   [IncludesCustoms],[DeclarationNumber], [DeclarationDate],[CustomsClearanceDate],[TerminalAvailable],[Warehouse Last free Date],[Warehouse Entry Date],[Warehouse Release Date],[VolumetricWeight],[Ratio],[FirstPickupATD],[FirstPickupATA],[FinalDeliveryETD],[FinalDeliveryETA],[FinalDeliveryATD],[FinalDeliveryATA],[Transshipment1ETA],[Transshipment1ETD],[Transshipment1ATA],[Transshipment1ATD],[Transshipment 1 Vessel],[Transshipment1Carrier],[Transshipment 1 Master Date],[Transshipment 1 Master],[First Pickup Location],[ContainersNumbersArray]) 
      ...
      values(@Id, @SourceTenant,@ParentTenant,@Direction,@TransportMode, @DirectHouse, @Type , @Department ,@Branch , @ShipmentNumber , @House ,@Master , @Shipper,  @Consignee , @Agent,@Customer,@Incoterm ,@TotalGrossWeightInKG,@TotalChargeableWeightInKG, @TotalVolumeInCBM,  @NumberOfPackages, @NumberOfContainers, @Salesman , @AccountManager ,    @TotalProfitInLocalCurrency , @TotalProfitInProfitCurrency , @LocalCurrency,@ProfitCurrency ,@OperationallyClosed,@AccountingClosed, @Status, @Location,  @MainCarriageFromPort , @FinalDestination , @IsDeparted ,
	         ...
	         @IncludesCustoms,@DeclarationNumber, @DeclarationDate,@CustomsClearanceDate, @TerminalAvailable,@WarehouseLegLastFreeDate,@WarehouseLegEntryDate ,@WarehouseLegReleaseDate,@FinalVolumetricWeight,@FinalRatio,@FirstPickupATD,@FirstPickupATA,@FinalDeliveryETD,@FinalDeliveryETA,@FinalDeliveryATD,@FinalDeliveryATA,@Transshipment1ETA,@Transshipment1ETD,@Transshipment1ATA,@Transshipment1ATD,@Transshipment1Vessel ,@Transshipment1Carrier,@MAWBOBLDate,@Transshipment1AdditionalMAWBOBLBL,@FirstPickupLocation,@ContainersNumbers)
