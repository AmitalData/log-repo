...
--Tenants
 ...
 declare @SQLShipmentPayables  as varchar(8000)
         SET @SQLShipmentPayables ='CREATE TRIGGER Trigger_AutomaticLastUpdateDateShipmentPayables ON ShipmentPayables AFTER UPDATE  AS  BEGIN UPDATE ShipmentPayables SET AutomaticLastUpdateDate = GETDATE() WHERE Id IN (SELECT DISTINCT Id FROM Inserted)END;'
         EXEC (@SQLShipmentPayables);
 end
 --APInvoiceLines
  IF not EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'Trigger_AutomaticLastUpdateDateAPInvoiceLines'))
 Begin
 declare @SQLAPInvoiceLines  as varchar(8000)
         SET @SQLAPInvoiceLines ='CREATE TRIGGER Trigger_AutomaticLastUpdateDateAPInvoiceLines ON APInvoiceLines AFTER UPDATE  AS  BEGIN UPDATE APInvoiceLines SET AutomaticLastUpdateDate = GETDATE() WHERE APInvoiceId IN (SELECT DISTINCT APInvoiceId FROM Inserted)END;'
         EXEC (@SQLAPInvoiceLines);
 end

 --APInvoices

  IF not EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'Trigger_AutomaticLastUpdateDateAPInvoices'))
 Begin
 declare @SQLAPInvoices  as varchar(8000)
         SET @SQLAPInvoices ='CREATE TRIGGER Trigger_AutomaticLastUpdateDateAPInvoices ON APInvoices AFTER UPDATE  AS  BEGIN UPDATE APInvoices SET AutomaticLastUpdateDate = GETDATE() WHERE Id IN (SELECT DISTINCT Id FROM Inserted)END;'
         EXEC (@SQLAPInvoices);
 end

 --ShipmentReceivables
 IF not EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'Trigger_AutomaticLastUpdateDateShipmentReceivables'))
 Begin
 declare @SQLShipmentReceivables  as varchar(8000)
         SET @SQLShipmentReceivables ='CREATE TRIGGER Trigger_AutomaticLastUpdateDateShipmentReceivables ON ShipmentReceivables AFTER UPDATE  AS  BEGIN UPDATE ShipmentReceivables SET AutomaticLastUpdateDate = GETDATE() WHERE Id IN (SELECT DISTINCT Id FROM Inserted)END;'
         EXEC (@SQLShipmentReceivables);
 end


  --ARInvoiceLines
  IF not EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'Trigger_AutomaticLastUpdateDateARInvoiceLines'))
 Begin
 declare @SQLARInvoiceLines  as varchar(8000)
         SET @SQLARInvoiceLines ='CREATE TRIGGER Trigger_AutomaticLastUpdateDateARInvoiceLines ON ARInvoiceLines AFTER UPDATE  AS  BEGIN UPDATE ARInvoiceLines SET AutomaticLastUpdateDate = GETDATE() WHERE Id IN (SELECT DISTINCT Id FROM Inserted)END;'
         EXEC (@SQLARInvoiceLines);
 end

 --ARInvoices

  IF not EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'Trigger_AutomaticLastUpdateDateARInvoices'))
 Begin
 declare @SQLARInvoices  as varchar(8000)
         SET @SQLARInvoices ='CREATE TRIGGER Trigger_AutomaticLastUpdateDateARInvoices ON ARInvoices AFTER UPDATE  AS  BEGIN UPDATE ARInvoices SET AutomaticLastUpdateDate = GETDATE() WHERE Id IN (SELECT DISTINCT Id FROM Inserted)END;'
         EXEC (@SQLARInvoices);
 end
  ...
  IF not EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'Trigger_AutomaticLastUpdateDateChargesTypes'))
 Begin
 declare @SQLChargesTypes  as varchar(8000)
         SET @SQLChargesTypes ='CREATE TRIGGER Trigger_AutomaticLastUpdateDateChargesTypes ON ChargesTypes AFTER UPDATE  AS  BEGIN UPDATE ChargesTypes SET AutomaticLastUpdateDate = GETDATE() WHERE Id IN (SELECT DISTINCT Id FROM Inserted)END;'
 ...
 -- IF not EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'Trigger_AutomaticLastUpdateDateChargesGroups'))
 --Begin
 --declare @SQLChargesGroups  as varchar(8000)
 --        SET @SQLChargesGroups ='CREATE TRIGGER Trigger_AutomaticLastUpdateDateChargesGroups ON ChargesGroups AFTER UPDATE  AS  BEGIN UPDATE ChargesGroups SET AutomaticLastUpdateDate = GETDATE() WHERE Id IN (SELECT DISTINCT Id FROM Inserted)END;'
 --        EXEC (@SQLChargesGroups);
 --end