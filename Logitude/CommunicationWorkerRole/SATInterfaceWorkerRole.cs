...
namespace CommunicationWorkerRole
{
    public class SATInterfaceWorkerRole : WorkerEntryPoint
    {
            ...
            if (resultadoTimbre.Exitoso)
            {
                if (waitingCommLog.Subject == "Payment SAT Interface")
                {
                    ...
                    if (payment != null)
                    {
                        ...
                        UpdatePaymentInvoicesSATStatus(payment, comprobante, arinvoiceRep, arpaymentRep);
        ...
        private void UpdatePaymentInvoicesSATStatus(ARPayment payment, Profact.TimbraCFDI33.Comprobante paymentComprobante, ARInvoiceRepository arinvoiceRep, ARPaymentRepository arpaymentRep)
        {
            ...
            if (waitingCommLog.Subject == "Payment SAT Interface")
            {
                if (descrip.Length > 1)
                {
                    ...
                    if (payment != null && resultadoConsulta.Exitoso)
                    {
                        ...
                        arpaymentRep.Update(payment);
                        arpaymentRep.SubmitChanges();
                        UpdatePaymentInvoicesSATStatus(payment, paymentComprobante, arinvoiceRep, arpaymentRep);
