...
{
    public class ImporterApprovalReceivedWorkerRole : WorkerEntryPoint
    {
        IQueueService queue;
        string URI = "";//"http://localhost:9996/api/CustomerTenantAccess";
        APILogsService apiLogsService;

        public ImporterApprovalReceivedWorkerRole()
        {
        ...
        public override bool OnStart()
        {
            ...
            BatchServiceCode = "ImporterApprovalReceived";
            DoneItemsInRange = new Dictionary<DateTime, int>();

            try
            {
                queue = new DbQueueService();
                queue.InitializeQueue("ImporterApprovalReceivedQueue", 0);
            ...
            catch (Exception ex)
            {
                ...
                ExceptionHandler.HandleException(ex, DateTime.Now, 0, null, "ImporterApprovalReceivedQueue Role", null, ip);
        ...
        public override async void AsyncRun()
        {
                                    ...
                                    if (Data != null)
                                    {
                                        ...
                                        using (var client = new HttpClient())
                                        {
                                            ...
                                            if (IsNewLog)
                                            {
                                                ...
                                                LogPM.QueueType = "ImporterApprovalReceived";
                                ...
                                if (response.MessageValues.Keys.Contains("ShipmentId"))
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(ShipmentId))
                                    {
                                        ...
                                        if (response.RetryNumber >= 3)
                                        {
                                            ...
                                            if (IsNewLog)
                                            {
                                                LogPM.Subject = "Send ImporterApprovalReceived Confirmation To Forwarder By ForwarderShipment Controller";
                                                //LogPM.CustomerId = CustomerId;
                                                LogPM.QueueMessage = DictionaryJsonConverter.FromDictionaryToJson((Dictionary<string, string>)response.MessageValues);
            ...
            catch (Exception ex)
            {
                ...
                ConnectClient();
                ExceptionHandler.HandleException(ex, DateTime.Now, 0, null, "ImporterApprovalReceived  worker role start", null, null);
                Thread.Sleep(10000);
            }
        }
