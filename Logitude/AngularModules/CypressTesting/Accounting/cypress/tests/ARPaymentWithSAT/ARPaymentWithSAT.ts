...
Given("the user navigates to shipments workspace", () => {
  ...
  Given("an ARInvoice with the following details", (dataTable) => {
    let ARInvoiceData = Assists.CreateInstance<ARInvoiceDetails>(dataTable, true);
...
Given("the user Create an AR Payment with the following details", (dataTable) => {
    ARPaymentActions.NavigatesARPaymentWorkspaceSAT()
    let ARPaymentDetails = Assists.CreateInstance<ARPaymentDetails>(dataTable, true);
    const yesterday = new Date(new Date().getTime() - 24*60*60*1000)
    console.log('yesterday ', yesterday);
    const formatted =  `${yesterday.getDate()}/${yesterday.getMonth()+1}/${yesterday.getFullYear()}`;
    ARPaymentDetails.RegisterDate= formatted;
...
Given("search for the specific invoice", () => {
    Actions.SearchInvoice(invoiceNumber);
});


When("choose this invoice", () => {
ARPaymentActions.ChooseInvoiceNumber();
});
