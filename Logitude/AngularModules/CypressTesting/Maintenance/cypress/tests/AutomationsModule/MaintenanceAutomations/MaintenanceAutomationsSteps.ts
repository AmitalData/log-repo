...
Given("the user add the following condition", (dataTable) => {
    ...
    conditionsDetails.forEach(element => {
        AutomationsActions.AddCondition(element,i++) 
    });
});
...
Given("the user adds FU Creation result", (dataTable) => {
    let followUpCreation = Assists.CreateInstance<FollowUpCreationDetails>(dataTable, true);
    AutomationsActions.addResultFUCreation(followUpCreation)
});
...
Then("the Automation should executed successfully", () => {
    AutomationsActions.assertAutomationExcution(setFieldValueResultDetailes.Field, setFieldValueResultDetailes.Value
        ,automationsDetails.Name)
});
//#endregion
