...
When("search for {string} special services type", (specialServicesType) => {
    MaintenanceActions.SearchCardByValue(specialServicesType)
});

Then("the {string} special services type should appear successfully", (specialServicesType) => {
    MaintenanceActions.AssertSearchSpecialServicesType(specialServicesType)
});
