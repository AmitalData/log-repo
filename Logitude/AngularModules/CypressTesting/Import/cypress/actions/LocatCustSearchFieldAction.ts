...
export function FillSearchField(searchFieldDetails: SearchFieldDetails) {
    cy.FillLogTextBox(SearchFieldSelectors.SearchField, searchFieldDetails.Customer, true) 

}

export function Compare(searchFieldDetails: SearchFieldDetails) {
        debugger
    BaseAssertion.AssertElementExist(SearchFieldSelectors.FindCustInGrid.replace("{0}",searchFieldDetails.Customer))
