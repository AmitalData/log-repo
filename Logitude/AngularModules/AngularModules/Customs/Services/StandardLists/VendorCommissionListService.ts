...
export class VendorCommissionListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<VendorCommissionList> = [];
    constructor() {
			...
			return this._http.get(this._apiUrl + '/getsingle/?' + 'vendorid=' + vendorid+'&'+'customerid=' + customerid+'&'+'modificationstypecode=' + modificationstypecode, ServiceHelper.GetHttpFullHeaders())
				.pipe(			
					map((response: HttpResponse<any>) => {
						...
						var entity: VendorCommissionList;
						if (list) {
			...
			return this._http.get(this._apiUrl + '/getall', ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						var _mappedListsArray: Array<VendorCommissionList> = [];
						if (allLists) {
							for (var key in allLists) {				
								var entity: VendorCommissionList = this.MapJsonToEntityList(allLists[key]);
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "VendorCommission", "GetAllLists", ""); 
	...
	getByFilters(filters: ApiQueryFilters) {
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: VendorCommissionList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "VendorCommission", "GetByFilters", "PageIndex:" +filters.PageIndex +", PageSize:"+filters.PageSize + ", GetAll:" + filters.GetAll);
	    ...
	    MapJsonToEntityList(jsonList: any) {
            ...
            var entityList: VendorCommissionList;
            entityList = new VendorCommissionList();
