			...
			return this._http.get(this._apiUrl + '/getsingle/?' + 'code=' + code, ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "CustomsPaymentTerm", "GetSingleList", 'code=' + code); 
					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {			
								var entity: CustomsPaymentTermList = this.MapJsonToEntityList(allLists[key]);
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "CustomsPaymentTerm", "GetAll", ""); 
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: CustomsPaymentTermList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "CustomsPaymentTerm", "GetByFilters", "PageIndex:" +filters.PageIndex +", PageSize:"+filters.PageSize + ", GetAll:" + filters.GetAll); 
				...
				map((cachedJson:any) => {
					...
					if (cachedJson) {
						for (var key in cachedJson) {
							var entity: CustomsPaymentTermList = this.MapJsonToEntityList(cachedJson[key]);
							_mappedListsArray.push(entity);
						}
					}
					...
					PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "CustomsPaymentTerm", "GetSingleListFromCache", 'code=' + code); 
            ...
            return CachedDataManager.GetClosedTableData("Customs.CustomsPaymentTerm").pipe(
				map((cachedJson:any) => {
					...
					if (filters.GetAll) {
						...
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "CustomsPaymentTerm", "GetAllFromCache", "PageIndex:" + filters.PageIndex + ", PageSize:" + filters.PageSize + ", GetAll:" + filters.GetAll); 
