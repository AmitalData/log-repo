...
import {ClientIndicationList} from '../../EntityLists/ClientIndicationList';

@Injectable()

export class ClientIndicationListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<ClientIndicationList> = [];
    constructor() {
			...
			return this._http.get(this._apiUrl + '/getsingle/?' + 'indicationid=' + indicationid+'&'+'clientid=' + clientid, ServiceHelper.GetHttpFullHeaders())
				.pipe(			
					map((response: HttpResponse<any>) => {
						...
						var entity: ClientIndicationList;
						if (list) {
			...
			return this._http.get(this._apiUrl + '/getall', ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						var _mappedListsArray: Array<ClientIndicationList> = [];
						if (allLists) {
							for (var key in allLists) {				
								var entity: ClientIndicationList = this.MapJsonToEntityList(allLists[key]);
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "ClientIndication", "GetAllLists", ""); 
	...
	getByFilters(filters: ApiQueryFilters) {
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: ClientIndicationList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "ClientIndication", "GetByFilters", "PageIndex:" +filters.PageIndex +", PageSize:"+filters.PageSize + ", GetAll:" + filters.GetAll);
	    ...
	    MapJsonToEntityList(jsonList: any) {
            ...
            var entityList: ClientIndicationList;
            entityList = new ClientIndicationList();
