...
import {ExportStorageList} from '../../EntityLists/ExportStorageList';

@Injectable()

export class ExportStorageListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<ExportStorageList> = [];
    constructor() {
					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {				
								var entity: ExportStorageList = this.MapJsonToEntityList(allLists[key]);
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "ExportStorage", "GetAllLists", ""); 
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: ExportStorageList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "ExportStorage", "GetByFilters", "PageIndex:" +filters.PageIndex +", PageSize:"+filters.PageSize + ", GetAll:" + filters.GetAll);
