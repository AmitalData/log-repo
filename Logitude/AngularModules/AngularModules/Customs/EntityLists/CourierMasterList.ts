//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a logitude.
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

export class CourierMasterList {
   
	Id: string;
	Tenant: number;
	CreateDateTime: Date;
	CreatedByUserId: string;
	UpdateDateTime: Date;
	SearchFields: string;
	CreatedByUserName: string;
	AirlineId: string;
	AirlineName: string;
	MAWB: string;
	MAWBTypeName: string;
	HAWB: string;
	EstimatedArrivalDate: Date;
	GatewayPortCode: string;
	GatewayPortName: string;
	OriginPortCode: string;
	OriginPortName: string;
	IsOpen: boolean;
	IsCancelled: boolean;
	UpdatedByUserId: string;
	UpdatedByUserName: string;
	AirlinePrefix: string;
	MAWBTypeCode: string;
	ManifestNumber: string;
	PackageQuantity: number;
	GrossMassMeasure: number;
	ShortHAWB: string;
	FlightNumber: string;
	DepartureDate: Date;
	EstimatedArrivalDateOnly: Date;
	EstimatedArrivalTimeOnly: Date;
	WeightValueCode: string;
	WeightValueName: string;
	StorageSiteCode: string;
	StorageSiteName: string;
	TruckerId: string;
	IntegratorCode: string;
	IntegratorName: string;
	IntegratorNumber: string;
	IsReadyForInvoice: boolean;
	IsAllDecClosedForFollowUp: boolean;
	CalcClosedForFollowUp: number;
	CalcMissingClassification: number;
	CalcMissingImporterId: number;
	CalcPendingCustoms: number;
	CalcPending900: number;
	CalcSuspendedDeclarations: number;
	NoOfCourierHawb: string;
	IsAutomaticManifestSent: boolean;
	IsEstimatedArrivalToDay: boolean;
	EstimatedArrivalColor: string;
	PackageQuantityInMAWB: number;
	LandingDate: Date;
	UnifreightLeadingFile: string;
	LandingDateDateOnly: Date;
	LandingDateTimeOnly: Date;
	CourierMasterRemarks: string;
	OpenDeclarations: number;
	NoOfCourierHawbwWithoutHatara: string;
	NoOfCourierHawbWithoutDelivery: string;
	HawbQuantityNoDocuments: string;
	HawbQuantityNoClassification: string;
	HawbQuantityNoTransManifest: string;
	HawbQuantityNoTransDeclaration: string;
	HawbQuantityNoTransPayment: string;
	
}
 