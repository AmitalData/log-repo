...
export class OverviewTabComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    get ActivitiesContent() {
        return TextCodeTranslator.Translate("Quote.S.Overview.Activities") +  " (" + this.ActivitiesList.length + ")";
    }
    ...
    AddEmail() {
        if (!this.EmailSender || (this.EmailSender && !this.EmailSender.LoadingSendingComponent)) {
            this.EmailSender = new GeneralEmailSender("Quote", "QUOT", this.EntityPM.Id, this.EntityPM.QuoteNumber, "", "", "", "", null, "LoadActivity", this.EntityPM, true, "QEMO");
