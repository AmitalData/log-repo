...
export class ARInvoiceListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<ARInvoiceList> = [];
    constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/arinvoiceviews';  
    }
			...
			return this._http.get(this._apiUrl + '/getsingle/?' + 'id=' + id, ServiceHelper.GetHttpFullHeaders())
				.pipe(			
					map((response: HttpResponse<any>) => {
						...
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "ARInvoice", "GetSingleList", 'id=' + id); 
					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {				
								var entity: ARInvoiceList = this.MapJsonToEntityList(allLists[key]);
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "ARInvoice", "GetAllLists", ""); 
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: ARInvoiceList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "ARInvoice", "GetByFilters", "PageIndex:" +filters.PageIndex +", PageSize:"+filters.PageSize + ", GetAll:" + filters.GetAll);
