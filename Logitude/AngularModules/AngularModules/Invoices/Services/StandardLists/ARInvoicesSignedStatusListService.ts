...
export class ARInvoicesSignedStatusListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<ARInvoicesSignedStatusList> = [];
    constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/arinvoicessignedstatusviews';  
    }
			...
			return this._http.get(this._apiUrl + '/getsingle/?' + 'code=' + code, ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						var entity: ARInvoicesSignedStatusList;
						if (list) {
					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {			
								var entity: ARInvoicesSignedStatusList = this.MapJsonToEntityList(allLists[key]);
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: ARInvoicesSignedStatusList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
	...
	getSingleFromCache(code: string) {
		...
		if (ARInvoicesSignedStatusListService.CachedData.length > 0) {
			return defer(() => {
				var filteredData = ARInvoicesSignedStatusListService.CachedData.filter(a => a.Code === code)[0];
				serviceResponse.CallTime = callTime;
				...
				map((cachedJson:any) => {
					...
					if (cachedJson) {
						for (var key in cachedJson) {
							var entity: ARInvoicesSignedStatusList = this.MapJsonToEntityList(cachedJson[key]);
							_mappedListsArray.push(entity);
						}
					}

					ARInvoicesSignedStatusListService.CachedData = _mappedListsArray;
    ...
    getAllFromCache(filters: ApiQueryFilters= new ApiQueryFilters(true)) {
        ...
        if (ARInvoicesSignedStatusListService.CachedData.length > 0) {
            return defer(() => {
                if (filters.GetAll) {
					serviceResponse.Result = ARInvoicesSignedStatusListService.CachedData; 
				}

				else {
					var filteredData = InfraGenericFilter.GetFilteredArray(ARInvoicesSignedStatusListService.CachedData, filters);
					serviceResponse.Result = filteredData; 
					serviceResponse.CallTime = callTime;
				}
	    ...
	    MapJsonToEntityList(jsonList: any) {
            ...
            var entityList: ARInvoicesSignedStatusList;
            entityList = new ARInvoicesSignedStatusList();
