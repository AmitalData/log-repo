...
export class DashboardDomainService {
    ...
    GetTop10DashBoard(type:string,lastMonths: number, lastDays: number, measurment: number, currentTenant: number, top: number, includeOthers: boolean) {
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/ShipmentDomain'
        ...
        return Observable.defer(() => {
            return this._http.get(this._apiUrl + '/GetTop10DashBoard?type=' + type +'&lastMonths=' + lastMonths + '&lastDays=' + lastDays + '&measurment=' + measurment + '&currentTenant=' + currentTenant + '&top=' + top + '&includeOthers=' + includeOthers , {
                headers: authHeader
            }).map(response => {
