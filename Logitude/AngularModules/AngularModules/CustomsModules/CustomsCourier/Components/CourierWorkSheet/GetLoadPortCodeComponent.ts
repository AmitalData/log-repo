...
@Component({
    selector: 'GetLoadPortCodeComponent',
    templateUrl: './GetLoadPortCodeComponent.html',
})

export class GetLoadPortCodeComponent extends BaseComponent {
    public DataContext: GetLoadPortCodeComponent = this;
    ...
    OkButtonClicked() {
        ...
        confirm.Show("שינוי יבצע עדכון של כל ההצהרות נמל טעינה החדש וישדר את ההצהרות למכס");
        confirm.WindowClosed.subscribe((event: any) => {
            if (confirm.Yes) {
                this.ChangeLoadPortCode();
            }
            confirm.Close();
        });
    }
