        ...
        this.supplierInvoiceExtendedListService.GetSupplierInvoiceItemsForInvoice(this.DeclarationPM.Id, invoice.InvoiceCounterKey).subscribe((response: any) => {
            if (response) {
                ...
                for (let item of response.Result) {
                    ...
                    if (this.paymentProtest) {
                        selectedItem = response.Result.filter(a => a.SequenceNumeric == this.paymentProtest.GoodsItemLineNumber && a.ClassificationCode == this.paymentProtest.InvoiceItemClassificationCode && a.DeclarationId == this.paymentProtest.DeclarationId)[0];
                    }
                    ...
                    if (this.SelectedInvoiceItems.Collection.includes(item)) {
