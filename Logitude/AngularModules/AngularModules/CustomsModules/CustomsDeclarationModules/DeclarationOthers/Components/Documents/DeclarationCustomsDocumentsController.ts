        ...
        this.GeneratedCustomsDocumentTicketViewModel = [];
         if (this.declarationPM.Direction == 'E') {
            ...
            return defer(() => {
                ...
                return  custDocRelatedDocsWebService.GetDocumentsFilingsForRelatedDocuments(this.declarationPM.Id, null, table.Id, 'I', this.declarationPM.CustomFileNo, "all").pipe(
                    map((response: ServiceResponse) => {
                       // debugger;
                    //  return  custDocsTicketWebService.GetCustomsDocumentsTicketsByEntityIdAndChilds(this.declarationPM.Id, null, null, null, "Declaration").pipe(

                          //  map((response1: ServiceResponse) => {
                                var relatedDocs = response.Result;

                             //   var CustomsDocumentsTickets = response1.Result;

                                for (var i = 0; i < relatedDocs.length; i++) {

                               //     debugger;
                                    var ticket = CustomsDocumentsTickets.filter(d => d.DocumentsFilingId == relatedDocs[i].Id)[0];
