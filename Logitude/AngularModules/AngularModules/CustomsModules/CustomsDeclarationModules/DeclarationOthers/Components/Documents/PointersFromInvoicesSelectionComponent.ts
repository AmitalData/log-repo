...
export class PointersFromInvoicesSelectionComponent {
    ...
    SetWindowArgs(args: any) {
        if (args.CertificateOfOriginItem) {
            ...
            this.DeclarationPM = args.DeclarationPM;
            this.IsFromSIIRequest = args.IsFromSIIRequest;
            if (this.DeclarationPM != null && this.DeclarationPM.Direction == "E") {
                this.isExportDeclaration = true;
            }
            ...
            let getCounterKey = (pointer) => this.IsFromSIIRequest ? pointer.Child2EntityId : pointer.Child1EntityId;
            let getLineNumber = (pointer) => this.IsFromSIIRequest ? pointer.Child3EntityId : pointer.Child2EntityId;

            this.CustomsDocumentsTicket.CustomsDocumentPointers.forEach((pointer) => {
                let counterKey = getCounterKey(pointer);
                let lineNumber = getLineNumber(pointer);

                if (counterKey != null && lineNumber != null) {
                    connectedCounterKeys += "," + counterKey;
                    if (this.CustomsDocumentsTicket.ConnectedInvoiceItemsSequences.indexOf(lineNumber) > -1) {
                        connectedLineNumbers += "," + lineNumber;
                    }
                }
            });
    ...
    BuildInvoicesList() {
        ...
        this.CustomsDocumentsTicket.CustomsDocumentPointers.forEach((pointer) => {
            var invoiceCounterKey = this.IsFromSIIRequest ? pointer.Child2EntityId : pointer.Child1EntityId;
            var invoice: SupplierInvoicePM = this.Invoices.filter(d => d.InvoiceCounterKey + "" == invoiceCounterKey)[0];
            if (invoice) {
