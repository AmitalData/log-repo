...
export class SupplierInvoiceItemLine extends BaseComponent {
    ...
    public set ItemPrice(newValue: number) {
        if (newValue != this.entityPM.ItemPrice) {
            ...
            if(!AppTool.IsNullOrEmpty(this.entityPM.EntityParentPM.supplierInvoicePayments) && this.entityPM.EntityParentPM.supplierInvoicePayments.length == 1 && this.entityPM.EntityParentPM.supplierInvoicePayments[0].paymentAmount != null && hasOcr && this.CurrentSession.CurrentEditComponent.EntityPM.Direction == "E"){
                let sum = 0;
