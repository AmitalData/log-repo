...
export class LogisticActionRequestCustomsDocumentsController implements ICustomsDocumentsController {
    ...
    constructor(private logisticActionRequestPM: LogisticActionRequestPM, private childEntity1Id: string, private ChildEntity1Code: string) {
        ...
        this.IsDisplayOnly = logisticActionRequestPM.OperationalStatus === TextCodeTranslator.Translate('Customs.General.O.Failed')
    }
    ...
    GetAutoGeneratedTickets(customsDocumentsTicketViewModels: CustomsDocumentTicketViewModel[]) {
        return defer(() => {
            ...
            var IL_287exists = this.originalCustomsDocumentTicketViewModel.filter(d => d.DocumentTypeCode == 'IL_287')[0];
            if (!IL_287exists) {
                var entityParams: RelatedEntityParams = new RelatedEntityParams()
                entityParams.ParentEntityCode = 'LogisticActionRequest';
                entityParams.ParentEntityId = this.logisticActionRequestPM.Id;
                var otherModelTicket: CustomsDocumentsTicketPM = this.GetGeneratedCustomTicketAndPointer(entityParams, "IL_287");
                ...
                var otherModelTicketViewModel: CustomsDocumentTicketViewModel = new CustomsDocumentTicketViewModel(otherModelTicket, null, true,
                    this.IsDisplayOnly, this.logisticActionRequestPM, "Customs.LogisticActionRequest", this);
    ...
    DisplayOnlyCheck() {
        ...
        return defer(() => {
            var rresponse: ServiceResponse = new ServiceResponse();
            rresponse.Result = { IsDisplayOnly: this.IsDisplayOnly, DisplayOnlyMessage: "מסר כבר נשלח" };
            return of(rresponse);
        });
    }
