...
var GenericFilter = /** @class */ (function () {
    function GenericFilter() {
    }
    GenericFilter.ApplyFilters = function (apiFilter, dataArray) {
    ...
    GenericFilter.AddFilter = function (filterName, filterValue, filterOperator) {
        switch (filterOperator.toLowerCase()) {
            case ('startswith'): {
                this.StaticDataArray = this.StaticDataArray.filter(function (v) { return v[filterName].startsWith(filterValue); });
                break;
            }
            case ('contains'): {
                this.StaticDataArray = this.StaticDataArray.filter(function (v) { return v[filterName].search(filterValue); });
                break;
            }
            case ('equals'): {
                this.StaticDataArray = this.StaticDataArray.filter(function (v) { return v[filterName] === (filterValue); });
                break;
            }
            case ('largerthan'): {
                this.StaticDataArray = this.StaticDataArray.filter(function (v) { return v[filterName] > (filterValue); });
