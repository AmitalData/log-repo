...
var AddEditPickupComponent = /** @class */ (function () {
    function AddEditPickupComponent(entityResourceService) {
    ...
    AddEditPickupComponent.prototype.SetWindowArgs = function (args) {
        var _this = this;
    ...
    AddEditPickupComponent.prototype.ngOnDestroy = function () {
        Tools_2.AppTool.KillEventEmitter(this.SaveCompletedEvent);
        this.SaveCompletedEvent = null;
    };
    AddEditPickupComponent.prototype.BuildTabs = function () {
        this.TabsItemsSource = [];
    ...
    AddEditPickupComponent.prototype.RunComponent = function () {
    ...
    AddEditPickupComponent.prototype.RunComponentTimer = function () {
        var _this = this;
        this.Retries++;
        if (this.timerToken) {
    ...
    AddEditPickupComponent.prototype.InitializeComponent = function () {
        if (this.isViewInited) {
            this.SelectionChanged();
        }
    };
    Object.defineProperty(AddEditPickupComponent.prototype, "SelectedTabCode", {
    ...
    AddEditPickupComponent.prototype.SelectionChanged = function () {
        var _this = this;
                ...
                switch (this.SelectedTabCode) {
                    case "MAIN": {
                        if (this.PageChild_MAIN == null) {
                            SessionLocator_1.SessionLocator.DynamicLoader.Load('./ShipmentModules/ShipmentRouting/Components/Routings/PickupTabs/PickupMainTabComponent', myLocation_1.viewContainerRef)
                                .then(function (cmpRef) {
                                _this.PageChild_MAIN = cmpRef.instance;
                                _this.PageChild_MAIN.InitTab(_this.EntityPM, _this.ShipmentPM);
                                if (_this.IsNewEntity) {
                                    var toPortId = _this.ShipmentPM.MainCarriageFromPortId;
                                    if (!Tools_2.AppTool.IsNullOrEmpty(_this.ShipmentPM.PreCarriageFromPortId)) {
                                        toPortId = _this.ShipmentPM.PreCarriageFromPortId;
                                    }
                                    _this.PageChild_MAIN.FromPartnerCardId = _this.ShipmentPM.ShipperId;
                                    _this.PageChild_MAIN.ToPortId = toPortId;
                                }
                                else {
                    ...
                    case "PACG": {
                        if (this.PageChild_PACG == null) {
                            this.entityResourceService.getEntityResourceByTableName("ShipmentPickUpDeliveryPackage").subscribe(function (response) {
                                SessionLocator_1.SessionLocator.DynamicLoader.Load('./ShipmentModules/ShipmentRouting/Components/Routings/PickupTabs/PickupPackagesTabComponent', myLocation_1.viewContainerRef)
                                    .then(function (cmpRef) {
                                    _this.PageChild_PACG = cmpRef.instance;
                                    _this.PageChild_PACG.InitTab(_this.EntityPM, _this.ShipmentPM);
                                });
                            });
                        }
                        break;
                    ...
                    case "DCSO": {
                        if (this.PageChild_DCSO == null) {
                            SessionLocator_1.SessionLocator.DynamicLoader.Load('./ShipmentModules/ShipmentRouting/Components/Routings/PickupTabs/PickupDocsOutTabComponent', myLocation_1.viewContainerRef)
                                .then(function (cmpRef) {
                    ...
                    case "DCSI": {
                        if (this.PageChild_DCSI == null) {
                            SessionLocator_1.SessionLocator.DynamicLoader.Load('./ShipmentModules/ShipmentRouting/Components/Routings/PickupTabs/PickupDocsInTabComponent', myLocation_1.viewContainerRef)
                                .then(function (cmpRef) {
    ...
    AddEditPickupComponent.prototype.CloseClicked = function () {
        var _this = this;
        if (this.EntityPM.IsDirty) {
            ...
            confirmWindow.Show(TextCodeTranslator_1.TextCodeTranslator.Translate("General.M.ThisEntityhasunsavedchanges").replace("%Entity", "Pickup"));
    ...
    AddEditPickupComponent.prototype.CloseWindow = function () {
        this.RejectChanges();
        this.CurrentSession.CloseCurrentWindow();
    };
    AddEditPickupComponent.prototype.OkButtonClicked = function () {
        this.Save(false);
    };
    AddEditPickupComponent.prototype.Save = function (isClosingWindow) {
        var _this = this;
        var errors = [];
        ...
        var isPreCarriageExists = (this.ShipmentPM.PreCarriageFromPortId != null && this.ShipmentPM.PreCarriageToPortId != null) ? true : false;
        var PreCarriageETD = isPreCarriageExists ? Tools_2.DateTool.GetDateParts(this.ShipmentPM.PreCarriageETD).DateTicks : 0;
        var PreCarriageETA = isPreCarriageExists ? Tools_2.DateTool.GetDateParts(this.ShipmentPM.PreCarriageETA).DateTicks : 0;
        var PreCarriageATD = isPreCarriageExists ? Tools_2.DateTool.GetDateParts(this.ShipmentPM.PreCarriageATD).DateTicks : 0;
        var PreCarriageATA = isPreCarriageExists ? Tools_2.DateTool.GetDateParts(this.ShipmentPM.PreCarriageATA).DateTicks : 0;
        ...
        if (errors.length == 0) {
            ...
            if (myShipmentErrors.length > 0) {
                errors.push(TextCodeTranslator_1.TextCodeTranslator.Translate("Shipment.M.Routings.CantProceedAddingPickUp"));
            }
        }
            ...
            if (this.CurrentSession.CurrentEditComponent != null) {
                if (!this.SaveCompletedEvent) {
                    this.SaveCompletedEvent = this.CurrentSession.CurrentEditComponent.SaveCompleted.subscribe(function (isSaveSuccess) {
                        if (isSaveSuccess) {
                            if (_this.IsNewEntity) {
                                ServiceLocator_1.ServiceLocator.SendTotangoUserActivity("Shipment", "PickUpOpen");
                                _this.IsNewEntity = false;
                                _this.TabsItemsSource.forEach(function (item) {
                            ...
                            if (isClosingWindow) {
                                ...
                                if (SavedEntityId) {
                                    _this.EntityPM = _this.ShipmentPM.ShipmentPickUps.filter(function (f) { return f.Id == SavedEntityId; })[0];
                                }
                                else if (SavedEntityNumber) {
                                    _this.EntityPM = _this.ShipmentPM.ShipmentPickUps.filter(function (f) { return f.PickUpDeliveryNumber == SavedEntityNumber; })[0];
                                }
                                if (_this.PageChild_MAIN) {
                                    _this.PageChild_MAIN.InitTab(_this.EntityPM, _this.ShipmentPM);
                                }
                                if (_this.PageChild_PACG) {
                                    _this.PageChild_PACG.InitTab(_this.EntityPM, _this.ShipmentPM);
                                }
                                if (_this.PageChild_DCSO) {
                                    _this.PageChild_DCSO.InitTab(_this.EntityPM, _this.ShipmentPM);
                                }
                                if (_this.PageChild_DCSI) {
                                    _this.PageChild_DCSI.InitTab(_this.EntityPM, _this.ShipmentPM);
                                }
                                _this.Clone();
                            }
    ...
    AddEditPickupComponent.prototype.NewWarehouseEntryButtonClicked = function () {
        var _this = this;
                    ...
                    if (!this.SaveCompletedEvent) {
                        this.SaveCompletedEvent = this.CurrentSession.CurrentEditComponent.SaveCompleted.subscribe(function (isSaveSuccess) {
                            if (isSaveSuccess) {
                                _this.ShipmentPM = _this.CurrentSession.CurrentEditComponent.EntityPM;
                                _this.Clone();
                                _this.InitializeWareHousEntryWindow();
                            }
                            else {
                                _this.ValidationErrorsList = _this.CurrentSession.CurrentEditComponent.ValidationErrorsList;
                            }
    ...
    AddEditPickupComponent.prototype.InitializeWareHousEntryWindow = function () {
        var _this = this;
        if (this.EntityPM.PickUpDeliveryToTypeCode == "PART" && !Tools_2.AppTool.IsNullOrEmpty(this.EntityPM.ToPartnerCardId)) {
            this.myCardListService.getSingle(this.EntityPM.ToPartnerCardId).subscribe(function (myResponse) {
                if (!myResponse.HasError) {
                    var myCardList = myResponse.Result;
                    if (myCardList && myCardList.PartnerTypeId == "WH") {
                        _this.OpenWareHouseEntryWindow(myCardList.Id);
                    }
                    else
                        _this.OpenWareHouseEntryWindow("");
                }
                else
                    _this.OpenWareHouseEntryWindow("");
            });
    ...
    AddEditPickupComponent.prototype.OpenWareHouseEntryWindow = function (warehouseId) {
        var windowArgs = {};
    ...
    AddEditPickupComponent.prototype.Clone = function () {
        var _this = this;
        this.ClonePackages();
        this.oldFollowups = [];
    ...
    AddEditPickupComponent.prototype.RejectChanges = function () {
        var _this = this;
        if (this.EntityPM.IsDirty) {
            this.RejectPackages();
            var addedItems = [];
    ...
    AddEditPickupComponent.prototype.ClonePackages = function () {
        var _this = this;
        var PackageFields = [];
    ...
    AddEditPickupComponent.prototype.RejectPackages = function () {
        var _this = this;
    ...
    ], AddEditPickupComponent.prototype, "AllLocations", void 0);
    AddEditPickupComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
    ...
    return AddEditPickupComponent;
}());
exports.AddEditPickupComponent = AddEditPickupComponent;
...
//# sourceMappingURL=AddEditPickupComponent.js.map