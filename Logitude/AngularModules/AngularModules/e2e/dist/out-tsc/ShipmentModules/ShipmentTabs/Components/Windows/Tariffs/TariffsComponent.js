...
var PayablesTabComponent_1 = require("../../Payables/PayablesTabComponent");
    ...
    TariffsComponent.prototype.OkButtonClicked = function () {
        ...
        if (itemChecked != null) {
            ...
            myCharges.forEach(function (item) {
                var payableItem = _this.fatherComponent.ItemsSource.Collection.filter(function (d) { return d.ChargesTypeId == item.ChargesTypeId && d.MeasurementId == item.MeasurementId && d.CurrencyId == item.CurrencyId; })[0];
                if (payableItem != null) {
                    ...
                    var payablePM = new ShipmentPayablePM_1.ShipmentPayablePM(_this.EntityPM);
                    payablePM.Tenant = _this.EntityPM.Tenant;
                    payablePM.ShipmentId = _this.EntityPM.Id;
                    payablePM.ShipmentNumber = _this.EntityPM.ShipmentNumber;
                    payablePM.ShipmentPayableLineStatusCode = "EMPT";
                    payablePM.ShipmentPayableAmountTypeCode = "ACCU";
                    payablePM.CreateDate = Tools_1.DateTool.GetCurrentDateAsUtc();
                    payablePM.CreatedByUserId = SessionLocator_1.SessionLocator.LoggedUserId;
                    payablePM.UpdateDate = Tools_1.DateTool.GetCurrentDateAsUtc();
                    payablePM.UpdateByUserId = SessionLocator_1.SessionLocator.LoggedUserId;
                    payablePM.ProfitCurrencyExchangeRate = _this.fatherComponent.GetCurrencyRate(_this.EntityPM.ProfitCurrencyId);
                    _this.EntityPM.AddPayable(payablePM);
                    payableItem = new PayablesTabComponent_1.ShipmentPayableItem(payablePM, _this.fatherComponent, false);
                    _this.fatherComponent.ItemsSource.Insert(payableItem);
