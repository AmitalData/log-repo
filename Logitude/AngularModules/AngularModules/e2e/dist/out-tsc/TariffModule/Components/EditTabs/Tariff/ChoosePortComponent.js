...
var UpdateSurchargesComponent_1 = require("./UpdateSurchargesComponent");
...
var ChoosePortComponent = /** @class */ (function (_super) {
    ...
    ChoosePortComponent.prototype.AddButtonClicked = function () {
        var _this = this;
        var errors = [];
        if (this.Port == null || Tools_1.AppTool.IsNullOrEmpty(this.PortId)) {
            errors.push("Please Choose port");
        }
        else {
            if (this.UpdateClass.Type == "From") {
                if (this.UpdateClass.fatherComponent.FromObsList.filter(function (d) { return d.Code == _this.Port.Code; }).length > 0) {
                    errors.push("Port with the same code already added");
                }
            }
            else {
                if (this.UpdateClass.fatherComponent.ToObsList.filter(function (d) { return d.Code == _this.Port.Code; }).length > 0) {
                    errors.push("Port with the same code already added");
                }
            }
        }
        ...
        if (errors.length == 0) {
            var newItem = new UpdateSurchargesComponent_1.DestinationClass(this.UpdateClass.fatherComponent, this.UpdateClass.Type, this.Port);
            if (this.UpdateClass.Type == "From") {
