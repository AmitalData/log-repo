...
var ExternalReconciliationPMService = /** @class */ (function () {
    function ExternalReconciliationPMService() {
        this._http = ServiceHelper_1.ServiceHelper.Http;
        this._apiUrl = ServiceHelper_1.ServiceHelper.GetLogitudeURL() + 'api/externalreconciliations';
    }
    ExternalReconciliationPMService.prototype.get = function (id) {
        var _this = this;
        ...
        return Rx_1.Observable.defer(function () {
            return _this._http.get(_this._apiUrl + '/getsingle?' + 'id=' + id, {
                ...
                PerformanceLogger_1.PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "ExternalReconciliation", "GetSinglePM", 'id=' + id);
                return serviceResponse;
            }).catch(ServiceHelper_1.ServiceHelper.HandleServiceError);
        });
    };
    ExternalReconciliationPMService.prototype.insert = function (entityPM) {
        var _this = this;
        var callTime = new Date();
        return Rx_1.Observable.defer(function () {
            var authHeader = new http_1.Headers();
            ...
            if (errorsArray.length == 0) {
                ...
                return _this._http.post(_this._apiUrl, JSON.stringify(mappedEntity), { headers: authHeader }).map(function (response) {
                    ...
                    var servertime = response.headers.get('ServerExecutionTime');
                    PerformanceLogger_1.PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "ExternalReconciliation", "SaveChanges", "");
                    return serviceResponse;
                }).catch(ServiceHelper_1.ServiceHelper.HandleServiceError);
            }
            else {
    ...
    ExternalReconciliationPMService.prototype.update = function (entityPM) {
        var _this = this;
        var callTime = new Date();
        return Rx_1.Observable.defer(function () {
            var authHeader = new http_1.Headers();
    ...
    ExternalReconciliationPMService.prototype.MapJsonToEntityPM = function (jsonPM, mapParent, entityPM) {
        if (mapParent === void 0) { mapParent = true; }
        if (entityPM === void 0) { entityPM = null; }
        if (!entityPM) {
            entityPM = new ExternalReconciliationPM_1.ExternalReconciliationPM();
        }
        var customFields = [];
        for (var i = 1; i < 11; i++) {
        ...
        if (mapParent) {
            ...
            for (var item in entityPM.ExternalReconciliationLines) {
                ...
                var newExternalReconciliationLinePM = this.clone(myExternalReconciliationLinePM);
    ...
    ExternalReconciliationPMService.prototype.MapExternalReconciliationLines = function (entityPM, jsonPM, mapParent) {
        if (mapParent === void 0) { mapParent = true; }
        var oldExternalReconciliationLines = [];
        if (entityPM.OldEntityPM && !mapParent) {
            oldExternalReconciliationLines = entityPM.OldEntityPM.ExternalReconciliationLines;
        }
        entityPM.ExternalReconciliationLines = new Array();
        for (var item in jsonPM.ExternalReconciliationLines) {
            var jItem = jsonPM.ExternalReconciliationLines[item];
            if (mapParent && (jItem.ChangeSetOp == "Delete" || jItem.ChangeSetOp == 3)) {
                continue;
            }
            var newExternalReconciliationLinePM;
            if (mapParent) {
                newExternalReconciliationLinePM = new ExternalReconciliationLinePM_1.ExternalReconciliationLinePM(entityPM);
            }
            else {
                newExternalReconciliationLinePM = new ExternalReconciliationLinePM_1.ExternalReconciliationLinePM(null);
            }
            ...
            if (mapParent) {
                newExternalReconciliationLinePM.UniqueKey = Guid_1.Guid.newGuid();
        ...
        if (oldExternalReconciliationLines) {
            for (var itemKey in oldExternalReconciliationLines) {
                if (entityPM.ExternalReconciliationLines.filter(function (p) { return p.UniqueKey === oldExternalReconciliationLines[itemKey].UniqueKey; }).length === 0) {
                    if (oldExternalReconciliationLines[itemKey]) {
                        ...
                        var deletedPM = new ExternalReconciliationLinePM_1.ExternalReconciliationLinePM(null);
    ...
    ExternalReconciliationPMService.prototype.clone = function (jsonPM) {
        var entityPM;
        entityPM = {};
        var jsonPMKeys = Object.keys(jsonPM);
        for (var key in jsonPMKeys) {
    ...
    ExternalReconciliationPMService.prototype.GetNewEntityPM = function () {
        var entityPM;
        entityPM = new ExternalReconciliationPM_1.ExternalReconciliationPM();
        entityPM.Tenant = InfraSettings_1.InfraSettings.TenantPM.Id;
        return entityPM;
    };
    ExternalReconciliationPMService = __decorate([
        core_1.Injectable(),
        __metadata("design:paramtypes", [])
    ], ExternalReconciliationPMService);
    return ExternalReconciliationPMService;
}());
exports.ExternalReconciliationPMService = ExternalReconciliationPMService;
//# sourceMappingURL=ExternalReconciliationPMService.js.map