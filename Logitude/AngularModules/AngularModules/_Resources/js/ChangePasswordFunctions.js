...
function PassWordValueTriming(passtring) {
...
function ClientSideStrongPassword(password, arguments1, arguments2) {
    return (IsLongEnough(password, "8") && SpansEnoughCharacterSets(password, "3") && (!(IsCloseVariationOfAWordInDictionary(password, "0.6", arguments1, arguments2))));
}

function ClientSideBestPassword(password, arguments1, arguments2) {
    return (IsLongEnough(password, "14") && SpansEnoughCharacterSets(password, "3") && (!(IsCloseVariationOfAWordInDictionary(password, "0.6", arguments1, arguments2))));
}

function ClientSideMediumPassword(password, arguments1, arguments2) {
            return (IsLongEnough(password, "8") && SpansEnoughCharacterSets(password, "2") && (!(FoundInDictionary(password, arguments1, arguments2))));
        }

function ClientSideWeakPassword(password, arguments1, arguments2) {
                return (IsLongEnough(password, "1") || (!(IsLongEnough(password, "0"))));
            }
...
function DispPwdStrength(iN, sHL, PdwcheckmsgKey, sMKey) {
                    ...
                    for (var i = 0; i < 5; i++) {
                        ...
                        if (i > 0) {
                            GEId(sMKey + i).className = sHCR;
                        } else { }
                        GEId(PdwcheckmsgKey + i).style.display = ((i == iN) ? "inline" : "none");
                    }
}
