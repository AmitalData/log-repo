...
export class LogBoxMainComponent implements OnInit, AfterViewInit {
    ...
    constructor(private _entityListService: EntityListService) {
        ...
        let FeatureToggle = SessionLocator.FeatureToggles.filter(d => d.ToggleCode == "LEX" && d.TenantNumber == SessionLocator.Tenant)[0];
        if (FeatureToggle) {
    ...
    LoadQueriesCounts() {
        const shipmentsQueriesCountsArgs: ShipmentsQueriesCountsArgs  = {
            ...
            ForwarderPartnerId: this.isPrivateLabel && !this.IsDSV ? SessionLocator.PrivateLableSettings.HybridPartnerId : null,
        };
