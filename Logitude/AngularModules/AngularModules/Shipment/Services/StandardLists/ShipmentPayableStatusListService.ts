...
export class ShipmentPayableStatusListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<ShipmentPayableStatusList> = [];
    constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/shipmentpayablestatusviews';  
    }
			...
			return this._http.get(this._apiUrl + '/getsingle/?' + 'code=' + code, ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						var entity: ShipmentPayableStatusList;
						if (list) {
					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {			
								var entity: ShipmentPayableStatusList = this.MapJsonToEntityList(allLists[key]);
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: ShipmentPayableStatusList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
	...
	getSingleFromCache(code: string) {
		...
		if (ShipmentPayableStatusListService.CachedData.length > 0) {
			return defer(() => {
				...
				map((cachedJson:any) => {
					...
					if (cachedJson) {
						for (var key in cachedJson) {
							var entity: ShipmentPayableStatusList = this.MapJsonToEntityList(cachedJson[key]);
							_mappedListsArray.push(entity);
						}
					}

					ShipmentPayableStatusListService.CachedData = _mappedListsArray;
    ...
    getAllFromCache(filters: ApiQueryFilters= new ApiQueryFilters(true)) {
        ...
        if (ShipmentPayableStatusListService.CachedData.length > 0) {
            return defer(() => {
                if (filters.GetAll) {
					serviceResponse.Result = ShipmentPayableStatusListService.CachedData; 
				}
	    ...
	    MapJsonToEntityList(jsonList: any) {
            ...
            var entityList: ShipmentPayableStatusList;
            entityList = new ShipmentPayableStatusList();
