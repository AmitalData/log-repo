...
export class MasterActionConfirmationComponent extends BaseComponent {
    ...
    private connectedShipments: Array<ShipmentPM>;
    private LoadConnectedHouses() {
        this.CurrentSession.StartBusyIndicator("Loading...");

        this.shipmentService.GetConnectedShipmentsByMasterIdAndTenant(this.FatherComponent.EntityPM.Id, this.FatherComponent.EntityPM.Tenant).subscribe((response: any) => {
            if (!response.HasError && response.Result) {
                this.connectedShipments = response.Result;
                this.LoadConnectedHousesPackages();                
            }
        });
    }
    private LoadConnectedHousesPackages() {
        this.shipmentService.GetShipmentConsolidationPackages(this.FatherComponent.EntityPM.Id).subscribe((result: ServiceResponse) => {
            this.AddMasterToMastersList(result.Result);
            this.AddHousesToMastersList();
            this.SetGeneralProperties();
        });
    }
    private AddMasterToMastersList(houseShipmentsPackaes: Array<ShipmentPackagePM>) {
        this.FatherComponent.EntityPM.IsOperationalClosed = true;
        this.MasterViewModel = new MasterActionConfirmationViewModel(this.FatherComponent.EntityPM, houseShipmentsPackaes);
    ...
    private AddHousesToMastersList() {
        this.connectedShipments.forEach(shipment => {
