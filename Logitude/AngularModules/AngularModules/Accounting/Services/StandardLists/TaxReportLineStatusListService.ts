...
import {TaxReportLineStatusList} from '../../EntityLists/TaxReportLineStatusList';
...
export class TaxReportLineStatusListService {
					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {			
								var entity: TaxReportLineStatusList = this.MapJsonToEntityList(allLists[key]);
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: TaxReportLineStatusList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
	...
	getSingleFromCache(code: string) {
		...
		if (TaxReportLineStatusListService.CachedData.length > 0) {
			return defer(() => {
				...
				map((cachedJson:any) => {
					...
					if (cachedJson) {
						for (var key in cachedJson) {
							var entity: TaxReportLineStatusList = this.MapJsonToEntityList(cachedJson[key]);
							_mappedListsArray.push(entity);
						}
					}
