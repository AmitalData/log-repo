...
export class FCLChargesComponent extends BaseComponent implements OnDestroy {
    ...
    constructor(private entityArgs: EntityArgs) {
        ...
        this.AllInMatchText = TextCodeTranslator.Translate("QuoteOP.M.UnableToDoAllIn") + "\n" + TextCodeTranslator.Translate("QuoteOP.M.IfMatchesFrieghtCharge");
        this.HideFCLAllIn = SessionLocator.TenantPM.HideFCLAllIn;

       if( FeatureLocator.HasFeaturePermession("QuoteOP", "TOTALPERCONTAINER")) {
            this.IsShowTotalPerContainer = true;
        }

        var featureToggle = SessionLocator.FeatureToggles.filter(d => d.ToggleCode == "TAR")[0];
        if (FeatureLocator.HasFeaturePermession("QuoteOP", "QuotePriceCheck") && featureToggle != null) {
    ...
    SetLabels() {
        this.CostQuentityHeader = TextCodeTranslator.Translate("QuoteOP.O.Charges.CostQuantity").split('%n');
        this.CostPriceHeader = TextCodeTranslator.Translate("QuoteOP.O.Charges.CostPrice").split('%n');
        this.CostAmountHeader = TextCodeTranslator.Translate("QuoteOP.O.Charges.CostAmount").split('%n');
        this.SaleQuantityHeader = TextCodeTranslator.Translate("QuoteOP.O.Charges.SaleQuantity").split('%n');
        this.SaleLocalAmountHeader = TextCodeTranslator.Translate("QuoteOP.O.Charges.SaleAmountLocal").replace("%LocalCurrencyCode", this.LocalCurrencyCode).split('%n');
    ...
    SetLabelsAttached() {
        var myCostString = TextCodeTranslator.Translate("QuoteOP.O.Charges.Cost");
        ...
        if (this.IsSaleCurrencySameAsCost || this.IsMultiCurrency) {
            this.SalePriceHeader = TextCodeTranslator.Translate("QuoteOP.O.Charges.SalePrice").replace("%SaleCurrencyCode", "").replace("(", "").replace(")", "").split('%n');
            this.SaleAmountHeader = TextCodeTranslator.Translate("QuoteOP.O.Charges.SaleAmount").replace("%SaleCurrencyCode", "").replace("(", "").replace(")", "").split('%n');
        }

        else {
            this.SalePriceHeader = TextCodeTranslator.Translate("QuoteOP.O.Charges.SalePrice").replace("%SaleCurrencyCode", mySaleCurrencyCode).split('%n');
            this.SaleAmountHeader = TextCodeTranslator.Translate("QuoteOP.O.Charges.SaleAmount").replace("%SaleCurrencyCode", mySaleCurrencyCode).split('%n');
        }
        ...
        if (!AppTool.IsNullOrEmpty(this.EntityPM.PackageType1Id)) {
            ...
            if (item) {
                ...
                this.Sale1HeaderTooltip = TextCodeTranslator.Translate("QuoteOP.O.Charges.SalePrice").replace("%SaleCurrencyCode", "").replace("(", "").replace(")", "").replace("%n", " ").replace("\n"," ") + " " + value1;
            }
        }
        ...
        if (!AppTool.IsNullOrEmpty(this.EntityPM.PackageType2Id)) {
            ...
            if (item) {
                ...
                this.Sale2HeaderTooltip = TextCodeTranslator.Translate("QuoteOP.O.Charges.SalePrice").replace("%SaleCurrencyCode", "").replace("(", "").replace(")", "").replace("%n", " ").replace("\n", " ") + " " + value2;
            }
        }
        ...
        if (!AppTool.IsNullOrEmpty(this.EntityPM.PackageType3Id)) {
            ...
            if (item) {
                ...
                this.Sale3HeaderTooltip = TextCodeTranslator.Translate("QuoteOP.O.Charges.SalePrice").replace("%SaleCurrencyCode", "").replace("(", "").replace(")", "").replace("%n", " ").replace("\n", " ") + " " + value3;
            }
        }
        ...
        if (!AppTool.IsNullOrEmpty(this.EntityPM.PackageType4Id)) {
            ...
            if (item) {
                ...
                this.Sale4HeaderTooltip = TextCodeTranslator.Translate("QuoteOP.O.Charges.SalePrice").replace("%SaleCurrencyCode", "").replace("(", "").replace(")", "").replace("%n", " ").replace("\n", " ") + " " + value4;
            }
        }
        ...
        if (!AppTool.IsNullOrEmpty(this.EntityPM.PackageType5Id)) {
            ...
            if (item) {
                ...
                this.Sale5HeaderTooltip = TextCodeTranslator.Translate("QuoteOP.O.Charges.SalePrice").replace("%SaleCurrencyCode", "").replace("(", "").replace(")", "").replace("%n", " ").replace("\n", " ") + " " + value5;
            }
        }
    ...
    CheckUpdateQuantities() {
        if (this.IsAdhoc) {
            ...
            if (isDifferentOrders && AppTool.IsNullOrEmpty(updateMessage)) {
