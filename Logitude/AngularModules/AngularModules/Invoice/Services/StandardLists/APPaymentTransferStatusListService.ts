...
export class APPaymentTransferStatusListService {
					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {			
								var entity: APPaymentTransferStatusList = this.MapJsonToEntityList(allLists[key]);
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: APPaymentTransferStatusList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
	...
	getSingleFromCache(code: string) {
		...
		if (APPaymentTransferStatusListService.CachedData.length > 0) {
            ...
            return CachedDataManager.GetClosedTableData("APPaymentTransferStatus").pipe(
				map(cachedJson => {
					...
					if (cachedJson) {
						for (var key in cachedJson) {
							var entity: APPaymentTransferStatusList = this.MapJsonToEntityList(cachedJson[key]);
							_mappedListsArray.push(entity);
						}
					}
