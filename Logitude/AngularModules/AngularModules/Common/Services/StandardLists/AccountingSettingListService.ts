...
export class AccountingSettingListService {
    ...
    getSingleFromCache(id: number) {
        ...
        var authHeader = new Headers();
        authHeader.append('Token', SessionInfo.Token);
		var exists = AccountingSettingListService.CachedData.filter(a => a.Id === id).length;

        var serviceResponse: ServiceResponse;
        serviceResponse = new ServiceResponse(); 
        if (exists === 0) {
        return Observable.defer(() => {
            var cacheKey = "AccountingSetting_CachedData_" + SessionLocator.Tenant;
            var _mappedListsArray: Array<AccountingSettingList> = [];
                ...
                if (cachedString) {
                    ...
                    AccountingSettingListService.CachedData = _mappedListsArray;
                    serviceResponse = new ServiceResponse();
                    
                    var filteredData = AccountingSettingListService.CachedData.filter(a => a.Id === id)[0];
                    serviceResponse.Result = filteredData;
					serviceResponse.CallTime = callTime;
 
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "AccountingSetting", "GetSingleListFromCache", 'id=' + id); 
  ...
  var exists = AccountingSettingListService.CachedData.length;
   var urlparameters = '/getbyfilters?';
        var mykeys = Object.keys(filters);
        var addtionalFiltersValues = null;
        for (var i in mykeys) {
        ...
        if (exists === 0 || filters.ForceCacheRefresh) {
            ...
            if (!filters.ForceCacheRefresh) {
                ...
                if (cachedString) {
                    ...
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "AccountingSetting", "GetAllFromCache", ""); 
            ...
            if (serviceResponse) {
                ...
                return Observable.defer(() => {
                    ...
                    var servertime = response.headers.get('ServerExecutionTime');
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "AccountingSetting", "GetAll", ""); 
            ...
            var filteredData = AccountingSettingListService.CachedData;
            if (!filters.GetAll) {
