					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {				
								var entity: VatTypeList = this.MapJsonToEntityList(allLists[key]);
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: VatTypeList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
			...
			return defer(() => {
                ...
                if (cachedString) {
                    ...
                    for (var key in cachedJson) {
                        var entity: VatTypeList = this.MapJsonToEntityList(cachedJson[key]);
                        _mappedListsArray.push(entity);
                    }
