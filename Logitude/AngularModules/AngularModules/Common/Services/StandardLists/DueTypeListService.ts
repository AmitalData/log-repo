...
export class DueTypeListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<DueTypeList> = [];
    constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/duetypeviews';  
    }
			...
			return this._http.get(this._apiUrl + '/getall', ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						var _mappedListsArray: Array<DueTypeList> = [];
						if (allLists) {
							for (var key in allLists) {			
								var entity: DueTypeList = this.MapJsonToEntityList(allLists[key]);
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: DueTypeList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
	...
	getSingleFromCache(code: string) {
		...
		if (DueTypeListService.CachedData.length > 0) {
			return defer(() => {
            ...
            return CachedDataManager.GetClosedTableData("DueType").pipe(
				map((cachedJson:any) => {
					...
					if (cachedJson) {
						for (var key in cachedJson) {
							var entity: DueTypeList = this.MapJsonToEntityList(cachedJson[key]);
							_mappedListsArray.push(entity);
						}
					}

					DueTypeListService.CachedData = _mappedListsArray;
    ...
    getAllFromCache(filters: ApiQueryFilters= new ApiQueryFilters(true)) {
        ...
        if (DueTypeListService.CachedData.length > 0) {
            return defer(() => {
                if (filters.GetAll) {
					serviceResponse.Result = DueTypeListService.CachedData; 
				}
	    ...
	    MapJsonToEntityList(jsonList: any) {
            ...
            var entityList: DueTypeList;
            entityList = new DueTypeList();
