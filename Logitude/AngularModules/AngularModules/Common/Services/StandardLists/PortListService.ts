...
import {PortCustomFilter} from '../../CustomFilters/PortCustomFilter';

@Injectable()

export class PortListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<PortList> = [];
    constructor() {
			...
			return this._http.get(this._apiUrl + '/getsingle/?' + 'id=' + id, ServiceHelper.GetHttpFullHeaders())
				.pipe(			
					map((response: HttpResponse<any>) => {
						...
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "Port", "GetSingleList", 'id=' + id); 
					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {				
								var entity: PortList = this.MapJsonToEntityList(allLists[key]);
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "Port", "GetAllLists", ""); 
			...
			return this._http.get(callUrl, ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "Port", "GetByFilters", "PageIndex:" +filters.PageIndex +", PageSize:"+filters.PageSize + ", GetAll:" + filters.GetAll);
	...
	getSingleFromCache(id: string) {
		...
		var exists = PortListService.CachedData.filter(a => a.Id === id).length;

        var serviceResponse: ServiceResponse = new ServiceResponse(); 

        if (exists === 0) {
			return defer(() => {
				var cacheKey = "Port_CachedData_" + SessionLocator.Tenant;
				var _mappedListsArray: Array<PortList> = [];
                ...
                if (cachedString) {
                    ...
                    for (var key in cachedJson) {
                        var entity: PortList = this.MapJsonToEntityList(cachedJson[key]);
                        _mappedListsArray.push(entity);
                    }

                    PortListService.CachedData = _mappedListsArray;
                    serviceResponse = new ServiceResponse();
                    
                    var filteredData = PortListService.CachedData.filter(a => a.Id === id)[0];
                    serviceResponse.Result = filteredData;
					serviceResponse.CallTime = callTime;
 
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "Port", "GetSingleListFromCache", 'id=' + id); 
    ...
    getAllFromCache(filters: ApiQueryFilters = new ApiQueryFilters(true)) {
		...
		var exists = PortListService.CachedData.length;
		var urlparameters = '/getbyfilters?';
        var mykeys = Object.keys(filters);
        var addtionalFiltersValues = null;
        ...
        if (exists === 0 || filters.ForceCacheRefresh) {
            var cacheKey = "Port_CachedData_" + filters.Tenant;
            var _mappedListsArray: Array<PortList> = [];
            ...
            if (!filters.ForceCacheRefresh) {
                ...
                if (cachedString) {
                    ...
                    serviceResponse = new ServiceResponse();
                     ...
                     if (!filters.GetAll) {
					  if (addtionalFiltersValues) {
                            _mappedListsArray = PortCustomFilter.GetFilteredQuery(addtionalFiltersValues, _mappedListsArray);
                        }
                    ...
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "Port", "GetAllFromCache", "");                     
                }
            }
                    ...
                    return this._http.get(callUrl, ServiceHelper.GetHttpFullHeaders())
						.pipe(
                            ...
                            if (addtionalFiltersValues) {
								...
								var servertime = response.headers.get('ServerExecutionTime');
								PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "Port", "GetAll", ""); 
            ...
            var filteredData = PortListService.CachedData;
            if (!filters.GetAll) {
                if (addtionalFiltersValues) {
                    filteredData = PortCustomFilter.GetFilteredQuery(addtionalFiltersValues, filteredData);
                }
