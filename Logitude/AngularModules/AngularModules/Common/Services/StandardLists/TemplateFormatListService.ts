...
import {TemplateFormatList} from '../../EntityLists/TemplateFormatList';

@Injectable()

export class TemplateFormatListService {
    private _http: HttpClient;
    private _apiUrl: string;
    public static CachedData: Array<TemplateFormatList> = [];
    constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/TemplateFormatviews';
    }
            ...
            return this._http.get(this._apiUrl + '/getsingle/?' + 'code=' + code, ServiceHelper.GetHttpFullHeaders())
                .pipe(
                    map((response: HttpResponse<any>) => {
                        ...
                        PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "TemplateFormat", "GetSingleList", 'code=' + code);
            ...
            return this._http.get(this._apiUrl + '/getall', ServiceHelper.GetHttpFullHeaders())
                .pipe(
                    map((response: HttpResponse<any>) => {
                        ...
                        var servertime = response.headers.get('ServerExecutionTime');
                        PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "TemplateFormat", "GetAll", "");
            ...
            return this._http.get(callUrl, ServiceHelper.GetHttpFullHeaders())
                .pipe(
                    map((response: HttpResponse<any>) => {
                        ...
                        var servertime = response.headers.get('ServerExecutionTime');
                        PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "TemplateFormat", "GetByFilters", "PageIndex:" + filters.PageIndex + ", PageSize:" + filters.PageSize + ", GetAll:" + filters.GetAll);
    ...
    getSingleFromCache(code: string) {
        ...
        if (TemplateFormatListService.CachedData.length > 0) {
            ...
            return CachedDataManager.GetClosedTableData("TemplateFormat").pipe(map((cachedJson:any) => {
                ...
                TemplateFormatListService.CachedData = _mappedListsArray;

                var filteredData = TemplateFormatListService.CachedData.filter(a => a.Code === code)[0];
                serviceResponse.Result = filteredData;


                PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "TemplateFormat", "GetSingleListFromCache", 'code=' + code);
        ...
        if (TemplateFormatListService.CachedData.length > 0) {
            ...
            return CachedDataManager.GetClosedTableData("TemplateFormat").pipe(map((cachedJson:any) => {
                ...
                if (filters.GetAll) {
                    ...
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "TemplateFormat", "GetAllFromCache", "PageIndex:" + filters.PageIndex + ", PageSize:" + filters.PageSize + ", GetAll:" + filters.GetAll);
    ...
    MapJsonToEntityList(jsonList: any) {
        ...
        var entityList: TemplateFormatList;
        entityList = new TemplateFormatList();
