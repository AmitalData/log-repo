...
import {FeaturePackageTypeList} from '../../EntityLists/FeaturePackageTypeList';

@Injectable()

export class FeaturePackageTypeListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<FeaturePackageTypeList> = [];
    constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/featurepackagetypeviews';  
    }
			...
			return this._http.get(this._apiUrl + '/getsingle/?' + 'code=' + code, ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						var entity: FeaturePackageTypeList;
						if (list) {
					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {			
								var entity: FeaturePackageTypeList = this.MapJsonToEntityList(allLists[key]);
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: FeaturePackageTypeList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
	...
	getSingleFromCache(code: string) {
		...
		if (FeaturePackageTypeListService.CachedData.length > 0) {
			return Observable.defer(() => {
				...
				map(cachedJson => {
					...
					if (cachedJson) {
						for (var key in cachedJson) {
							var entity: FeaturePackageTypeList = this.MapJsonToEntityList(cachedJson[key]);
							_mappedListsArray.push(entity);
						}
					}

					FeaturePackageTypeListService.CachedData = _mappedListsArray;
    ...
    getAllFromCache(filters: ApiQueryFilters= new ApiQueryFilters(true)) {
        ...
        if (FeaturePackageTypeListService.CachedData.length > 0) {
            return Observable.defer(() => {
                if (filters.GetAll) {
					serviceResponse.Result = FeaturePackageTypeListService.CachedData; 
				}
	    ...
	    MapJsonToEntityList(jsonList: any) {
            ...
            var entityList: FeaturePackageTypeList;
            entityList = new FeaturePackageTypeList();
