...
import {UserCustomFilter} from '../../CustomFilters/UserCustomFilter';
...
export class UserListService {
    ...
    public static CachedData: Array<UserList> = [];
    constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/userviews';
    }
            ...
            return this._http.get(this._apiUrl + '/getsingle/?' + 'id=' + id, ServiceHelper.GetHttpFullHeaders())
                .pipe(
                    map((response: HttpResponse<any>) => {
                        ...
                        PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "User", "GetSingleList", 'id=' + id);
            ...
            return this._http.get(this._apiUrl + '/getall', ServiceHelper.GetHttpFullHeaders())
                .pipe(
                    map((response: HttpResponse<any>) => {
                        ...
                        serviceResponse.CallTime = callTime;
                        var servertime = response.headers.get('ServerExecutionTime');
                        PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "User", "GetAllLists", "");
    ...
    getSingleFromCache(id: string) {
        ...
        var authHeader = new Headers();
        authHeader.append('Token', SessionInfo.Token);
        var exists = UserListService.CachedData.filter(a => a.Id === id).length;
        ...
        if (exists === 0) {
            return defer(() => {
                ...
                if (cachedString) {
                    ...
                    UserListService.CachedData = _mappedListsArray;
                    serviceResponse = new ServiceResponse();
    ...
    getAllFromCache(filters: ApiQueryFilters = new ApiQueryFilters(true)) {
        ...
        var exists = UserListService.CachedData.length;
        var urlparameters = '/getbyfilters?';
        var mykeys = Object.keys(filters);
        var addtionalFiltersValues = null;
        for (var i in mykeys) {
        ...
        if (exists === 0 || filters.ForceCacheRefresh) {
            ...
            if (!filters.ForceCacheRefresh) {
                ...
                if (cachedString) {
                    ...
                    if (!filters.GetAll) {
                        if (addtionalFiltersValues) {
                            _mappedListsArray = UserCustomFilter.GetFilteredQuery(addtionalFiltersValues, _mappedListsArray);
                        }
                    ...
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "User", "GetAllFromCache", "");
                    ...
                    return this._http.get(callUrl, ServiceHelper.GetHttpFullHeaders())
                        .pipe(
                            map((response: HttpResponse<any>) => {
                                ...
                                var servertime = response.headers.get('ServerExecutionTime');
                                PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "User", "GetAll", "");
            ...
            var filteredData = UserListService.CachedData;
            if (!filters.GetAll) {
                if (addtionalFiltersValues) {
                    filteredData = UserCustomFilter.GetFilteredQuery(addtionalFiltersValues, filteredData);
                }
    ...
    MapJsonToEntityList(jsonList: any) {
        ...
        var entityList: UserList;
        entityList = new UserList();
