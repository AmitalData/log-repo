...
export class UserLastSettingsPMService {
 private _http: HttpClient;
 private _apiUrl: string;
 constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/userlastsettings';      
    }
	...
	insert(entityPM: UserLastSettingsPM) {
		...
		return defer(() => {
			...
			var errorsArray = validator.Validate("UserLastSettings", entityPM);


			if (errorsArray.length == 0) {

				var mappedEntity: UserLastSettingsPM = this.MapJsonToEntityPM(entityPM, false);
				...
				return this._http.post(this._apiUrl, JSON.stringify(mappedEntity), ServiceHelper.GetHttpFullHeaders())
					.pipe(
						map((response: HttpResponse<any>) => {
							...
							var pm = response.body;
							if (pm) {
								var mappedResult: UserLastSettingsPM = this.MapJsonToEntityPM(pm, true, entityPM);
								serviceResponse.Result = mappedResult;
							}						

							var servertime = response.headers.get('ServerExecutionTime');
							PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "UserLastSettings", "SaveChanges", "");                    
	...
	update(entityPM: UserLastSettingsPM) {
