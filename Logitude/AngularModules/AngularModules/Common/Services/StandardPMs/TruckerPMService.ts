...
export class TruckerPMService {
 private _http: HttpClient;
 private _apiUrl: string;
 constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/truckers';      
    }
			...
			return this._http.get(this._apiUrl + '/getsingle?' + 'id=' + id, ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "Trucker", "GetSinglePM", 'id=' + id);
	...
	insert(entityPM: TruckerPM) {
		...
		return defer(() => {
			...
			var errorsArray = validator.Validate("Trucker", entityPM);


			if (errorsArray.length == 0) {

				var mappedEntity: TruckerPM = this.MapJsonToEntityPM(entityPM, false);
				...
				return this._http.post(this._apiUrl, JSON.stringify(mappedEntity), ServiceHelper.GetHttpFullHeaders())
					.pipe(
						map((response: HttpResponse<any>) => {
							...
							var pm = response.body;
							if (pm) {
								var mappedResult: TruckerPM = this.MapJsonToEntityPM(pm, true, entityPM);
								serviceResponse.Result = mappedResult;
							}						

							var servertime = response.headers.get('ServerExecutionTime');
							PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "Trucker", "SaveChanges", "");                    
	...
	update(entityPM: TruckerPM) {
	  ...
	  MapJsonToEntityPM(jsonPM: any, mapParent: boolean = true, entityPM: TruckerPM = null) {

         
        if (!entityPM) {
            
            entityPM = new TruckerPM();
        }
    ...
    MapAddresses(entityPM: TruckerPM, jsonPM: any, mapParent: boolean = true) {
    ...
    MapContacts(entityPM: TruckerPM, jsonPM: any, mapParent: boolean = true) {
    ...
    MapCardExternalCodeByCurrencies(entityPM: TruckerPM, jsonPM: any, mapParent: boolean = true) {
    ...
    MapCardCurrenciesAccountings(entityPM: TruckerPM, jsonPM: any, mapParent: boolean = true) {
