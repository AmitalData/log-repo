...
export class APEditMultiCurrency extends BaseComponent {
    public EntityPM: ARInvoicePaymentPM = null;
    public LocalCurrencyId: string;
    public DataContext = this;
    public ObjectTableName: string = "APInvoicePayment";
    ...
    SetWindowArgs(myParam: APPaymentInvoiceArgs) {
    ...
    ValidateForeignAmount() {
        ...
        if (this.lineComponent.InvoiceAmount < 0) {
            if (inputEntry > 0) {
                isValid = false;
                isValidMessage = TextCodeTranslator.Translate("APPayment.M.OnlyMinusValue");
            }

            else if (inputEntry < allowedAmount) {
                isValid = false;
                isValidMessage = TextCodeTranslator.Translate("APPayment.M.AmountPaidNotLess") + " " + allowedAmount + " " + this.InvoiceCurrencyCode;
            }
        }
            ...
            if (inputEntry < 0) {
                isValid = false;
                isValidMessage = TextCodeTranslator.Translate("APPayment.M.CantPayMinusValue");
            }

            else if (inputEntry > allowedAmount) {
                isValid = false;
                isValidMessage = TextCodeTranslator.Translate("APPayment.M.AmountPaidLessOrEqual") + " " + allowedAmount + " " + this.InvoiceCurrencyCode;
            }
        }
