...
export class RulesValidator {
    ...
    public Initizialize(entity:any = null) {
        ...
        if (window.ObjectTableRules != null && window.ObjectTableRules != undefined) {
            ...
            if (this.IsNewEntity) {
                this._requiredFieldRules = this._tenantRules.filter(r => r.RuleTypeCode == "REQ" && r.InActive == false && r.InActive == false && (r.ActiveForNew == this.IsNewEntity));

                this._entityLevelRules = this._tenantRules.filter(r => r.RuleTypeCode == "EVAL" && r.InActive == false && r.InActive == false && (r.ActiveForNew == this.IsNewEntity));

                this._blockRules = this._tenantRules.filter(r => r.RuleTypeCode == "BLCK" && r.InActive == false && r.InActive == false && (r.ActiveForNew == this.IsNewEntity));

                this._unConditionalsetFieldValueRules = this._tenantRules.filter(r =>
                    r.RuleTypeCode == "SETV" && r.TriggerTypeCode == "ALLW" && r.InActive == false && r.InActive == false && (r.ActiveForNew == this.IsNewEntity)
                );

                this._conditionalSetFieldValueRules = this._tenantRules.filter(r =>
                    r.RuleTypeCode == "SETV" && r.TriggerTypeCode == "COND" && r.InActive == false && r.InActive == false && (r.ActiveForNew == this.IsNewEntity)
                );

                this._triggeredSetFieldValueRules = this._tenantRules.filter(r =>
                    r.RuleTypeCode == "SETV" && r.TriggerTypeCode == "FLDC" && r.InActive == false && r.InActive == false && (r.ActiveForNew == this.IsNewEntity)
                );
            }
            else {

                this._requiredFieldRules = this._tenantRules.filter(r => r.RuleTypeCode == "REQ" && r.InActive == false && r.InActive == false && (r.ActiveForUpdate));

                this._entityLevelRules = this._tenantRules.filter(r => r.RuleTypeCode == "EVAL" && r.InActive == false && r.InActive == false && (r.ActiveForUpdate));

                this._blockRules = this._tenantRules.filter(r => r.RuleTypeCode == "BLCK" && r.InActive == false && r.InActive == false && (r.ActiveForUpdate));

                this._unConditionalsetFieldValueRules = this._tenantRules.filter(r =>
                    r.RuleTypeCode == "SETV" && r.TriggerTypeCode == "ALLW" && r.InActive == false && r.InActive == false && (r.ActiveForUpdate)
                );

                this._conditionalSetFieldValueRules = this._tenantRules.filter(r =>
                    r.RuleTypeCode == "SETV" && r.TriggerTypeCode == "COND" && r.InActive == false && r.InActive == false && (r.ActiveForUpdate)
                );
    ...
    public ApplyTriggeredSetFieldValueRules(propertyName: string, entity: any, objectTableName: string): void {
        ...
        var entityTableRules: Array<ObjectTableRulePM> = this._triggeredSetFieldValueRules.filter(o => o.ObjectTableId == objectTableId);
    ...
    private SetInsideEntityFieldValue(parentEntity: Object, entity: Object, fieldName: string, ruleField: ObjectTableRuleFieldPM, table: ObjectTablePM) {
        ...
        if (objectField && currentValue && objectField.DataTypeCode == "LookUp") {
            ...
            var insideTable: ObjectTablePM = this._objectTables.filter(t => t.Name == insideEntityName && (t.Tenant == SessionInfo.LoggedUserTenant || t.Tenant == 0))[0];

            var insideObjectField: ObjectFieldPM = this._tenantObjectFields.filter(f => f.FieldName === fieldsAray[i + 1] && f.ObjectTableId === insideTable.Id)[0];
    ...
    public ApplyConditionalSetFieldRules(propertyName: string, entity: any, objectTableName: string, onPropertyChange: boolean): void {
        ...
        for (var k in conditionalTableSetValueRules) {
            var rule = conditionalTableSetValueRules[k];
            var ruleFields: Array<ObjectTableRuleFieldPM> = this._objectTableRuleFields.filter(rf => rf.ObjectTableRuleId == rule.Id);
            if (rule.RuleConditionFields.some(f => f.ObjectFieldName == propertyName) || (ruleFields.some(f => f.ObjectFieldName == propertyName) && !onPropertyChange)) {
    ...
    public ValidateAllRequiredFieldRules(entity: any, objectTableId: string, errorsArray: Array<string>) {
        ...
        var tableRules = this._requiredFieldRules.filter(a => a.ObjectTableId == objectTableId && (a.Tenant == SessionInfo.LoggedUserTenant || a.Tenant == 0));
    ...
    public ExecuteRequierdFieldRule(ruleCode: string, entity: any, requiredFields: Array<ObjectTableRuleFieldPM>) {
        ...
        var rule: ObjectTableRulePM = this._requiredFieldRules.filter(a => a.RuleCode == ruleCode && (a.Tenant == SessionInfo.LoggedUserTenant || a.Tenant == 0) && a.InActive == false)[0];
        if (rule != null) {
            var ruleFields: Array<ObjectTableRuleFieldPM> = this._objectTableRuleFields.filter(rf => rf.ObjectTableRuleId == rule.Id && rf.RuleNotificationTypeCode == "ERR");
    ...
    public ApplyRequiredFieldRules(propertyName: string, entity: Object, objectTableName: string, viewModelBase: any = entity): void {
        if (Settings.DisableRuleValidation) {
            return
        }
        ...
        var fieldFormat: string = "[" + propertyName + "]";
        var advancedConditionalRules: Array<ObjectTableRulePM> = this._requiredFieldRules.filter(r => r.Condition != null && r.AdvancedCondition == true && r.ObjectTableId == table.Id);
        var fieldsConditionalRules: Array<ObjectTableRulePM> = this._requiredFieldRules.filter(r => r.RuleConditionFields.some(f => f.ObjectFieldName == propertyName) && r.AdvancedCondition == false && r.ObjectTableId == table.Id);
        var unConditionalRules: Array<ObjectTableRulePM> = this._requiredFieldRules.filter(r => r.Condition == null && r.TriggerTypeCode == "ALLW" && r.ObjectTableId == table.Id);
        //var type1: Type = entity.GetType();
        //------------------ Advanced Conditional rules
        var advancedRulesToExecute: Array<ObjectTableRulePM> = advancedConditionalRules.filter(r => r.Condition.indexOf(propertyName) != -1 && r.ObjectTableId == table.Id);
    ...
    public SetRequiredFields(ruleFields: Array<ObjectTableRuleFieldPM>, required: boolean, objectTableName: string, entity: any, viewModelBase: any): void {
        //var type1: Type = entity.GetType();
    ...
    public ApplyUnConditionalBlockFieldRules(propertyName: string, entity: any, objectTableName: string, onPropertyChange: boolean, uiPoperty: UIProperty = null): void {
        ...
        var fieldFormat: string = "[" + propertyName + "]";
        var unconditionalTableBlockRules: Array<ObjectTableRulePM> = this._blockRules.filter(r => r.TriggerTypeCode == "ALLW" && r.ObjectTableId == table.Id);
