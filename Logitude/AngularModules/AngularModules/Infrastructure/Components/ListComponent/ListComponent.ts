...
import {
    AmitalGatewayUtil,
    UnifreightMessageM,
} from '../../Utilities/AmitalGatewayUtil';
...
export class ListComponent implements OnInit, AfterViewInit {
    ...
    customsSettingListService: CustomsSettingListService =
    ...
    customsSettingExtendedListService: CustomsSettingExtendedListService =
    ...
    onColumnsClick() {
        ...
        logitudeWindow.Title = TextCodeTranslator.Translate(
            'General.O.QueryColumnsEdit'
        ); //"Query Columns Edit";
        logitudeWindow.WindowArgs = windowArgs;
        logitudeWindow.Show(
            './Infrastructure/Components/QueryColumnsComponents/QueryColumnsEditComponent'
        );
        logitudeWindow.WindowClosed.subscribe(($event: any) => {
            ...
            //     });
            // }
            this.QueryValueChanged({
                QueryCode: this.SelectedQueryCode,
                Title: TextCodeTranslator.Translate(
                    this.SelectedQuery.NameTextCodeCode
                ),
                Filters: this.CurrentQueryFilters,
                IgnoreSearchFields: true,
            });
    ...
    SearchMethod() {
        ...
        var searchFieldName: string = this.IsUseCardSearchMechanism()
            ? 'CardSearchField'
            : 'SearchFields';

        this.CurrentQueryFilters.AdditionalFilters =
            this.CurrentQueryFilters.AdditionalFilters.filter(
                (a) => a.FieldName != searchFieldName
            );
        ...
        this.CurrentQueryFilters.addAdditionalFilter(
            searchFieldName,
            this.searchFields,
            null,
            null,
    ...
    ApplyPreDefinedFilters() {
        if (this.SelectedQuery != null) {
            ...
            if (
                ...
                !AppTool.IsNullOrEmpty(this.listArgs.Filters.SortBy)
            ) {
                this.dataSource.sortingCol = this.listArgs.Filters.SortBy;
            } else {
                this.dataSource.sortingCol =
                    this.SelectedQuery.DefaultSortColumn;
            }
            if (
                this.listArgs &&
                this.listArgs.Filters &&
                !AppTool.IsNullOrEmpty(this.listArgs.Filters.SortDirection)
            ) {
                this.dataSource.sortingDir =
                    this.listArgs.Filters.SortDirection;
            } else {
                this.dataSource.sortingDir =
        ...
        if (
            window.PreDefinedFilters.filter(
                (d) => d.QueryCode == this.SelectedQuery.UniqueCode
            ) != null
        ) {
            var predefinedFilters = window.PreDefinedFilters.filter(
                (d) => d.QueryCode == this.SelectedQuery.UniqueCode
            );
            predefinedFilters.forEach((filter, key) => {
                var filterOperator = !AppTool.IsNullOrEmpty(filter.Operator)
                    ? filter.Operator
                    : filter.ObjectFieldOperator;
                ...
                if (
                    filter.DataTypeCode == 'DateTime' ||
                    filter.DataTypeCode == 'Date'
                ) {
                    ...
                    var CurrentYearFromDate = new Date(
                ...
                var field = window.ObjectFields.filter(
                    (a) => a.FieldCode == filter.ObjectFieldCode
                )[0];
                if (field) {
                    this.CurrentQueryFilters.addAdditionalFilter(
                        filter.ObjectFieldName,
                        value1,
                        value2,
                        null,
                        filterOperator,
                        field.IsCustomFilter,
                        filter.DisplayInList,
                        field.IsCustom,
                        filter.DataTypeCode
                    );
                }
        ...
        if (
            !AppTool.IsNullOrEmpty(this.SelectedQuery.DefaultSortColumn) &&
            AppTool.IsNullOrEmpty(this.CurrentQueryFilters.SortBy)
        ) {
            this.CurrentQueryFilters.SortBy =
                this.SelectedQuery.DefaultSortColumn;
        }
        if (
            !AppTool.IsNullOrEmpty(this.SelectedQuery.DefaultSortDirection) &&
            AppTool.IsNullOrEmpty(this.CurrentQueryFilters.SortDirection)
        ) {
            this.CurrentQueryFilters.SortDirection =
                this.SelectedQuery.DefaultSortDirection;
        }
        if (this.listArgs.SelectedTransportMode != 'All') {
            this.CurrentQueryFilters.addAdditionalFilter(
                'TransportModeId',
                this.listArgs.SelectedTransportMode,
                null,
                null,
                'Equals',
                false,
                ...
                false,
                'string',
                this.listArgs.SelectedTransportMode == 'All' ? true : false
            );
        }
        if (this.listArgs.SelectedDirection != 'All') {
            this.CurrentQueryFilters.addAdditionalFilter(
                'DirectionId',
                this.listArgs.SelectedDirection,
                null,
                null,
                'Equals',
                false,
                ...
                false,
                'string',
                this.listArgs.SelectedDirection == 'All' ? true : false
            );
        }
    }
    ...
    @ViewChildren(LocationDirective)
    public AllLocations: QueryList<LocationDirective>;
    private SessionEvent: any = null;
    private CurrentSession = SessionLocator.SelectedSession;
    constructor(
        public _ListComponentArgs: ListComponentArgs,
        private _http: HttpClient,
        private _entityListService: EntityListService,
        private _entityResourceService: EntityResourceService,
        public pubSubAdvanceQueryFiltersService: PubSubService,
        private temp: PubSubService1,
        private entityPMService: EntityPMService,
        private _totangoService: TotangoService,
        private CD: ChangeDetectorRef
    ) {
        var UsingV2FeatureToggle = SessionLocator.FeatureToggles.filter(
            (d) => d.ToggleCode == 'LV2'
        )[0];
        if (
            UsingV2FeatureToggle ||
            SessionLocator.LoggedUserPM.Email == 'ahmada@logitudeworld.com'
        ) {
            this.UsingLogGridV2 = true;
        }

        if (this.CurrentSession == null) {
            this.ListComponentId = 'ListComponentId_-1_-1';
        } else {
            this.ListComponentId =
                'ListComponentId_' +
                this.CurrentSession.LogitudeGridHelper.GetLListComponentIndexId();
        }
    ...
    processAdvanceQueryFilters(filters) {
        ...
        if (
            filters.IsDeleted ||
            (filters.textValue == '' &&
                filters.textValue.toString() != 'false') ||
            filters.textValue == 'No Filter'
        ) {
            this.AdvanceFilters.AdditionalFilters =
                this.AdvanceFilters.AdditionalFilters.filter(
                    (a) => a.FieldName != filters.FieldName
                );
            ...
            this.AdvanceFilters.addAdditionalFilter(
                filters.FieldName,
                null,
                null,
                null,
                'NoDate',
                filters.ObjectField.IsCustomFilter,
                filters.ObjectField.DisplayInList,
                filters.ObjectField.IsCustom,
                filters.ObjectField.DataTypeCode
            );
            ...
            if (filters.ObjectField.DataTypeCode == 'Boolean') {
                this.AdvanceFilters.addAdditionalFilter(
                    filters.FieldName,
                    filters.TextValue.toString().toLowerCase(),
                    null,
                    null,
                    filters.Operation.Code,
                    filters.ObjectField.IsCustomFilter,
    ...
    ngOnInit() {
        ...
        if (
            ObjectsLocator.IsDemoTenant(SessionLocator.Tenant.toString()) &&
        ...
        if (
            !this.CheckPermissions(this.ObjectTableName, 'NEW', false) &&
        ...
        var subscription =
            this.pubSubAdvanceQueryFiltersService.Stream.subscribe((customer) =>
                this.processAdvanceQueryFilters(customer)
            );
    ...
    public CheckPermissions(
        ...
        if (objectTableName == 'DeploymentPackage')
            return CustomizationPermissionService.HasEntityPermessions(
                objectTableName,
                featureCode,
                showWindow
            );
        return FeatureLocator.HasEntityPermessions(
            objectTableName,
            featureCode,
            showWindow
        );
    ...
    LinkAddDocumentFromLibraryClcik() {
        ...
        logWindow.Show(
            './InfrastructureModules/InfrastructureDocuments/Components/DocumentComponent/FromLibrary/AddDocumentTypeFromLibraryComponent'
        );
    ...
    LinkAddReportFromLibraryClick() {
        ...
        logWindow.WindowArgs = windowArgs;
        logWindow.Show(
            './InfrastructureModules/InfrastructureBIReport/Components/NewEntity/NewBIReport'
        );
    ...
    EditBIReportFolderClicked() {
        ...
        logWindow.WindowArgs = windowArgs;
        logWindow.Show(
            './InfrastructureModules/InfrastructureBIReport/Components/NewEntity/NewBIReportFolderComponent'
        );
        logWindow.WindowClosed.subscribe((event: any) => {
            if (event) {
                this.Title = event;
    ...
    LinkAddQuoteTemplateFromLibraryClcik() {
        ...
        logWindow.Width = 800;
        logWindow.Height = 550;
        logWindow.Title = TextCodeTranslator.Translate(
            'QuoteTemplate.S.NewQuoteTemplate'
        );
        logWindow.WindowArgs = windowArgs;
        logWindow.IsShowCloseButton = true;
        logWindow.Show(
            './QuoteModules/QuoteTemplates/Components/AddQuoteTemplateFromLibraryComponent'
        );
        logWindow.WindowClosed.subscribe(($event: any) => {
            this.isEditControlOpened = false;
            //this.OnBackFromEdit();
    ...
    RunComponent() {
        if (window.Tips) {
            var tip = window.Tips.filter(
                (d) => d.Code == this.ObjectTable.MainTipCode
            )[0];
            if (tip) {
                var hasTip = true;
                ...
                var tipVisibility = window.TipsVisibilities.filter(
                    (d) =>
                        d.TipCode == tip.Code &&
                        d.UserId == SessionInfo.LoggedUserId
                )[0];
        ...
        if (this.ObjectTable.Name == 'Customer') {
            ...
            this.View = TextCodeTranslator.Translate('General.O.View');
        }
        if (FeatureLocator.HasFeaturePermession('General', 'EXPORTEXCEL')) {
            if (
                !AppTool.IsNullOrEmpty(
                    this.ObjectTable.DownloadToExcelFeatureCode
                )
            ) {
                if (
            ...
            } else this.HasExcelExportButton = true;
        }
        ...
        if (this.ObjectTable.Name == 'BIReport') {
            ...
            if (
                FeatureLocator.HasFeaturePermession('BIReportFolder', 'UPDATE')
            ) {
        ...
        if (this.ObjectTable.HasFiltersMenu || this.HasActionBar()) {
            if (this.AllLocations) {
                if (this.AllLocations.length == 0) {
                    ...
                    if (this.ObjectTable.HasFiltersMenu) {
                        let myLocation: LocationDirective =
                            this.AllLocations.toArray().filter(
                                (d) => d.Code == 'MNH'
                            )[0];
                        if (myLocation != null) {
                            let myObjectTableName = this.ObjectTable.Name;
                            if (
                                myObjectTableName.startsWith(
                                    this.ObjectTable.ClientModuleName + '.'
                                )
                            ) {
                                myObjectTableName = myObjectTableName.substr(
                                    (this.ObjectTable.ClientModuleName + '.')
                                        .length
                                );
                            }
                            ...
                            if (
                                this.ObjectTable.ClientModuleName == 'Customs'
                            ) {
                            ...
                            var myComponentPath =
                                ...
                                /*this.ObjectTable.Name*/ myObjectTableName +
                            ...
                            SessionLocator.DynamicLoader.Load(
                                ...
                                if (
                                    this.listArgs.Filters != null &&
                                    isCustomsObjectTableWith
                                ) {
                                    cmpRef.instance.SetFiltersMenu(
                                        this.listArgs.Filters
                                    );
                                }
                                    cmpRef.instance.SelectedValueChanged.subscribe(($event: any) => {
                                        ...
                                        this.MenuHeaderchangeevent.emit({
                                            ...
                                            RemoveFilter: $event.RemoveFilter,
                                        });
                                    }
                                );
                                cmpRef.instance?.CustomGetTotalCount?.subscribe(
                                    ($event: number) => {
    ...
    Run(args: ListComponentArgs) {
        ...
        if (!this.IsDemoTenant) {
            ...
            this.MenuTableQuerySection = args.QuerySection
                ? args.QuerySection
                : args.ObjectTableName;

            this.ObjectTable = window.ObjectTables.filter(
                (x) => x.Name === this.ObjectTableName
            )[0];
            this.ObjectTableDisplayName =
                new TextCodeTranslationPipe().transform(this.ObjectTable.Name);
    ...
    private HasMultiUpdateFeature(): boolean {
        if (this.ObjectTableName == 'Shipment') {
            return FeatureLocator.HasFeaturePermession(
                'Shipment',
                'MULTIUPDATE'
            );
    ...
    private IsMultiPrintFeatureOn(): boolean {
        if (this.ObjectTableName == 'ARInvoice') {
            ...
            return (
                ...
                FeatureLocator.HasFeaturePermession('General', 'MultiPrint')
            );
        }
    }
    ...
    MutliUpdate() {
        ...
        newWindow.Show(
            './Infrastructure/Components/MultiUpdateComponent/MultiEntityUpdateBaseComponent'
        );
    ...
    ViewInitCompleted(event) {
        ...
        this._entityResourceService
            .getEntityResourceByTableName(this.ObjectTableName, 0)
            .subscribe((response: any) => {
                ...
                this.ViewQuery(
                    this.listArgs.Filters,
                    this.listArgs.DisplayTitle,
                    this.listArgs.BackButtonTitle,
                    this.listArgs.IsReadOnlyList,
                    this.listArgs.IsBackToCurrentListView
                );
                //this.CD.detectChanges();
            });
    }
    ...
    ShowFieldsList() {
        document
            .getElementById('FieldsDropdown')
            .classList.toggle('showDDButton');
    }
    ...
    SetAddButtonTitle() {
        if (
            ...
            this.ObjectTableName == 'Port' ||
            this.ObjectTableName == 'Warehouse'
        ) {
            this.AddButtonTitle = TextCodeTranslator.Translate(
                'General.O.ImportEntities'
            ).replace(
                /%Entity/g,
                TextCodeTranslator.TranslateTablePlural(this.ObjectTableName)
            );
        }
    }
    ...
    FilterQuerysByQuerySection(allQueries: any) {
        let querySection: string = !AppTool.IsNullOrEmpty(
        ...
        return allQueries.filter(
            (d) =>
                d.QuerySection == querySection ||
                d.QuerySection == querySection + 'FollowUp'
        );
    }
    ...
    ApplyQueriesAdvancedFilter(allQueries: any) {
        if (this.ObjectTableName != 'Shipment') return this.Queries;
        if (SessionLocator.TenantPM.ApproveUploadedDocuments == false) {
            return allQueries.filter(
                (x) =>
                    x.Code != 'Pending Approval Documents' &&
                    x.UserId == null &&
                    x.SystemLevel == true
            );
        }
    ...
    CustomerCareDeploymentPackage(objectTableName: string) {
        if (objectTableName != 'DeploymentPackage') return false;
        if (
            FeatureLocator.IsFeatureGrantedByUniqeCode(
                'General.Customization.DeploymentPackage'
            )
        )
            return false;
        if (
            SessionLocator.LoggedUserPM.IsCustomerCare ||
            SessionLocator.LoggedUserPM.IsDistributor ||
    ...
    GetQueries() {
        var allQueries: any[] = window.Queries.filter(
            (x) => x.ObjectTableId === this.ObjectTable.Id
        ).sort((a, b) => {
        ...
        this.Queries = allQueries.filter(
            (x) => x.UserId == null && x.SystemLevel == true
        );

        if (this.ObjectTableName == 'DeploymentPackage')
            this.listArgs.DontCheckQueryFeature = true;
        if (
            !this.ObjectTable.IsClosed &&
            !this.listArgs.DontCheckQueryFeature
        ) {
            this.Queries = allQueries.filter(
                (x) =>
                    FeatureLocator.IsFeatureGrantedByUniqeCode(
                        x.FeatureUniqeCode
                    ) || this.CustomerCareDeploymentPackage(x.ObjectTableName)
            );
        }
        this.Queries = this.ApplyQueriesAdvancedFilter(allQueries);

        this.UserQueries = allQueries.filter(
            (x) => x.UserId != null && x.Tenant == SessionInfo.LoggedUserTenant
        );

        if (
            this.listArgs.Perspective != null &&
            this.listArgs.IgnoreSelectedPerspective == false
        ) {
            ...
            this.SelectedQuery = allQueries.filter(
                (f) => f.Perspective == this.listArgs.Perspective
            )[0];
            ...
            this.Queries = allQueries.filter(
                (f) =>
                    ...
                    (FeatureLocator.IsFeatureGrantedByUniqeCode(
                        ...
                        this.ObjectTable?.IsCustom ||
                        this.CustomerCareDeploymentPackage(
                            f.ObjectTableName
                        )) &&
                    f.SystemLevel == true &&
                    f.Perspective == this.listArgs.Perspective
            );
        } else if (
            this.listArgs.Perspective != null &&
            this.listArgs.IgnoreSelectedPerspective == true
        ) {
            ...
            if (this.QueryCode.includes(this.ObjectTableName + '.')) {
                this.SelectedQuery = allQueries.filter(
                    (f) =>
                        f.UniqueCode ==
                        /*this.ObjectTableName+ (f.UserId!= undefined?"." + f.UserId:"") + '.'+*/ this
                            .QueryCode
                )[0];
                if (this.SelectedQuery == null) {
                    this.SelectedQuery = allQueries.filter(
        ...
        let forceExistQuery =
            ObjectsLocator.GlobalSetting.WorkEnvironment == 'customs';
            ...
            if (!existSelectedQuery && this.UserQueries != null) {
                existSelectedQuery =
                    this.UserQueries.filter(
                        (r) => r.Code == this.SelectedQuery.Code
                    ).length > 0;
        ...
        if (AppTool.IsNullOrEmpty(this.Title) && this.SelectedQuery) {
            this.Title = TextCodeTranslator.Translate(
                this.SelectedQuery.NameTextCodeCode
            );
        }
        if (this.SelectedQuery != null) {
    ...
    IFSelectedQueryEmpty() {
        return !this.SelectedQuery && this.Queries && this.Queries.length > 0
            ? true
            : false;
    }
    ...
    CheckIfQueriesConatinDefaultPerspectiveQuery() {
        if (this.Queries && !AppTool.IsNullOrEmpty(this.listArgs.Perspective)) {
            ...
            for (let i = 0; i < this.Queries.length; i++) {
                if (
                    this.Queries[i].UniqueCode == this.SelectedQuery.UniqueCode
                ) {
    ...
    GetQueryColumns(queryCode, userId) {
        ...
        this._http
            ...
            .subscribe((response: any) => {
                ...
                this.QueryColumns = this.QueryColumns.sort((a, b) => {
                    return a.IndexOrder > b.IndexOrder
                        ? 1
                ...
                this.QueryColumns.forEach((value, key) => {
                    ...
                    var mutaztouch14 = window.ObjectFields.filter(
                        (x) => x.FieldCode === value.ObjectFieldCode
                    );
                    var mutazTouch = window.ObjectFields.filter(
                        (x) => x.FieldCode === value.ObjectFieldCode
                    )[0];

                    this.columnsObjectFields.push(
                        window.ObjectFields.filter(
                            (x) => x.FieldCode === value.ObjectFieldCode
                        )[0]
                    );
                });
                ...
                for (var i = 0; i < this.QueryColumns.length; i++) {
                    var CurColumn = this.columns.filter(
                        (a) =>
                            a.FieldName == this.QueryColumns[i].ObjectFieldName
                    );
                    if (
                        this.columns != null &&
                        (CurColumn == null || CurColumn.length == 0)
                    ) {
                        this.columns.push({
                            ...
                            DataTypeCode:
                                this.columnsObjectFields[i].DataTypeCode,
                            Display: TextCodeTranslator.Translate(
                                this.columnsObjectFields[i].ListTextCodeCode
                            ),
                            Styles: {
                                width: this.QueryColumns[i].ColumnWidth + 'px',
                            },
                            HtmlListComponentName:
                                this.columnsObjectFields[i]
                                    .HtmlListComponentName, //'TransportModeCellDisplayListTemplate',
                            HtmlListComponentUrl:
                                this.columnsObjectFields[i]
                                    .HtmlListComponentUrl, //'./Shipment/Components/ListTemplates/TransportModeCellDisplayListTemplate',
                            ServerSideSortable: true, //this.columnsObjectFields[i].CanFilter
                            ColumnHeaderTemplateName:
                                this.columnsObjectFields[i]
                                    .ColumnHeaderTemplateName, //'TransportModeCellDisplayListTemplate',
    ...
    QueryValueChanged(Args) {
        ...
        this.UserQueries = window.Queries.filter(
            (x) =>
                x.ObjectTableId === this.ObjectTable.Id &&
                x.UserId != null &&
                x.Tenant == SessionInfo.LoggedUserTenant
        );
        this.Title = Args.Title;
        this.columns = [];
        this.columnsObjectFields = [];
        if (this.Queries == null || this.Queries.length == 0) {
            var MyQueries = window.Queries.filter(
                (x) => x.ObjectTableId === this.ObjectTable.Id
            ).sort((a, b) => {
                return a.IndexOrder - b.IndexOrder;
            });
            this.SelectedQuery = MyQueries.filter(
                (x) => x.UniqueCode === Args.QueryCode
            )[0];
        } else {
            this.SelectedQuery = this.Queries.filter(
                (x) => x.UniqueCode === Args.QueryCode
            )[0];
        }

        if (this.SelectedQuery == null) {
            this.SelectedQuery = this.UserQueries.filter(
                (x) => x.UniqueCode === Args.QueryCode
            )[0];
        }
        if (this.SelectedQuery != null) {
            ...
            if (
                ...
                predefinedFilters.forEach((filter, key) => {
                    ...
                    if (
                        ...
                        var YesterdayDate = DateTool.AddDays(new Date(), -1);
                        YesterdayDate.setUTCHours(0, 0, 0, 0);
                        var LastSevenDaysDate = DateTool.AddDays(
                            new Date(),
                            -7
                        );
                        LastSevenDaysDate.setUTCHours(0, 0, 0, 0);
                        var LastThirtyDaysDate = DateTool.AddDays(
                            new Date(),
                            -30
                        );
                        LastThirtyDaysDate.setUTCHours(0, 0, 0, 0);
                        ...
                        var CurrentYearToDate = DateTool.AddDays(new Date(), 1);
                        CurrentYearToDate.setUTCHours(0, 0, 0, 0);
                        var LastYearFromDate = DateTool.AddDays(
                            new Date(),
                            -365
                        );
                        LastYearFromDate.setUTCHours(0, 0, 0, 0);
                ...
                case "Last Year":
                    ...
                    if (field) {
                        if (
                            Args.Filters.AdditionalFilters.filter(
                                (a) => a.FieldName == filter.ObjectFieldName
                            ).length > 0
                        ) {
                            Args.Filters.AdditionalFilters =
                                Args.Filters.AdditionalFilters.filter(
                                    (a) => a.FieldName != filter.ObjectFieldName
                                );
                        }
                        Args.Filters.addAdditionalFilter(
                            filter.ObjectFieldName,
                            value1,
                            value2,
                            null,
        ...
        if (this.FiltersMenu) {
            this.FiltersMenu.AdditionalFilters.forEach((filter, key) => {
                if (
                    Args.Filters &&
                    (filter.IgnoreFilter || filter['SpecificMenuFilter'])
                ) {
                    Args.Filters.AdditionalFilters =
                        Args.Filters.AdditionalFilters.filter(
                            (a) => a.FieldName != filter.FieldName
                        );
                } else {
    ...
    QueriesChangedEvent(Args) {
        ...
        this.UserQueries = window.Queries.filter(
            (x) =>
                ...
                x.SystemLevel == false &&
                x.Tenant == SessionInfo.LoggedUserTenant &&
                x.QuerySection == this.MenuTableQuerySection
        );
        ...
        if (SelectedQuery) {
            if (
                SelectedQuery.QueryGroupCode == 'SFLU' ||
                SelectedQuery.QueryGroupCode == 'QFLU'
            ) {
                ServiceLocator.SendTotangoUserActivity(
                    'Shipment',
                    'FollowUpQueryUse'
                );
            }
    ...
    private ViewQuery(
        ...
        if (query != null) {
            ...
            if (
                window.PreDefinedFilters.filter(
                    (d) => d.QueryCode == query.UniqueCode
                ) != null
            ) {
                var predefinedFilters = window.PreDefinedFilters.filter(
                    (d) => d.QueryCode == query.UniqueCode
                ...
                predefinedFilters.forEach((filter, key) => {
                    ...
                    if (value1 == '#logged-user')
                        value1 = SessionLocator.LoggedUserId;
            ...
            if (
                !AppTool.IsNullOrEmpty(query.DefaultSortColumn) &&
                AppTool.IsNullOrEmpty(filterAgrs.SortBy)
            ) {
                filterAgrs.SortBy = query.DefaultSortColumn;
            }
            if (
                !AppTool.IsNullOrEmpty(query.DefaultSortDirection) &&
                AppTool.IsNullOrEmpty(filterAgrs.SortDirection)
            ) {
            ...
            var ListComponentPostFex = this.ListComponentId.replace(
                'ListComponentId_',
                ''
            );
            ...
            this.FilterChangedEvent.emit({
                QueryCode: query.UniqueCode,
                Filters: filterAgrs,
                ListComponentPostFex: ListComponentPostFex,
            });
        }
    }
    ...
    getRows(
        ...
        if (
            this.listArgs.DefaultFilterItems &&
            this.listArgs.DefaultFilterItems.length > 0
        ) {
        ...
        return this._entityListService.getByFilters(
            this.ObjectTableName,
            MyFilters,
            this.MethodName == undefined ? null : this.MethodName
        );
    }
            ...
            if (!this.isEditControlOpened) {
                ...
                if (myObjectTableName != 'TicketEscalation') {
                    ...
                    if (
                        ...
                        if (
                            entityList.DirectionId == 'E' ||
                            entityList.DirectionId == 'R'
                        ) {
                            isFullWizard = true;
                        ...
                        logWindow.WindowClosed.subscribe(($event1: any) => {
                            $event.isEntityChange =
                                this.getIsEntityChange(logWindow);
                            this.isEditControlOpened = false;
                            this.OnBackFromEdit(selectedEntityId, $event);
                        });
                    } else if (
                        ...
                        windowTitle = TextCodeTranslator.Translate(
                            'General.O.EditEntity'
                        ).replace(
                            '%Entity',
                            TextCodeTranslator.Translate(myObjectTableName)
                        );
                        ...
                        !AppTool.IsNullOrEmpty(
                            this.SelectedQuery.EditWizardName
                        ) ||
                        myObjectTableName == 'AgentSharedManifest' ||
                        myObjectTableName == 'Customs.CourierMaster'
                    ) {
                        if (
                            this.SelectedQuery.EditWizardName ==
                        ...
                        if (
                            this.SelectedQuery.EditWizardName ==
                            'Simplog.ShipmentLib.Views.AWBWizardEditControl'
                        ) {
                            var isFullWizard = false;
                            var windowTitle = null;
                            ...
                            this._entityResourceService
                                .getEntityResourceByTableName(
                                    'Customs.CourierMaster'
                                )
                                .subscribe((response: any) => {
                                    this._entityResourceService
                                        .getEntityResourceByTableName(
                                            'Customs.DeclarationCourierStatus'
                                        )
                                        .subscribe((response: any) => {
                                            this._entityResourceService
                                                .getEntityResourceByTableName(
                                                                ...
                                                                (
                                                                    ...
                                                                    if (
                                                                        ...
                                                                        logWindow.WindowArgs =
                                                                            windowArgs;
                                                                        logWindow.ShowCloseButton =
                                                                            true;
                                                                        //logWindow.IsHideHeader = true;
                                                                        logWindow.IsFillScreen =
                                                                            true;
                                                                        AmitalGatewayUtil.Instance.IsAmitalBackButtonDisable =
                                                                            true;
                                                                            ...
                                                                            .subscribe(
                                                                                (
                                                                                    ...
                                                                                    if (
                                                                                        ...
                                                                                        logWindow.Show(
                                                                                            './CustomsModules/CustomsCourier/Components/CourierWorkSheet/CourierWorksheetComponent'
                                                                                        );
                                                                                    } else {
                                                                                        if (
                                                                                            response
                                                                                            ...
                                                                                            logWindow.Show(
                                                                                                './CustomsModules/CustomsCourier/Components/CourierWorkSheet/CourierWorksheetNGComponent'
                                                                                            );
                                                                                        } else {
                                                                                            logWindow.Show(
                                                                                                './CustomsModules/CustomsCourier/Components/CourierWorkSheet/CourierWorksheetComponent'
                            ...
                            switch (myObjectTableName) {
                                case 'TenantManagmentPrivateLabels': {
                                    ...
                                    logWindow.WindowArgs = selectedEntityId;
                                    logWindow.Show(
                                        './InfrastructureModules/InfrastructureTenantManagement/Components/TenantManagement/PrivateLabelLoadComponent'
                                    );
                                ...
                                case 'Booking': {
                                    ...
                                    logWindow.WindowArgs = selectedEntityId;
                                    logWindow.Show(
                                        './Booking/Components/BookingWizard/BookingWizardLoadComponent'
                                    );
                                ...
                                case 'Card': {
                                    var windowArgs: any = {};
                                    if (
                                        this.listArgs.IsDigitalPortalMenuClicked
                                    ) {
                                        windowArgs.IsDigitalPortal = true;
                                    }
                                    windowArgs.CurrentEntity = entityList;
                                    windowArgs.IsCargoTrackingMenuClicked =
                                        this.listArgs.IsCargoTrackingMenuClicked;
                                    var logWindow = new LogitudeWindow();
                                    ...
                                    logWindow.IsShowCloseButton = true;
                                    logWindow.Show(
                                        './SharedLogistics/Components/InviteCustomersComponent'
                                    );
                                    logWindow.WindowClosed.subscribe(
                                        ($event1: any) => {
                                            this.isEditControlOpened = false;
                                ...
                                case 'Customs.CustomsVendor': {
                                    if (
                                        !AppTool.IsNullOrEmpty(selectedEntityId)
                                    ) {
                                        this._entityResourceService
                                            .getEntityResourceByTableName(
                                                'Customs.CustomsVendor'
                                            )
                                            .subscribe((response: any) => {
                                                this._entityResourceService
                                                    .getEntityResourceByTableName(
                                                        'Customs.VendorCommunication'
                                                    )
                                                    .subscribe(
                                                        (response: any) => {
                                                            this.entityPMService
                                                                            ...
                                                                            (
                                                                                ...
                                                                                if (
                                                                                    ...
                                                                                    logWindow.ShowCloseButton =
                                                                                        true;
                                                                                    logWindow.Show(
                                                                                        './CustomsModules/CustomsVendor/Components/EditTabs/VendorEditComponent'
                                                                                    );
                                                                                        ...
                                                                                        (
                                                                                            ...
                                                                                            $event.isEntityChange =
                                                                                                (
                                                                                                    logWindow
                                                                                                        .InstanceComponent
                                                                                                        .ComponentInstance as VendorEditComponent
                                                                                                ).GENERAL.isEntityChange;
                                                                                            this.isEditControlOpened =
                                                                                                false;
                                                                                            this.OnBackFromEdit(
                                                                                                selectedEntityId,
                                ...
                                case 'Customs.CustomsCollateral': {
                                    ...
                                    this._entityResourceService
                                        .getEntityResourceByTableName(
                                            'Customs.CustomsCollateral'
                                        )
                                        .subscribe((response: any) => {
                                            this._entityResourceService
                                                .getEntityResourceByTableName(
                                                    'Customs.CustomsCollateralsAnswer'
                                                )
                                                .subscribe((response: any) => {
                                                    this._entityResourceService
                                                        .getEntityResourceByTableName(
                                                        ...
                                                        .subscribe(
                                                            (response: any) => {
                                                                this._entityResourceService
                                                                    .getEntityResourceByTableName(
                                                                        'Customs.CustomsCollateralsCondition'
                                                                    )
                                                                    .subscribe(
                                                                        (
                                                                            response: any
                                                                                                    ...
                                                                                                    res.subscribe(
                                                                                                        (
                                                                                                            ...
                                                                                                            if (
                                                                                                                ...
                                                                                                                logWindow.ShowCloseButton =
                                                                                                                    true;
                                                                                                                logWindow.IsHideHeader =
                                                                                                                    true;
                                                                                                                logWindow.Show(
                                                                                                                    './CustomsModules/CustomsCollateral/Components/CustomsCollateralComponent'
                                                                                                                );
                                                                                                                    ...
                                                                                                                    (
                                                                                                                        ...
                                                                                                                        $event.isEntityChange =
                                                                                                                            (
                                                                                                                                logWindow
                                                                                                                                    .InstanceComponent
                                                                                                                                    .ComponentInstance as CustomsCollateralComponent
                                                                                                                            ).isEntityChange;
                                                                                                                        this.isEditControlOpened =
                                                                                                                            false;
                                                                                                                        this.OnBackFromEdit(
                                                        ...
                                                        (myResponse: any) => {
                                                            if (
                                                                ...
                                                                logWindow.ShowCloseButton =
                                                                    true;
                                                                logWindow.Show(
                                                                    './CustomsModules/CustomsProceduralFault/Components/EditTabs/General/ProceduralFaultsGeneralTabComponent'
                                                                );
                                                                    ...
                                                                    (
                                                                        ...
                                                                        $event.isEntityChange =
                                                                            (
                                                                                logWindow
                                                                                    .InstanceComponent
                                                                                    .ComponentInstance as ProceduralFaultsGeneralTabComponent
                                                                            ).isEntityChange;
                                                                        this.isEditControlOpened =
                                                                            false;
                                                                        this.OnBackFromEdit(
                                ...
                                case 'AgentSharedManifest': {
                                    ...
                                    logWindow.IsShowCloseButton = true;
                                    logWindow.Show(
                                        './ShipmentModules/ShipmentSharedManifest/Components/SharedManifestComponent'
                                    );
                                    ...
                                    logWindow.WindowClosed.subscribe(
                                        ($event1: any) => {
                                            this.isEditControlOpened = false;
                                            $event.isEntityChange = (
                                                logWindow.InstanceComponent
                                                    .ComponentInstance as SharedManifestComponent
                                            ).isEntityChange;
                                            this.OnBackFromEdit(
                                                selectedEntityId,
                                                $event
                                                            ...
                                                            res.subscribe(
                                                                (
                                                                    ...
                                                                    if (
                                                                        ...
                                                                        logWindow.Title =
                                                                            'בקשת פיצול מטען '; // + myResponse.Result != null ? ((!AppTool.IsNullOrEmpty(myResponse.Result.RequestNumber) ? myResponse.Result.RequestNumber : null) + ((!AppTool.IsNullOrEmpty(myResponse.Result.ResponseStatusName) ? " - " + myResponse.Result.ResponseStatusName : null))) : null;
                                                                        if (
                                                                            myResponse.Result !=
                                                                            null
                                                                        ) {
                                                                            ...
                                                                            (
                                                                                ...
                                                                                $event.isEntityChange =
                                                                                    (
                                                                                        logWindow
                                                                                            .InstanceComponent
                                                                                            .ComponentInstance as CargoSplitGeneralTabComponent
                                                                                    ).isEntityChange;
                                                                                this.isEditControlOpened =
                                                                                    false;
                                                                                this.OnBackFromEdit(
                                        ...
                                        .then((res: any) => {
                                            res.subscribe((myResponse: any) => {
                                                if (myResponse.HasError) {
                                                    ...
                                                    //logWindow.IsHideHeader = true;
                                                    logWindow.Show(
                                                        './CustomsModules/CustomsLogisticActionRequest/Components/EditTabs/General/LogisticActionRequestGeneralTabComponent'
                                                    );
                                                        ...
                                                        ($event1: any) => {
                                                            $event.isEntityChange =
                                                                (
                                                                    logWindow
                                                                        .InstanceComponent
                                                                        .ComponentInstance as LogisticActionRequestGeneralTabComponent
                                                                ).isEntityChange;
                                                            this.isEditControlOpened =
                                                                false;
                                                            this.OnBackFromEdit(
                        ...
                        SessionLocator.DynamicLoader.Load(
                            './InfrastructureModules/InfrastructureBIReport/Components/Workspaces/BIReportPreviewComponent',
                            this.CurrentSession.SessionLocation.viewContainerRef
                        ).then((cmpRef) => {
                            cmpRef.instance.ComponentRef = cmpRef;
                            cmpRef.instance.Run({
                            ...
                            cmpRef.instance.BackCompleted.subscribe(
                                ($event1: any) => {
                                    $event.isEntityChange = (
                                        cmpRef.instance as BIReportPreviewComponent
                                    ).isEntityChange;
                                    this.isEditControlOpened = false;
                                    this.OnBackFromEdit(
                                        selectedEntityId,
                        ...
                        myObjectTableName == 'Customs.Containerization'
                    ) {
                        SessionLocator.DynamicLoader.Load(
                            './Infrastructure/Components/EditComponent/EditComponent',
                            SessionLocator.SelectedSession.SessionLocation
                                .viewContainerRef
                        ).then((cmpRef) => {
                            cmpRef.instance.ComponentRef = cmpRef;
                            cmpRef.instance.Run({
                            ...
                            cmpRef.instance.BackCompleted.subscribe(
                                ($event1: any) => {
                                    $event.isEntityChange =
                                        cmpRef.instance.isEntityChange;
                                    this.isEditControlOpened = false;
                                    this.OnBackFromEdit(
                                        selectedEntityId,
                                        $event
                        ...
                        myObjectTableName == 'WorkFlowVersion' ||
                        myObjectTableName == 'WorkFlowInstance'
                    ) {
                        this.isEditControlOpened = false;
                        ...
                        var customFile = '';
                        if ($event != null)
                            customFile = $event.rowData.CustomFileNo;
                        let myViewModelName =
                            'FieldTemplateComponent.ts-ShowCFIUFILEFromDeclarationReferantData';
                        if (AmitalGatewayUtil.Instance.AmitalBrowserInUse) {
                            SessionLocator.SelectedSession.StartBusyIndicatorLoading();
                            let sub =
                                AmitalGatewayUtil.Instance.UnifaceRequestArrived.subscribe(
                                    (mess: UnifreightMessageM) => {
                                        ...
                                        if (IsMatchUnifreightCallbackCommand) {
                                            ...
                                            let sBool =
                                                UnifreightMessageM.GetStringValue(
                                                    mess,
                                                    AmitalGatewayUtil.Instance
                                                        .DeclarationMessaging
                                                        .UnifreightResponseStatus
                                                );
                            ...
                            var unifreightMessageM =
                                AmitalGatewayUtil.Instance.DeclarationMessaging.GetMessage(
                                    customFile,
                                    selectedEntityId,
                                    myViewModelName,
                        ...
                        logWindow.Show(
                            './ShipmentModules/ShipmentPackages/Components/Packages/ContainerFU/ContainerFollowupWizardComponent'
                        );
                        logWindow.WindowClosed.subscribe(($event1: any) => {
                            $event.isEntityChange =
                                logWindow.ComponentRef.instance.isEntityChange;
                            this.isEditControlOpened = false;
                            this.OnBackFromEdit(selectedEntityId, $event);
                        });
                    } else if (myObjectTableName == 'GLAccount') {
                        GLAccountSecurityLevelService.CheckLevel(
                            selectedEntityId
                        ).then((hasAccess) => {
                            if (hasAccess) {
    ...
    private getIsEntityChange(logWindow: LogitudeWindow): any {
        return (
            (
                logWindow.InstanceComponent
                    .ComponentInstance as AWBWizardLoadComponent
            ).childComponentInstance as AWBWizardComponent as any
        )?.isEntityChange;
    }

    private OpenEditComponent(
        selectedEntityId: any,
        myObjectTableName: string,
        $event: any
    ) {
        SessionLocator.DynamicLoader.Load(
            './Infrastructure/Components/EditComponent/EditComponent',
    ...
    private ShowINTTRABookingWizard(selectedEntityId: string, $event) {
        ...
        logWindow.WindowArgs = selectedEntityId;
        logWindow.Show(
            './ShipmentModules/ShipmentINTTRA/Components/Wizard/SimulatorBookingLoadComponent'
        );
    ...
    OnBackFromEdit(selectedEntityId, $event) {
        ...
        if (
            ...
            this.MethodName.indexOf('Customs.') > -1
        ) {
        ...
        this._entityListService
            ...
            .then((res: any) => {
                ...
                res.subscribe((aa: any) => {
                    const backFromEdid: any = {
                        Data: aa.Result,
                        rowIndex: $event.rowIndex,
                        rowData: $event.rowData,
                        isEntityChange: $event.isEntityChange,
                    };

                    if (
                        !AppTool.IsNullOrEmpty($event) &&
                        !AppTool.IsNullOrEmpty($event.BackFromEdit)
                    ) {
                        ...
                        if (AppTool.IsNullOrEmpty($event.rowIndex)) {
                            ...
                            if (this.MyLogGridComponent) {
                                this.MyLogGridComponent.BackFromEditAction({
                                    Data: aa.Result,
                                    rowIndex: $event.rowIndex,
                                });
                            }
                            if (this.MyLogGridComponentV2) {
                                this.MyLogGridComponentV2.BackFromEditAction({
                                    Data: aa.Result,
                                    rowIndex: $event.rowIndex,
                                });
                            }
    ...
    private SetNewEntityLabel() {
        if (this.HaveFeatureNewExportDeclararion()) {
            this.NewEntityButtonLabel = TextCodeTranslator.Translate(
                'Customs.General.O.NewExportDeclaration'
            );
        } else if (this.listArgs.NewButtonLabel != null) {
            ...
            this.NewEntityButtonLabel =
                TextCodeTranslator.Translate('General.B.Add');
            ...
            if (AppTool.IsNullOrEmpty(this.listArgs.NewButtonLabel)) {
                var tempText = TextCodeTranslator.Translate(
                    this.listArgs.ObjectTableName + '.NewButton'
                );
            ...
            if (AppTool.IsNullOrEmpty(this.listArgs.NewButtonLabel)) {
                ...
                if (useLocal == true) {
                    ...
                    if (this.ObjectTableName == 'Customs.Vehicle') {
                        ChangedText =
                            TextCodeTranslator.Translate('General.O.New');
                    }

                    var NewText = TextCodeTranslator.TranslateTable(
                        this.ObjectTableName
                    );
                    ...
                    this.NewEntityButtonLabel = TextCodeTranslator.Translate(
                        ...
                        '%Entity',
                        TextCodeTranslator.TranslateTable(this.ObjectTableName)
                    );
                }
            } else {
                this.NewEntityButtonLabel = this.listArgs.NewButtonLabel;
        ...
        if (this.SelectedQuery != null) {
            if (this.SelectedQuery.IsAddNewEntityEnabled) {
                ...
                if (ObjectsLocator.IsDemoTenant(this.TenantPM.Id.toString())) {
                    ...
                    if (
                        SessionInfo.LoggedUserPM.IsCustomerCare &&
    ...
    private SetNewEntityButtonVisibility() {
        var isVisible = true;
        if (
            this.TenantPM.IsHybrid &&
            (this.ObjectTableName == 'User' ||
                this.ObjectTableName == 'Branche' ||
                this.ObjectTableName == 'Department' ||
                this.ObjectTableName == 'City' ||
                this.ObjectTableName == 'Vessel' ||
                this.ObjectTableName == ' Specialservice')
        ) {
            isVisible = false;
                ...
                switch (this.ObjectTableName) {
                    ...
                    case 'Customs.Declaration': {
                        ...
                        this.customsSettingListService
                            .getSingleFromCache(this.TenantPM.Id.toString())
    ...
    HaveFeatureNewExportDeclararion(): boolean {
        let b1 = FeatureLocator.HasFeaturePermession(
            this.ObjectTableName,
            'EXPORTDECLARATIONNEW2'
        );
        let b2 = FeatureLocator.HasFeaturePermession(
            this.ObjectTableName,
            'EXPORTDECLARATIONPSCREEN'
        );
    ...
    AddNewEntity() {
        ...
        if (
            ...
            AppTool.IsNullOrEmpty(this.ObjectTable.NewWizardControlName)
        ) {
        ...
        if (this.SelectedQuery != null) {
            if (
                !this.CheckPermissions(this.ObjectTableName, 'NEW', true) &&
                !this.ObjectTable?.IsCustom
            ) {
                return;
            }
            ...
            if (this.TenantPM.Id != 0 && this.ObjectTableName == 'Port') {
                ...
                if (useLocal == true) {
                    ...
                    var FinalText = TextCodeTranslator.Translate(
                        'General.O.NewEntity'
                    ).replace(
                        '%Entity',
                        TextCodeTranslator.TranslateTable(this.ObjectTableName)
                ...
                var message = TextCodeTranslator.Translate(
                    'General.M.AddingIsNotAvailable'
                ).replace(
                    /%Entity/g,
                    TextCodeTranslator.Translate(this.ObjectTableName)
                );
                    ...
                    .subscribe((response: any) => {
                        ...
                        if (isNewWizard) {
                            ...
                            if (
                                ...
                                if (
                                    query.Code == 'Masters' ||
                                    query.Code == 'Open Payables Masters' ||
                                    query.Code == 'All Masters'
                                ) {
                                    IsOriginalMaster = true;
                                }
                            }
                            if (
                                this.QueryCode == 'Masters' ||
                                this.QueryCode == 'Open Payables Masters' ||
                                this.QueryCode == 'All Masters' ||
                                IsOriginalMaster
                            ) {
                                ...
                                if (
                                    ...
                                    'Logitude.Customs.NewDeclarationControlCommand'
                                ) {
                                    if (this.HaveFeatureNewExportDeclararion())
                                    ...
                                    if (
                                        this.SelectedQuery
                                            .ObjectTableNewWizardControlName ==
                                        'Logitude.Customs.NewContainerizationControlCommand'
                                    ) {
                            ...
                            if (this.ObjectTableName == 'APPayment') {
                                ...
                                this.ObjectTableName == 'UserDefinedReport'
                            ) {
    ...
    RunNewExportDeclaration() {
        var logWindow = new LogitudeWindow();
        logWindow.Title = TextCodeTranslator.Translate(
            'Customs.General.O.OpenNewDeclarationExport'
        );
        logWindow.Width = 800;
        logWindow.Height = 500;
        logWindow.NewWizardArgs = { IsNewEntity: true };
        logWindow.Show(
            './CustomsModules/CustomsDeclarationModules/DeclarationOthers/Components/NewEntity/NewExportDeclarationComponent'
        );
        logWindow.WindowClosed.subscribe(($event: any) =>
    ...
    RunNewDeclaration() {
        var logWindow = new LogitudeWindow();
        logWindow.Title = TextCodeTranslator.Translate(
            'Customs.Declaration.O.NewDeclaration'
        );
        logWindow.Width = 800;
        logWindow.Height = 500;
        logWindow.NewWizardArgs = { IsNewEntity: true };
        logWindow.Show(
            './CustomsModules/CustomsDeclarationModules/DeclarationOthers/Components/NewEntity/NewDeclarationComponent'
        );
        logWindow.WindowClosed.subscribe(($event: any) =>
            this.OnNewEntityWindowClosed($event)
        );
    ...
    RunNewContainerization() {
        ...
        logWindow.Show(
            './CustomsModules/CustomsContainerization/Components/NewEntity/NewContainerizationComponent'
        );
        logWindow.WindowClosed.subscribe(($event: any) =>
            this.OnContainerizationWindowClosed($event)
        );
    }
    private OnContainerizationWindowClosed($event: any) {
        if ($event != null && $event != '0' && $event != 'cancel') {
            var item = this.CurrentQueryFilters.AdditionalFilters.filter(
                (d) => d.FieldName == 'Id'
            )[0];

            if (item) {
                var index =
                    this.CurrentQueryFilters.AdditionalFilters.indexOf(item);
    ...
    private RunNewEntityWizard(wizardControlName: string) {
        ...
        if (componentPath != null) {
            ...
            if (this.ObjectTableName == 'Currency') {
                str =
                    TextCodeTranslator.Translate('General.B.Add') + ' Currency';
            }
            ...
            if (this.ObjectTableName == 'Customs.CustomsVendor') {
                str = TextCodeTranslator.Translate(
                    'Customs.Vendor.O.SearchVendors'
                );
            }
            ...
            if (this.ObjectTableName == 'BankAccount') {
                str = TextCodeTranslator.Translate(
                    'BankAccount.O.NewBankAccount'
                );
            }

            if (this.ObjectTableName == 'Revaluation') {
                str = TextCodeTranslator.Translate(
                    'Revaluation.O.NewRevaluation'
                );
            }

            if (this.ObjectTableName == 'InterestBasesType') {
                str = TextCodeTranslator.Translate(
                    'Accounting.General.O.NewInterestBases'
                );
            }
            ...
            if (!AppTool.IsNullOrEmpty(this.SelectedQuery.Perspective)) {
                ...
                if (this.ObjectTableName == 'Tariff') {
                    ...
                    switch (QueryCodeOriginal) {
                        ...
                        case 'Tariff.Ocean.LCL.Surcharges.Cost': {
                            logWindow.Title =
                                'New ' +
                                TextCodeTranslator.TranslateTable(
                                    'Tariff.Q.Ocean.LCL.Surcharges.Cost'
                                );
                        ...
                        case 'Tariff.Ocean FCL Surcharges Cost': {
                            logWindow.Title =
                                'New ' +
                                TextCodeTranslator.TranslateTable(
                                    'Tariff.Q.OceanFCLSurchargesCost'
                                );
                        ...
                        case 'Tariff.Import Customs Charges Cost': {
                            logWindow.Title =
                                'New ' +
                                TextCodeTranslator.Translate(
                                    'Tariff.Q.ImportCustomsChargesCost'
                                );
                        ...
                        case 'Tariff.Export Customs Charges Cost': {
                            logWindow.Title =
                                'New ' +
                                TextCodeTranslator.Translate(
                                    'Tariff.Q.ExportCustomsChargesCost'
                                );
                        ...
                        case 'Tariff.Import Local Charges Sale': {
                            logWindow.Title =
                                'New ' +
                                TextCodeTranslator.Translate(
                                    'Tariff.Q.ImportLocalChargesSale'
                                );
                ...
                if (
                    this.ObjectTableName == 'Questionnaire' ||
                    this.ObjectTableName == 'CustomerFieldsUpdateSetting'
                ) {
                    var windowArgs: any = {};
                    windowArgs.IsNew = true;
                    logWindow.WindowArgs = windowArgs;
            ...
            messageWindow.Height = 190;
            messageWindow.Show(
                'Fill NewWizard Component Path and Name in ObjectTable !!'
            );
        }
    }
    ...
    RunNewLogisticActionRequest() {
        ...
        logWindow.Height = 750;
        logWindow.Title = TextCodeTranslator.Translate(
            'Customs.General.O.NewCancelExport'
        );
        logWindow.ShowCloseButton = true;
        logWindow.Show(
            './CustomsModules/CustomsLogisticActionRequest/Components/EditTabs/General/LogisticActionRequestGeneralTabComponent'
    ...
    private RunNewGenaricEntity() {
        var componentPath =
            './Infrastructure/GenericComponents/NewEntityComponent';
        this.entityPMService
            .getNewEntity(this.ObjectTableName)
            .then((response) => {
                ...
                if (this.ObjectTableName == 'InterestBasesType') {
                    FinalText = TextCodeTranslator.Translate(
                        'Accounting.General.O.NewInterestBases'
                    );
                }
    ...
    btnExcelCLicked() {
        if (!this.CheckPermissions(this.ObjectTableName, 'READ', true)) {
            ...
            logitudeWindow.Title = TextCodeTranslator.Translate(
                'General.B.ExportingDataToExcel'
            ); //"Exporting View Data List To Excel File";
            logitudeWindow.WindowArgs = windowArgs;
            logitudeWindow.Show(
                './Infrastructure/Components/Export2ExcelControl/Export2ExcelControl'
            );
            //logitudeWindow.WindowClosed.subscribe(($event: any) => {
    ...
    DoRefresh() {
        ...
        if (
            !this.CheckPermissions(this.ObjectTableName, 'READ', true) &&
            !this.ObjectTable?.IsCustom
        ) {
            return;
        } else {
            var query = window.Queries.filter(
                (q) =>
                    q.ObjectTableId == this.ObjectTable.Id &&
                    q.UniqueCode == this.SelectedQueryCode
            )[0];
            if (query != null) {
                if (
                    window.PreDefinedFilters.filter(
                    ...
                    predefinedFilters.forEach((filter, key) => {
                        var filterOperator = !AppTool.IsNullOrEmpty(
                            filter.Operator
                        )
                            ? filter.Operator
                            : filter.ObjectFieldOperator;
                        ...
                        if (
                            ...
                            var TommorowDate = DateTool.AddDays(new Date(), 1);
                            TommorowDate.setUTCHours(0, 0, 0, 0);
                            var YesterdayDate = DateTool.AddDays(
                                new Date(),
                                -1
                            );
                            YesterdayDate.setUTCHours(0, 0, 0, 0);
                            ...
                            CurrentYearFromDate.setUTCHours(0, 0, 0, 0);
                            var CurrentYearToDate = DateTool.AddDays(
                                new Date(),
                                1
                            );
                            CurrentYearToDate.setUTCHours(0, 0, 0, 0);
                            ...
                            LastYearFromDate.setUTCHours(0, 0, 0, 0);
                            var LastYearToDate = DateTool.AddDays(
                                new Date(),
                                1
                            );
                            LastYearToDate.setUTCHours(0, 0, 0, 0);
            ...
            .subscribe((response: any) => {
                ...
                if (QColumns != null) {
                    ...
                    if (!AppTool.IsNullOrEmpty(QColumns[0].UserId)) {
                        if (this.myQueryColumnsPMService == null) {
                            this.myQueryColumnsPMService =
                                new QueryColumnsPMService();
                            this.myQueryColumnsPMService.setServiceArgs(
                                this.serviceArgs
                            );
                        }

                        QColumns.forEach((querycolumn, key) => {
                            querycolumn.IndexOrder = Param.ColIndexes.filter(
                                (a) =>
                                    a.FieldName == querycolumn.ObjectFieldName
                            )[0].Index;
                            if (
                                Param.ColIndexes.filter(
                                    (a) =>
                                        a.FieldName ==
                                        querycolumn.ObjectFieldName
                                )[0].Width > 0
                            ) {
                                querycolumn.ColumnWidth =
                                    Param.ColIndexes.filter(
                                        (a) =>
                                            a.FieldName ==
                                            querycolumn.ObjectFieldName
                                ...
                                SessionLocator.HomeComponent.SelectedTabItem
                                    .Index &&
                                Param.ColIndexes.filter(
                                    (a) =>
                                        a.FieldName ==
                                ...
                                querycolumn.ColumnWidth =
                                    Param.ColIndexes.filter(
                                        ...
                                        SessionLocator.HomeComponent.SelectedTabItem.Index
                                    ].Width;
                            }
                            this.GeneralEntitiesArgs.QueryColumnsPMs.push(
                                querycolumn
                            );
                        });
                        this.GeneralEntitiesArgs.Tenant =
                            SessionInfo.LoggedUserTenant;
                        var myGeneralService: GeneralEntitiesService =
                            new GeneralEntitiesService();
    ...
    RunNewJournalWizard() {
        ...
        SessionLocator.DynamicLoader.Load(
            ...
            cmpRef.instance.Run({
                EntityPM: entityPM,
                ObjectTableName: 'Journal',
                BackButtonLabel: TextCodeTranslator.Translate(
                    'Accounting.General.O.FullAccounting'
                ),
            });
    ...
    RunNewMasterWizard() {
        var componentPath: string =
            './Shipment/Components/NewEntity/NewMasterComponent';
        var logWindow = new LogitudeWindow();
    ...
    private RunNewCustomObjectWizard() {
        var componentPath: string =
            './Infrastructure/Components/NewEntity/NewCustomObjectComponent';
        var logWindow = new LogitudeWindow();
    ...
    RunNewAccountingIntegrityCheckWizard() {
        var __entity: AccountingIntegrityCheckPM =
            new AccountingIntegrityCheckPM();
        ...
        logWindow.Show(
            './Accounting/Components/NewEntity/NewIntegrityCheckComponent'
        );
    ...
    NewAPPaymentMethod() {
        ...
        SessionLocator.DynamicLoader.Load(
            ...
            cmpRef.instance.ComponentRef = cmpRef;
            cmpRef.instance.Run({
                EntityId: newApPaymentPM.Id,
                EntityPM: newApPaymentPM,
                BackButtonLabel: 'A/P Payments',
    ...
    HasActionBar() {
        //ADD TO LXML\METADATA OBJECTTABLE- to be continue
    ...
    LoadedActionBar(locationCode: string, prefixComponent: string) {
        ...
        let myLocation: LocationDirective = this.AllLocations.toArray().filter(
            (d) => d.Code == locationCode /*"MNH"*/
        )[0];
        if (myLocation != null) {
            let myObjectTableName = this.ObjectTable.Name;
            if (
                ...
                myObjectTableName = myObjectTableName.substr(
                    (this.ObjectTable.ClientModuleName + '.').length
                );
            }
            var myComponentPath =
                './' +
    ...
    MultiPrintClicked() {
        ...
        logitudeWindow.WindowArgs = windowArgs;
        logitudeWindow.Show(
            './Infrastructure/Components/MultiPrint/MultiPrintMainComponent'
        );
        logitudeWindow.WindowClosed.subscribe(($event: any) => {
            this.RefreshBtnClick();
        });
