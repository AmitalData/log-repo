...
import { VirtualRowControllerV2, VirtualRowMetaData } from './VirtualRowControllerV2';
...
@Component({
    ...
    selector: 'logitude-grid-v2',
    templateUrl: './LogGridComponentV2.html',
...
export class LogGridComponentV2 implements OnInit, AfterViewInit, OnChanges, OnDestroy {
    ...
    controller: VirtualRowControllerV2;
    controllerForCount: VirtualRowControllerV2;
    ...
    public BackFromEditAction(res: any) {
        if (this.rows.filter(a => a.rowIndex == res.rowIndex).length > 0) {
            ...
            //this.updateDisplayList();
        } else {///itzik : why its null ??? trying 
            if (this.controller.cachedData.filter(a => a && a.rowIndex == res.rowIndex).length > 0) {
                //this.rows.filter(a => a.rowIndex == res.rowIndex)[0].rowData = res.Data;
                //this.controller.cachedData[res.rowIndex].rowData = res.Data;
                var myRow = this.controller.cachedData[res.rowIndex];
    ...
    ngOnInit() {
       /* if (this.ObjectTable.Name == "Customs.ExportStorge") {  task #139573
            this.RTL = false;
        }*/
    ...
    myReloadData() {
        ...
        //this.controller = new VirtualRowController(this.virtualRowMetaData);
    ...
    onSpotLightSelect(rowIndex) {
        ...
        //var OpenedSpotLight = this.controller.cachedData.filter(a => a && a.ShowDetails == true);
        //if (OpenedSpotLight != null && OpenedSpotLight.length > 0) {
        //    OpenedSpotLight[0].ShowDetails = false;
        //    OpenedSpotLight[0].DetailsIcon = "./Images/SpotLightPlusIcon.png";
        //}
