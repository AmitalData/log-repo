...
export class LoginComponent implements OnInit {
    ...
    public SampleLogoURL: string =
        './Images/ApplicationLogo/Angular/AngularLogo.png';
    ...
    private _objectTableRulePMService: ObjectTableRulePMService =
        new ObjectTableRulePMService();
    private _objectTableRuleFieldPMService: ObjectTableRuleFieldPMService =
        new ObjectTableRuleFieldPMService();
    ...
    constructor(
        ...
        private _applicationTimersManager: ApplicationTimersManager,
    ...
    ngOnInit() {
        ...
        if (
            ...
            this.loginService
                .PostAuthentication(loginParameters)
    ...
    async developerLogin() {
        ...
        this.Email ='shoshana@amital.co.il';
        this.Password = 'Sh254256';
        this.LoginClicked();

        while(!this.TenantList?.length)
            await new Promise<void>(resolve => setTimeout(() => resolve(), 100))

        this.SelectedCompany = this.TenantList.find(d => d.Tenant == 74);
    ...
    StartLoading(userData: any) {
        if (userData) {
            ...
            SessionInfo.WebTokenExpirationWarningInMinutes =
                userData.WebTokenExpirationWarningInMinutes;
            SessionInfo.WebTokenLifeTimeInMinutes =
                userData.WebTokenLifeTimeInMinutes;
            ...
            if (userData.CurrentTenant != null) {
                SessionInfo.LoggedUserTenant = Number(
            ...
            if (SessionInfo.LoggedUserTenant != null) {
                ...
                this.loginService.GetLoggedUser().subscribe((myResult: any) => {
                    SessionLocator.UseMemoryCachedData =
                        myResult.DisableCachedData;

                    var iGlobalDomainService = new GlobalDomainService();

                    iGlobalDomainService
                        .GetTenantManagementJS(SessionInfo.LoggedUserId)
                        .subscribe((myResponse: ServiceResponse) => {
                            ObjectsUpdater.UpdateLoggedUserPM(myResult);
                            ObjectsUpdater.UpdateTenantManagementJS(
                                myResponse.Result
                            );
    ...
    SetCToolAuthorizedUser(userData: any) {
        this.myInfrastructureDomainService
            .GetFeatureToggles()
            .subscribe((featureTogglesResponse: ServiceResponse) => {
                if (!featureTogglesResponse.HasError) {
                    var userTenant = Number(userData.CurrentTenant + '');
                    var ctoolFeatureToggle =
                        featureTogglesResponse.Result.filter(
                            (f: any) =>
                                (f.TenantNumber == userTenant ||
                                    (userTenant >= f.FromTenantNumber &&
                                        userTenant <= f.ToTenantNumber)) &&
                                f.ToggleCode === 'CTL'
                        )[0];
                    if (ctoolFeatureToggle) {
                        ...
                        if (authenticateService) {
                            ...
                            if (
                                authenticateService.hasOwnProperty(
                                    logitudeAuthenticate
                                )
                            ) {
                                authenticateService[logitudeAuthenticate]({
                                    Tenant: userTenant,
                                    ...
                                    if (
                                        ...
                                        CToolSessionInfo.IsUserAuthorized =
                                            true;
                                        CToolSessionInfo.AuthorizedUserToken =
                                            data.Token;
                                        CToolSessionInfo.IsLogitudeAuthentication =
                                            true;
    ...
    SetCToolLogitudeTokens() {
        ...
        if (logitudeTokensService) {
            ...
            if (logitudeTokensService.hasOwnProperty(getTokens)) {
                logitudeTokensService[getTokens]().then(
                    (logitudeTokens: any) => {
                        if (logitudeTokens) {
                            let logitudeTokensJson =
                                JSON.stringify(logitudeTokens);
                            CToolSessionInfo.LogitudeTokensJson =
                                logitudeTokensJson;
                        }
                    }
                );
    ...
    FillProtractorEmails() {
        SessionLocator.ProtractorEmails.push(
            'razantest@protractor.com'.toLowerCase()
        );
        SessionLocator.ProtractorEmails.push(
            'protractor@test.com'.toLowerCase()
        );
        ...
        SessionLocator.ProtractorEmails.push(
            'raghad@protractor.com'.toLowerCase()
        );
        SessionLocator.ProtractorEmails.push(
            'sgautomation@pro.com'.toLowerCase()
        );
        SessionLocator.ProtractorEmails.push('sumaya@cloud.com'.toLowerCase());
        SessionLocator.ProtractorEmails.push(
            'sumaya@automation.com'.toLowerCase()
        );
        ...
        SessionLocator.ProtractorEmails.push(
            'protractor2@test.com'.toLowerCase()
        );
    }
    ...
    OneUsePasswordMethod() {
        this.loginService.GetOneUsePassword().subscribe((userData: any) => {
            if (
                (userData &&
                    (userData.HasError == true || userData.ExceptionMessage)) ||
                !userData
            ) {
                var message =
                    "Can't use this key (" +
                    SessionLocator.ExternalParams.OneTimePasswordId +
                    ') again because you used it before ';
    ...
    VerifyClicked() {
        if (!AppTool.IsNullOrEmpty(this.VerficationCode)) {
            this.HidePendingLoading = false;
            this.loginService
                .PostAuthenticationDeviceVerificationCode(
                    this.LoggedUserData.TwoFactorkey,
                    this.VerficationCode,
                    Number(this.LoggedUserData.CurrentTenant + '')
                )
                .subscribe((res: any) => {
                    this.HidePendingLoading = true;
                    if (res == true) {
    ...
    ResendVerificationCodeClicked() {
        this.HidePendingLoading = false;
        this.loginService
            .PostResendAuthenticationDeviceVerificationCode(
                this.LoggedUserData.TwoFactorkey,
    ...
    LoadClosedTablesToWindow(CurrentTenant: number) {
        this.IndexedDbService.InitializeIndexedDB().subscribe(
            (response: any) => {
                ...
                this.loginService.GetLoggedUser().subscribe((myResult: any) => {
                    ...
                    SessionInfo.LoggedUserPM =
                        myUserPMService.MapJsonToEntityPM(myResult);

                    if (SessionInfo.LoggedUserPM) {
                        this.loginService.LoggedUserId =
                            SessionInfo.LoggedUserPM.Id;
                    ...
                    this.loginService
                        ...
                        .subscribe((myResult: any) => {
                            ...
                            InfraSettings.TenantPM =
                                myTenantPMService.MapJsonToEntityPM(myResult);
                    ...
                    this.loginService
                        ...
                        .subscribe((myResult: any) => {
                            ...
                            ObjectsLocator.UpdateGlobalSetting(
                                this.GlobalSettings
                            );

                            CachedDataManager.CheckSystemMetadataLastUpdate().subscribe(
                                (response: any) => {
                                        ...
                                        .subscribe((response: any) => {
                                            if (
                                                ...
                                                this.entityResourceService
                                                    ...
                                                    .subscribe(
                                                        (response: any) => {
                                                            this.generalTableResourcesIsLoaded =
                                                                true;
                                                            this.IncreaseProgressBar(
                                                                'General Resources'
                                                            );
                        ...
                        .subscribe((myResponse: ServiceResponse) => {
                            ...
                            this.loginService
                                ...
                                .subscribe((Result: any) => {
                                    ObjectsLocator.UpdatePrivateLableSettings(
                                        Result
                                    );
                                    SessionLocator.PrivateLableSettings =
                                        Result;
                                    this.IncreaseProgressBar();
                                    Environment.SetFavIconAndTitle();
                                    //5
                    ...
                    this.myInfrastructureDomainService
                        ...
                        .subscribe((myResponse: ServiceResponse) => {
                            ...
                            if (
                                ...
                                myCreditLimitSettingPMService
                                    .get(CurrentTenant + '')
                                    .subscribe(
                                        (myResponse: ServiceResponse) => {
                                            ObjectsLocator.UpdateCreditLimitSettingPM(
                                                myResponse.Result
                                            );
                                            this.IncreaseProgressBar();
                                            //24
                    ...
                    this.loginService
                        ...
                        .subscribe((myResult: any) => {
                            ...
                            if (myResult) {
                                var myAccountingSettingPMService =
                                    new AccountingSettingPMService();
                                myAccountingSettingPM =
                                    myAccountingSettingPMService.MapJsonToEntityPM(
                                        myResult
                                    );
                            }

                            ObjectsUpdater.UpdateAccountingSettingPM(
                                myAccountingSettingPM
                            );
                            ...
                            if (ObjectsLocator.AccountingSettingPM) {
                                myAccountingSystemCode =
                                    ObjectsLocator.AccountingSettingPM
                            ...
                            this.loginService
                                .GetAccountingSystem(myAccountingSystemCode)
                                .subscribe((myResult2) => {
                                    SessionLocator.AccountingSystemPM =
                                        myResult2;
                                    this.IncreaseProgressBar();
                                    //19
                                });
                    ...
                    this.loginService
                        ...
                        .subscribe((myResult: any) => {
                            if (myResult) {
                                var myCustomsInterfaceSettingPMService =
                                    new CustomsInterfaceSettingPMService();
                                ObjectsUpdater.UpdateCustomsInterfaceSettingPM(
                                    myCustomsInterfaceSettingPMService.MapJsonToEntityPM(
                                        myResult
                                    )
                                );
                            }
                    ...
                    this.loginService
                        ...
                        .subscribe((myResult: any) => {
                            if (myResult) {
                                var mySharedLogisticsSettingPMService =
                                    new SharedLogisticsSettingPMService();
                                ObjectsUpdater.UpdateSharedLogisticsSettingPM(
                                    mySharedLogisticsSettingPMService.MapJsonToEntityPM(
                                        myResult
                                    )
                                );
                            }
        ...
        this._objectTableRulePMService
            .getAllByTenant(CurrentTenant)
            .subscribe((response: any) => {
                if (response) {
        ...
        this._objectTableRuleFieldPMService
            .getAllByTenant(CurrentTenant)
            .subscribe((response: any) => {
                if (response) {
            ...
            .subscribe((response: any) => {
                if (!response.HasError && response.Result) {
                    ...
                    var computerId: string =
                        SessionLocator.GetComputerIdFromStorage();
        ...
        this.sATInterfaceSettingPMService
            .get(CurrentTenant)
            .subscribe((myResult: any) => {
        ...
        this.UserExtendedPMService.CheckUserReleaseNotesToolTip(
    ...
    private GetFeatureToggles() {
        this.myInfrastructureDomainService
            ...
            .subscribe((myResponse: ServiceResponse) => {
                if (!myResponse.HasError) {
                    SessionLocator.TenantZeroFeatureToggles =
                        myResponse.Result.filter((d) => d.TenantNumber == 0);

                    var tenant = InfraSettings.TenantPM.Id;
                    SessionLocator.FeatureToggles = myResponse.Result.filter(
                        (d) =>
                            d.TenantNumber == tenant ||
                            (tenant >= d.FromTenantNumber &&
                                tenant <= d.ToTenantNumber)
                    );
                }
                this.IncreaseProgressBar();
            });
    ...
    private CheckTenantBlocking(userData: any) {
        ...
        if (SessionLocator.TenantManagementJS.PaymentFailure) {
            if (
                AppTool.IsNullOrEmpty(
                    SessionLocator.TenantManagementJS.SuspendDate
                )
            ) {
        ...
        if (!isSystemBlocked) {
            if (SessionLocator.TenantManagementJS.IsTrial) {
                if (
                    AppTool.IsNullOrEmpty(
                        SessionLocator.TenantManagementJS.TrialEndDate
                    )
                ) {
                    isSystemBlocked = true;
                    SessionLocator.BlockType = 'company';
        ...
        if (!isSystemBlocked) {
            if (
                !AppTool.IsNullOrEmpty(
                    SessionLocator.TenantManagementJS.PaidUntilDate
                )
            ) {
                if (
                    DateTool.GetDateParts(
                    ...
                    !SessionLocator.TenantManagementJS.IsRecurring
                ) {
                    isSystemBlocked = true;
                    SessionLocator.BlockType = 'company';
                }
    ...
    IncreaseProgressBar(loadOPName: string = '') {
        ...
        if (this.TotalNumberOfLoads == 0) {
            ...
            if (this.LoadSize.toString().indexOf('.') > -1) {
                ...
                this.LastLoadSize =
                    100 - (this.TotalNumberOfLoads - 1) * this.LoadSize;
            } else {
        ...
        if (this.CompletedLoadsCount <= this.TotalNumberOfLoads) {
            ...
            if (
                this.CompletedLoadsCount === this.TotalNumberOfLoads &&
    ...
    private SetLayoutDirection() {
        if (InfraSettings.TenantPM) {
            this.GlobalSettings.LayoutDirection = SessionInfo.LoggedUserPM
                .LayoutDirection
