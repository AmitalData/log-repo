...
export class ExportBI2ExcelControl {
    ...
    SaveExcelFile(tenant: number, FileName: string, OTName: string) {
        ...
        var url = ServiceHelper.GetLogitudeURL() + "WebPages/DawnLoadExcelPage.aspx?fileName=" + FileName + "&tempId=" + ServiceHelper.GetLDocumentDownloadToken() + "&qname=" + this.queryName + "_" + MyDate ; //+ "&bireport=" + "bireport";
        {
            window.open(url);
        }
