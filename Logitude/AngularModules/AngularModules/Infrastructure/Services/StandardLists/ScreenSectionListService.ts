...
import {ScreenSectionList} from '../../EntityLists/ScreenSectionList';

@Injectable()

export class ScreenSectionListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<ScreenSectionList> = [];
    constructor() {
			...
			return this._http.get(this._apiUrl + '/getall', ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						var _mappedListsArray: Array<ScreenSectionList> = [];
						if (allLists) {
							for (var key in allLists) {				
								var entity: ScreenSectionList = this.MapJsonToEntityList(allLists[key]);
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "ScreenSection", "GetAllLists", ""); 
	...
	getByFilters(filters: ApiQueryFilters) {
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: ScreenSectionList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "ScreenSection", "GetByFilters", "PageIndex:" +filters.PageIndex +", PageSize:"+filters.PageSize + ", GetAll:" + filters.GetAll);
	    ...
	    MapJsonToEntityList(jsonList: any) {
            ...
            var entityList: ScreenSectionList;
            entityList = new ScreenSectionList();
