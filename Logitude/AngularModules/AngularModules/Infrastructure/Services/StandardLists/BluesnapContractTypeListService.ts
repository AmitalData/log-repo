					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {			
								var entity: BluesnapContractTypeList = this.MapJsonToEntityList(allLists[key]);
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: BluesnapContractTypeList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
				...
				map(cachedJson => {
					...
					if (cachedJson) {
						for (var key in cachedJson) {
							var entity: BluesnapContractTypeList = this.MapJsonToEntityList(cachedJson[key]);
							_mappedListsArray.push(entity);
						}
					}
