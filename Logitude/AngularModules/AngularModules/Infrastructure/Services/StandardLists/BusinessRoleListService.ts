...
export class BusinessRoleListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<BusinessRoleList> = [];
    constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/businessroleviews';  
    }
			...
			return this._http.get(this._apiUrl + '/getsingle/?' + 'id=' + id, ServiceHelper.GetHttpFullHeaders())
				.pipe(			
					map((response: HttpResponse<any>) => {
						...
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "BusinessRole", "GetSingleList", 'id=' + id); 
					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {				
								var entity: BusinessRoleList = this.MapJsonToEntityList(allLists[key]);
					...
					map((response: HttpResponse<any>) => {
						...
						if (serviceResponse.Result) {
							for (var key in serviceResponse.Result) {				
								var entity: BusinessRoleList = this.MapJsonToEntityList(serviceResponse.Result[key]);
								_mappedListsArray.push(entity);
							}
						}   
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "BusinessRole", "GetByFilters", "PageIndex:" +filters.PageIndex +", PageSize:"+filters.PageSize + ", GetAll:" + filters.GetAll);
	...
	getSingleFromCache(id: string) {
		...
		var exists = BusinessRoleListService.CachedData.filter(a => a.Id === id).length;

        var serviceResponse: ServiceResponse = new ServiceResponse(); 

        if (exists === 0) {
			return defer(() => {
				var cacheKey = "BusinessRole_CachedData_" + SessionLocator.Tenant;
				var _mappedListsArray: Array<BusinessRoleList> = [];
                ...
                if (cachedString) {
                    ...
                    for (var key in cachedJson) {
                        var entity: BusinessRoleList = this.MapJsonToEntityList(cachedJson[key]);
                        _mappedListsArray.push(entity);
                    }

                    BusinessRoleListService.CachedData = _mappedListsArray;
                    serviceResponse = new ServiceResponse();
                    
                    var filteredData = BusinessRoleListService.CachedData.filter(a => a.Id === id)[0];
                    serviceResponse.Result = filteredData;
					serviceResponse.CallTime = callTime;
 
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "BusinessRole", "GetSingleListFromCache", 'id=' + id); 
    ...
    getAllFromCache(filters: ApiQueryFilters = new ApiQueryFilters(true)) {
		...
		var exists = BusinessRoleListService.CachedData.length;
		var urlparameters = '/getbyfilters?';
        var mykeys = Object.keys(filters);
        var addtionalFiltersValues = null;
        ...
        if (exists === 0 || filters.ForceCacheRefresh) {
            ...
            if (!filters.ForceCacheRefresh) {
                ...
                if (cachedString) {
                    ...
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "BusinessRole", "GetAllFromCache", "");                     
                }
            }
                    ...
                    return this._http.get(callUrl, ServiceHelper.GetHttpFullHeaders())
						.pipe(
							map((response: HttpResponse<any>) => {
								...
								var servertime = response.headers.get('ServerExecutionTime');
								PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "BusinessRole", "GetAll", ""); 
            ...
            var filteredData = BusinessRoleListService.CachedData;
            if (!filters.GetAll) {
