...
export class RatesTableListService {
	private _http: Http;
    ...
    getSingle(id: string) {
        ...
        return Observable.defer(() => {
            return this._http.get(this._apiUrl+'/getsingle/?'+'id=' + id, { headers: authHeader }).map(response => {
                ...
                PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "RatesTable", "GetSingleList", 'id=' + id); 
    ...
    getAll() {
       ...
       return Observable.defer(() => {
            return this._http.get(this._apiUrl+'/getall', { headers: authHeader }).map(response => {
              ...
              var _mappedListsArray: Array< RatesTableList> = [];
		      if(allLists)
			  {
				...
				serviceResponse.CallTime = callTime;
                var servertime = response.headers.get('ServerExecutionTime');
                PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "RatesTable", "GetAllLists", ""); 
    ...
    getByFilters(filters: ApiQueryFilters) {
	   ...
	   return Observable.defer(() => {
            return this._http.get(callUrl, {
				...
				serviceResponse.CallTime = callTime;
                var servertime = response.headers.get('ServerExecutionTime');
                PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "RatesTable", "GetByFilters", "PageIndex:" +filters.PageIndex +", PageSize:"+filters.PageSize + ", GetAll:" + filters.GetAll);
    ...
    getSingleFromCache(id: string) {
        ...
        var authHeader = new Headers();
        authHeader.append('Token', SessionInfo.Token);
		var exists = RatesTableListService.CachedData.filter(a => a.Id === id).length;

        var serviceResponse: ServiceResponse;
        serviceResponse = new ServiceResponse(); 
        if (exists === 0) {
        return Observable.defer(() => {
            var cacheKey = "RatesTable_CachedData_" + SessionLocator.Tenant;
            var _mappedListsArray: Array<RatesTableList> = [];
                ...
                if (cachedString) {
                    ...
                    RatesTableListService.CachedData = _mappedListsArray;
                    serviceResponse = new ServiceResponse();
                    
                    var filteredData = RatesTableListService.CachedData.filter(a => a.Id === id)[0];
                    serviceResponse.Result = filteredData;
					serviceResponse.CallTime = callTime;
 
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "RatesTable", "GetSingleListFromCache", 'id=' + id); 
  ...
  var exists = RatesTableListService.CachedData.length;
   var urlparameters = '/getbyfilters?';
        var mykeys = Object.keys(filters);
        var addtionalFiltersValues = null;
        for (var i in mykeys) {
        ...
        if (exists === 0 || filters.ForceCacheRefresh) {
            var cacheKey = "RatesTable_CachedData_" + filters.Tenant;
            var _mappedListsArray: Array<RatesTableList> = [];
            var serviceResponse: ServiceResponse;
            ...
            if (!filters.ForceCacheRefresh) {
                ...
                if (cachedString) {
                    ...
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "RatesTable", "GetAllFromCache", ""); 
            ...
            if (serviceResponse) {
                ...
                return Observable.defer(() => {
                    ...
                    var servertime = response.headers.get('ServerExecutionTime');
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "RatesTable", "GetAll", ""); 
            ...
            var filteredData = RatesTableListService.CachedData;
            if (!filters.GetAll) {
