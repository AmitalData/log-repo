...
export class DWObjectFieldPMService {
 ...
 constructor() {
	...
	insert(entityPM: DWObjectFieldPM) {
		...
		return Observable.defer(() => {
			...
			if (errorsArray.length == 0) {
				...
				var mappedEntity: DWObjectFieldPM = this.MapJsonToEntityPM(entityPM, false);
				...
				return this._http.post(this._apiUrl, JSON.stringify(mappedEntity), ServiceHelper.GetHttpFullHeaders())
					.pipe(
						map((response: HttpResponse<any>) => {
							...
							var pm = response.body;
							if (pm) {
								var mappedResult: DWObjectFieldPM = this.MapJsonToEntityPM(pm, true, entityPM);
								serviceResponse.Result = mappedResult;
							}						

							var servertime = response.headers.get('ServerExecutionTime');
							PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "DWObjectField", "SaveChanges", "");                    
	...
	update(entityPM: DWObjectFieldPM) {
