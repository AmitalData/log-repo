...
export class WebhookKeysPMService {
 private _http: HttpClient;
 private _apiUrl: string;
 constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/webhookkeys';      
    }
	...
	insert(entityPM: WebhookKeysPM) {
		...
		return Observable.defer(() => {
			...
			if (errorsArray.length == 0) {
				...
				var mappedEntity: WebhookKeysPM = this.MapJsonToEntityPM(entityPM, false);
				...
				return this._http.post(this._apiUrl, JSON.stringify(mappedEntity), ServiceHelper.GetHttpFullHeaders())
					.pipe(
						map((response: HttpResponse<any>) => {
							...
							var pm = response.body;
							if (pm) {
								var mappedResult: WebhookKeysPM = this.MapJsonToEntityPM(pm, true, entityPM);
