...
export class RatesTablePMService {
 private _http: HttpClient;
 private _apiUrl: string;
 constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/ratestables';      
    }
			...
			return this._http.get(this._apiUrl + '/getsingle?' + 'id=' + id, ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "RatesTable", "GetSinglePM", 'id=' + id);
				...
				return this._http.post(this._apiUrl, JSON.stringify(mappedEntity), ServiceHelper.GetHttpFullHeaders())
					.pipe(
						map((response: HttpResponse<any>) => {
							...
							var pm = response.body;
							if (pm) {
								var mappedResult: RatesTablePM = this.MapJsonToEntityPM(pm, true, entityPM);
	...
	update(entityPM: RatesTablePM) {
	  ...
	  MapJsonToEntityPM(jsonPM: any, mapParent: boolean = true, entityPM: RatesTablePM = null) {

         
        if (!entityPM) {
            
            entityPM = new RatesTablePM();
        }
