...
export class IATACodePMService {
 private _http: HttpClient;
 private _apiUrl: string;
 constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/iatacodes';      
    }
	...
	insert(entityPM: IATACodePM) {
		...
		return Observable.defer(() => {
			...
			if (errorsArray.length == 0) {
				...
				var mappedEntity: IATACodePM = this.MapJsonToEntityPM(entityPM, false);
				...
				return this._http.post(this._apiUrl, JSON.stringify(mappedEntity), ServiceHelper.GetHttpFullHeaders())
					.pipe(
						map((response: HttpResponse<any>) => {
							...
							var pm = response.body;
							if (pm) {
								var mappedResult: IATACodePM = this.MapJsonToEntityPM(pm, true, entityPM);
