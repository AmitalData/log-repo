...
@Component({
    templateUrl: './LocalChargesVersionTabComponent.html',
})

export class LocalChargesVersionTabComponent extends BaseComponent implements OnDestroy {
    ...
    private ItemsCollection: LocalChargesTariffLineData[] = [];
    public DeletedTariffsLines: LocalChargesTariffLineData[] = [];
    FillTariffLines(tariffLines: TariffLinePM[]) {
        ...
        tariffLines.sort((a, b) => a.Index - b.Index).forEach(item => {
            var localChargesTariffLineData = new LocalChargesTariffLineData(item, this)
            count++;
            this.ItemsCollection.push(localChargesTariffLineData);
            if (!AppTool.IsNullOrEmpty(this.LineIdFromPriceCheck) && localChargesTariffLineData.EntityPM.Id == this.LineIdFromPriceCheck) {
                this.selectedRow = localChargesTariffLineData;
    ...
    ComaredLines() {
        this.ItemsCollection.forEach((item: LocalChargesTariffLineData) => {
    ...
    BuildDeletedLines() {
        ...
        this.compareTariffLines.sort((a, b) => a.Index - b.Index).forEach(item => {
            ...
            if (line == null) {
                this.DeletedTariffsLines.push(new LocalChargesTariffLineData(item, this));
            }
        });
    }
    ...
    AddTariffLine() {
        ...
        var itemComponent = new LocalChargesTariffLineData(itemPM, this, true);
    ...
    EditTariffButtonClicked(item: LocalChargesTariffLineData) {
        var logWindow = new LogitudeWindow();
    ...
    DeleteTariffButtonClicked(item: LocalChargesTariffLineData) {
    ...
    DeleteLinesClicked() {
        if (this.ItemsCollection.filter(f => f.IsLineSelected).length == 0) {
            ...
            confirmWindow.WindowClosed.subscribe((event: any) => {
                if (confirmWindow.Yes) {
                    this.ItemsCollection.filter(d => d.IsLineSelected).forEach((item: LocalChargesTariffLineData) => {
                        this.CurrentVersion.RemoveTariffLine(item.EntityPM);
                    });
...
export class LocalChargesTariffLineData extends BaseComponent {
    public EntityPM: TariffLinePM;
    public DataContext: LocalChargesTariffLineData = this;
    ...
    constructor(entity: TariffLinePM, public FatherComponent: LocalChargesVersionTabComponent, isNew: boolean = false) {
        super(); 
