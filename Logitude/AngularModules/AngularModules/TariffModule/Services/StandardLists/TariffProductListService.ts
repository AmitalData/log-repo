...
export class TariffProductListService {
	private _http: HttpClient;
    private _apiUrl: string;   
	public static CachedData: Array<TariffProductList> = [];
    constructor() {
        this._http = ServiceHelper.HttpClient;
        this._apiUrl = ServiceHelper.GetLogitudeURL() + 'api/tariffproductviews';  
    }
			...
			return this._http.get(this._apiUrl + '/getsingle/?' + 'id=' + id, ServiceHelper.GetHttpFullHeaders())
				.pipe(			
					map((response: HttpResponse<any>) => {
						...
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "TariffProduct", "GetSingleList", 'id=' + id); 
					...
					map((response: HttpResponse<any>) => {
						...
						if (allLists) {
							for (var key in allLists) {				
								var entity: TariffProductList = this.MapJsonToEntityList(allLists[key]);
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "TariffProduct", "GetAllLists", ""); 
			...
			return this._http.get(callUrl, ServiceHelper.GetHttpFullHeaders())
				.pipe(
					map((response: HttpResponse<any>) => {
						...
						var servertime = response.headers.get('ServerExecutionTime');
						PerformanceLogger.InsertPerformanceLog(callTime, new Date(), Number(servertime), "TariffProduct", "GetByFilters", "PageIndex:" +filters.PageIndex +", PageSize:"+filters.PageSize + ", GetAll:" + filters.GetAll);
	...
	getSingleFromCache(id: string) {
		...
		var exists = TariffProductListService.CachedData.filter(a => a.Id === id).length;

        var serviceResponse: ServiceResponse = new ServiceResponse(); 

        if (exists === 0) {
			return defer(() => {
				var cacheKey = "TariffProduct_CachedData_" + SessionLocator.Tenant;
				var _mappedListsArray: Array<TariffProductList> = [];
                ...
                if (cachedString) {
                    ...
                    for (var key in cachedJson) {
                        var entity: TariffProductList = this.MapJsonToEntityList(cachedJson[key]);
                        _mappedListsArray.push(entity);
                    }
                    ...
                    var filteredData = TariffProductListService.CachedData.filter(a => a.Id === id)[0];
                    serviceResponse.Result = filteredData;
					serviceResponse.CallTime = callTime;
 
                    PerformanceLogger.InsertPerformanceLog(callTime, new Date(), 0, "TariffProduct", "GetSingleListFromCache", 'id=' + id); 
    ...
    getAllFromCache(filters: ApiQueryFilters = new ApiQueryFilters(true)) {
	    ...
	    MapJsonToEntityList(jsonList: any) {
            ...
            var entityList: TariffProductList;
            entityList = new TariffProductList();
