using Logitude.AmitalMessaging.Customs.CustomFile.CourierFile;
...
{
    public class CourierService : UnifreightGenericService
    {
        private LOGICOURIER _LOGICOURIER;
        private LogitudeCourierFile _LogitudeCourierFile;
        private Def.EntityPMs.SupplierInvoicePM _MySupplierInvoicePM;
        private ICustomContext _context;

        private AmitalContext amitalContext;

        public const string UpsertActionConst = "Logitude.Customs.BL.Messaging.U2L.Courier.CourierService.Upsert()";
        private Logitude.AmitalMessaging.Customs.CustomFile.CourierFile.INVOICE _INVOICE;
        ...
        public override void ProccessGenericRequest(
              string xmlLOGICOURIER,
              ref string MoreParams,
              out string MessageOut)
        {
            MessageOut = "";
            _Stopwatch = Stopwatch.StartNew();
            MyCommunicationsParams.Subject = "CourierService ";

            DeserilazeObject(xmlLOGICOURIER);
            ...
            string xmlLOGICUSTFILE = xmlLOGICOURIER;
            xmlLOGICUSTFILE = xmlLOGICUSTFILE.Replace("LOGICOURIER", "LOGICUSTFILE");
            xmlLOGICUSTFILE = xmlLOGICUSTFILE.Replace("LogitudeCourierFile", "LogitudeCustomsFile");
            ...
            if (String.IsNullOrWhiteSpace(_LogitudeCourierFile.Id))
            {
                string existId = myQueryService.GetIdByCustomFileNo(_LogitudeCourierFile.CustomFileNo, ResolvedTenant());
                _LogitudeCourierFile.Id = existId;
            }

            MyGenericResponseObj.Stage = "GetSingle"; 
            this._MyDeclarationPM = myQueryService.GetSingle(this._LogitudeCourierFile.Id, true, false);
            if (this._MyDeclarationPM == null)
            {
                throw new BusinessErrorException("LOGITUDE FILE is " + this._LogitudeCourierFile.Id + " but not found");
            }
            ...
            this._MyDeclarationPM.TaxationDateTime = AmitalConvertUtil.GetUnifreightFormatedDate(_LogitudeCourierFile.TaxationDateTime, "LogitudeCourierFile.TaxationDateTime"); // moran 22.1.17 - AMI-58777
            
            if (!string.IsNullOrWhiteSpace(this._LogitudeCourierFile.TransferImporterId)) // moran 22.3.17 - AMI-59830
            {
                if (_LogitudeCourierFile.TransferImporterId.Length > 9)
                {
                    this._MyDeclarationPM.EntitleImporterId = TranslateClient(_LogitudeCourierFile.TransferImporterId.Substring(0, 9));
                    this._MyDeclarationPM.EntitleImporterCode = _LogitudeCourierFile.TransferImporterId.Substring(0, 9);
                }
                else
                {
                    this._MyDeclarationPM.EntitleImporterId = TranslateClient(_LogitudeCourierFile.TransferImporterId);
                    this._MyDeclarationPM.EntitleImporterCode = _LogitudeCourierFile.TransferImporterId;
                }
                if (!string.IsNullOrWhiteSpace(this._LogitudeCourierFile.ImporterEntitlementTypeCode))
                {
                    EntitlementTypeQueryService entitlementTypeQueryService = new EntitlementTypeQueryService(this._MyDeclarationPM.Tenant);
                    EntitlementTypePM entitlementType = entitlementTypeQueryService.GetSingle(this._LogitudeCourierFile.ImporterEntitlementTypeCode, false, false);
            ...
            if (this._MyDeclarationPM.Consignments.Count == 1) // moran 11.1.17 - AMI-59265 - moved before creating invoices
            {
                ...
                if (!String.IsNullOrWhiteSpace(_LogitudeCourierFile.ManifestNumber))
                {
                    this._MyDeclarationPM.Consignments[0].ManifestNumber = _LogitudeCourierFile.ManifestNumber;
                }
                
                string loadingPortCode = null;
                if (!String.IsNullOrWhiteSpace(_LogitudeCourierFile.LoadingPortCode))
                {
                    loadingPortCode = TranslateloadPort(_LogitudeCourierFile.LoadingPortCode);
                }
                this._MyDeclarationPM.Consignments[0].LoadingPortCode = loadingPortCode;

                if (!String.IsNullOrWhiteSpace(_LogitudeCourierFile.OriginCountryCode))
                {
                    string countryCode = "";
                    if (_LogitudeCourierFile.OriginCountryCode.Length > 2)
                    {
                        countryCode = GetTranslationL2P("IIGC", "CTBCOUNTRY", _LogitudeCourierFile.OriginCountryCode);
                    }
                    else
                    {
                        countryCode = _LogitudeCourierFile.OriginCountryCode;
                    }
                ...
                this._MyDeclarationPM.Consignments[0].CargoDescription = _LogitudeCourierFile.CargoDescription;
                //this._MyDeclarationPM.Consignments[0].ManifestDate = AmitalConvertUtil.GetUnifreightFormatedDate(_LogitudeCourierFile.ManifestDate, "LogitudeCourierFile.ManifestDate");
                this._MyDeclarationPM.Consignments[0].UnloadDate = AmitalConvertUtil.GetUnifreightFormatedDate(_LogitudeCourierFile.ArrivalDateTime, "LogitudeCourierFile.ArrivalDateTime");

                if (!String.IsNullOrWhiteSpace(_LogitudeCourierFile.OriginCountryId))
                {
                    string countryCode = "";
                    if (_LogitudeCourierFile.OriginCountryId.Length > 2)
                    {
                        countryCode = GetTranslationL2P("IIGC", "CTBCOUNTRY", _LogitudeCourierFile.OriginCountryId);
                    }
                    else
                    {
                        countryCode = _LogitudeCourierFile.OriginCountryId;
                    }
                    if (!string.IsNullOrWhiteSpace(countryCode)) this._MyDeclarationPM.Consignments[0].OriginCountryCode = countryCode;
                }
                this._MyDeclarationPM.Consignments[0].StorageSiteCode = TranslateDeliverySite(_LogitudeCourierFile.WarehouseId);


                if (this._LogitudeCourierFile.PACKAGES != null && this._LogitudeCourierFile.PACKAGES.Count() > 0)
                {
                    ...
                    this._MyDeclarationPM.Consignments[0].ConsignmentPackages = GetConsignmentPackages(this._LogitudeCourierFile.PACKAGES);
                }
            }
            else
            {
            ...
            if (this._LogitudeCourierFile.INVOICE != null)
            {
                if (this._LogitudeCourierFile.INVOICE.Count() > 0)
                {
                    this._MyDeclarationPM.SupplierInvoices = new List<SupplierInvoicePM>();
                    foreach (var itemINVOICE in this._LogitudeCourierFile.INVOICE)
                    {
            ...
            if (this._LogitudeCourierFile.CustomsDocuments != null && this._LogitudeCourierFile.CustomsDocuments.Where(d => d.Blocked != "1").Count() > 0) // moran 2.6.16 - AMI-56624
            {
                ...
                foreach (var customsDocument in this._LogitudeCourierFile.CustomsDocuments)
                {
        ...
        void DeserilazeObject(string xmlLOGICOURIER)
        {

            MyGenericResponseObj.Stage = "Initalize ProccessRequest";
            AppendLogLine("CourierService.ProccessRequest");

            AppendLogLine("Deserialize(DataIn1) ..");

            if (string.IsNullOrWhiteSpace(xmlLOGICOURIER))
            {
                throw new BusinessErrorException("DataIn1 is missing");
            }
            if (xmlLOGICOURIER.Length > 1000)
            {
                AppendLogLine("XmlIn=" + xmlLOGICOURIER.Substring(0, 1000));
                AppendLogLine(".Substring(0, 1000)");
            }
            else
            {
                AppendLogLine("XmlIn=" + xmlLOGICOURIER);
            }

            AppendLogLine("Tring DeserilazeObject");
            MyGenericResponseObj.Stage = "Trying DeserilazeObject";
            this._LOGICOURIER = XmlGenericUtil<LOGICOURIER>.DeSerializeObject(xmlLOGICOURIER);

            if (_LOGICOURIER.LogitudeCourierFile == null || _LOGICOURIER.LogitudeCourierFile.Length != 1)
            {
                throw new BusinessErrorException("_LOGICOURIER.Courier.Length != 1");
            }
            this._LogitudeCourierFile = _LOGICOURIER.LogitudeCourierFile[0];
        }
        ...
        private void CheckIntegrity()
        {
            ...
            if (String.IsNullOrWhiteSpace(this._LogitudeCourierFile.Id))
            {
                throw new BusinessErrorException("LOGITUDEFILE is missing");
            }
            AppendLogLine("LOGITUDE FILE = " + this._LogitudeCourierFile.Id);
        }
        ...
        {
            var xml = "";
            var amitalObjExample = new LOGICOURIER();
            var myAmitalCourier = new LogitudeCourierFile();
            var myAmitalCourierInvoice = new List<Logitude.AmitalMessaging.Customs.CustomFile.CourierFile.INVOICE>();
            var myAmitalCourierInvoiceItem = new List<Logitude.AmitalMessaging.Customs.CustomFile.CourierFile.INVOICEITEMS>();

            myAmitalCourier.Id = "1-1";
            myAmitalCourierInvoice[1].INVOICELINENO = "1";
            myAmitalCourierInvoice[1].INVOICELINENO = "1";
            myAmitalCourierInvoice[1].ACCOUNTTYPE = "380";
            myAmitalCourierInvoice[1].INVOICENUMBER = "999";
            myAmitalCourierInvoice[1].VENDORNUMBER = "2000475";
            myAmitalCourierInvoice[1].CURRENCYCODE = "18";
            myAmitalCourierInvoice[1].INVOICEAMOUNT = "2";
            myAmitalCourier.INVOICE = myAmitalCourierInvoice.ToArray();

            myAmitalCourierInvoiceItem[1].CLASSIFICATIONCODE = "260300009";
            myAmitalCourierInvoiceItem[1].TRADEAGREEMENTCODE = "BGR";
            myAmitalCourierInvoiceItem[1].QUANTITY = "1";
            myAmitalCourierInvoiceItem[1].ITEMPRICE = "1";
            myAmitalCourierInvoiceItem[1].ITEMORIGINCOUNTRY = "AD";
            myAmitalCourierInvoiceItem[1].ITEMCODE = "DFDF";
            myAmitalCourier.INVOICE[1].INVOICEITEMS = myAmitalCourierInvoiceItem.ToArray();

            amitalObjExample.LogitudeCourierFile = new LogitudeCourierFile[] { myAmitalCourier };

            xml = XmlGenericUtil<LOGICOURIER>.SerializeObject(amitalObjExample);
        ...
        private List<SupplierInvoiceModificationPM> GetSupplierInvoiceModificationsPM(INVOICE iNVOICE) // moran 26.5.16 - AMI-56711
        {
            ...
            if (this._MyDeclarationPM.SupplierInvoices.Count() < 1)
            {
                ...
                if (!string.IsNullOrWhiteSpace(this._LOGICOURIER.LogitudeCourierFile[0].agent_fee) && this._LOGICOURIER.LogitudeCourierFile[0].agent_fee != "0")
                {
                    if (decimal.TryParse(this._LOGICOURIER.LogitudeCourierFile[0].agent_fee, out decimal1))
                    {
                        SupplierInvoiceModificationPM.Amount = decimal1;
                    }
                    else
                    {
                        throw new BusinessErrorException("Error in parsing agent_fee (" + this._LOGICOURIER.LogitudeCourierFile[0].agent_fee + ") into integer");
                    }
                    if (!String.IsNullOrWhiteSpace(this._LOGICOURIER.LogitudeCourierFile[0].agent_fee_currency))
                    {


                        var agentFeeCurrency = new CurrencyTypeRepository(ResolvedTenant());
                        var myagentFeeCurrency = agentFeeCurrency.GetSingle(this._LOGICOURIER.LogitudeCourierFile[0].agent_fee_currency);
        ...
        private List<SupplierInvoiceItemPM> GetSupplierInvoiceItemPM(Logitude.AmitalMessaging.Customs.CustomFile.CourierFile.INVOICE invoice)
        {
        ...
        private List<SupplierInvoiceItemsConDeclarPM> GetSupplierInvoiceItemConDeclarsPM(AmitalMessaging.Customs.CustomFile.CourierFile.INVOICEITEMS invoiceItem, SupplierInvoiceItemPM supplierInvoiceItemPM)
        {
            ...
            foreach (var invoiceItemConDeclar in invoiceItem.POINTERS)
            {
                ...
                if (!String.IsNullOrWhiteSpace(_LogitudeCourierFile.ManifestNumber)) // moran 17.7.16 - AMI-57487
                {
                    SupplierInvoiceItemConDeclarPM.DeclarationNumber = _LogitudeCourierFile.ManifestNumber;
                }
                else
                {
                    if (invoiceItemConDeclar.DECLARATIONID != null)
                    {
                        ...
                        var myQueryService = new DeclarationQueryService(_context);
                        EntryDeclarationPM = myQueryService.GetSingle(this._LogitudeCourierFile.EntryFile, true, false);
        ...
        private List<SupplierInvioceItemCertificatPM> GetSupplierInvoiceItemCertificatePM(AmitalMessaging.Customs.CustomFile.CourierFile.INVOICEITEMS invoiceItem, SupplierInvoiceItemPM supplierInvoiceItemPM)
        {
        ...
        private List<SupplierInvoiceItemVehiclePM> GetSupplierInvoiceItemVehiclesPM(AmitalMessaging.Customs.CustomFile.CourierFile.INVOICEITEMS invoiceItem, SupplierInvoiceItemPM supplierInvoiceItemPM)
