        ...
        public void UpdateCertificateConnectedItems(string[] items, string declarationId, string invoiceNumber, string attachmentTypeCode, string certificateNumber, string resConfirmationTypeCode, string certificateExemptionTypeCode, string reqConfirmationTypeCode, int tenant)
        {
            ...
            if (dbms == "oracle")
            {
                using (OracleConnection cn = new OracleConnection(strConnString))
                {
                    ...
                    var stringBuilder = new StringBuilder();
                    stringBuilder
                        .AppendLine($"UpdateCertificateConnectedItems:357:(declarationId:{declarationId},certificateExemptionTypeCode:{certificateExemptionTypeCode})")
                        .AppendLine($"CertificateExemptionTypeCode:cmd.Contains(92)={cmd.Contains("92")}")
                        .AppendLine(cmd);
                ...
                if (nonValidItems.Count > 0)
                {
                    ...
                    if (dbms == "oracle")
                    {
                        using (OracleConnection cn = new OracleConnection(strConnString))
                        {
                            ...
                            var stringBuilder = new StringBuilder();
                            stringBuilder
                                .AppendLine($"UpdateCertificateConnectedItems:495:(declarationId:{declarationId},certificateExemptionTypeCode:{certificateExemptionTypeCode})")
                                .AppendLine($"CertificateExemptionTypeCode:updateCmd.Contains(92)={updateCmd.Contains("92")}")
                ...
                if (validItems.Count > 0)
                {
                    if (dbms == "oracle")
                    {
                        using (OracleConnection cn = new OracleConnection(strConnString))
                        {
                            ...
                            var stringBuilder = new StringBuilder();
                            stringBuilder
                                .AppendLine($"UpdateCertificateConnectedItems:564:(declarationId:{declarationId},certificateExemptionTypeCode:{certificateExemptionTypeCode})")
                                .AppendLine($"CertificateExemptionTypeCode:updateCmd.Contains(92)={updateCmd.Contains("92")}")
                                .AppendLine(updateCmd);
        ...
        public int UpdateAllCertificateWithoutResponse(string declarationId, int tenant)
        {
            ...
            if (dbms == "oracle")
            {
                using (OracleConnection con = new OracleConnection(strConnString))
                {
                    ...
                    var stringBuilder = new StringBuilder();
                    stringBuilder
                        .AppendLine($"UpdateAllCertificateWithoutResponse({declarationId})")
                        .AppendLine($"CertificateExemptionTypeCode")
                        .AppendLine($"count({count})=sqlCommand({cmd})")
                        .AppendLine($"sqlCommand1({cmd1})");
                    var logChangesService = new LogChangesService();
                    logChangesService.LogIt(
                        appSettingKeyValueIsLogUntilDateyyyyMMdd: SIICerExemptionHD379305,
                        stringBuilder);
