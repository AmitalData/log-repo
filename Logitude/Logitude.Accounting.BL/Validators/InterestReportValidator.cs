...
namespace Logitude.Accounting.BL.Validators
{
    public partial class InterestReportValidator
    {
        ...
        private static void ValidateIfPreviousInvoicedOrCloseWithoutInvoicedtInterestReportForCustomer(InterestReportPM entityPM, bool showLocals)
        {
            ...
            {
                throw new Exception(TextCodesTranslator.TranslateText("InterestReport.O.CannotCreateReportWithCalculationDateLess", entityPM.Tenant, showLocals) + " " + entityPM.ReportNumber + " " + TextCodesTranslator.TranslateText("InterestReport.O.on", entityPM.Tenant, showLocals) + " " + entityPM.InterestCalculationDate.ToString("dd/MM/yyyy") + "].");
            }
