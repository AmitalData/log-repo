    ...
    public class TaxReportService
    {
        ...
        public static void CalculateReportTotals(TaxReportPM taxReportPM, List<TaxReportLinePM> lines)
        {
            if (lines.Count > 0)
            {
                ...
                // OUTPUT
                taxReportPM.TaxableOutputAmount = outputLines.Where(d => d.TransmitStatusCode == TaxReportLineTransmitStatusValues.Fortransmit || d.TransmitStatusCode == TaxReportLineTransmitStatusValues.TransmitevenifDuplicate).Sum(d => d.VatableInvoiceAmount);
