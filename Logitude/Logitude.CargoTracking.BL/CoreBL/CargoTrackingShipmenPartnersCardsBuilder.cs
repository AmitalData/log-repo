...
namespace Logitude.CargoTracking.BL.EntityQueryServices
{
    public class CargoTrackingShipmenPartnersCardsBuilder
    {
        ...
        private string GetCardName(User user)
        {
            if (!IsDashesOrNullOrEmpty(user.Contact?.EnglishName))
            {
                return user.Contact?.EnglishName;
            }
            else if (!IsDashesOrNullOrEmpty(user.Contact?.LocalName))
            {
                return user.Contact?.LocalName;
            }
            else {
                return user.Contact?.EnglishName;
            }
        ...
        private void CreateShipmentPartnerCard(PartnerCardMetaData partnerCardMetaData)
        {
            ...
            if (id != null)
            {
                ...
                if (partnerCardMetaData.SkipIfEmpty && string.IsNullOrEmpty(name))
                    return;
