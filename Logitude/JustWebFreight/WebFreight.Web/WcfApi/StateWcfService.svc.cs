...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "StateWcfService" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select StateWcfService.svc or StateWcfService.svc.cs at the Solution Explorer and start debugging.
     [AspNetCompatibilityRequirements(RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]
    public class StateWcfService : IStateWcfService
    {
         public Response Upsert(StatePM entityPM, bool batch)
         {
             ...
             try
             {
                 ...
                 SecurityUtility.CheckContactFeature("State", "UPDATE", entityPM.Tenant);//UPDATE//READ
                 using (TransactionScope scope = TransactionFactory.GetTransaction())
                 {
                    
                     ClassLevelValidator validationClass = new ClassLevelValidator("State", entityPM.Tenant) { IsHybrid = true };
                     ...
                     State entity = StateRepository.GetSingleStateByCode(entityPM.Code, entityPM.Tenant);
                     if (entity == null)
                     {
