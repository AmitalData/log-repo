...
namespace WebFreight.Web.WcfApi
{
    ...
    public class DeclarationApprovalRequestWcfService : IDeclarationApprovalRequestWcfService

    {

        public Response RequestDeclarationApproval(DeclarationApprovalRequestPM declarationApprovalRequestPM)
        {
        ...
        private void AddQueueToSendApprovalRequestToLogBox(string shipmentAdditionalDataId, int tenant,int importerTenant)
        {
            ...
            queueservice.Send(new Dictionary<string, string>() { { "Id", shipmentAdditionalDataId }, { "Tenant", tenant.ToString() }, { "ImporterTenant", importerTenant.ToString() }, { "CorrelationId", Guid.NewGuid().ToString() } }, tenant);
        }
