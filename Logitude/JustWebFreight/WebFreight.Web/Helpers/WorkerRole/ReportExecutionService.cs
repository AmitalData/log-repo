            ...
            try
            {
                if (queueService != null && queueResponse != null)
                {
                    ...
                    else
                    {
                        ...
                        if (reportExecutionLog != null && reportExecutionLog.RetryNumber >= 2 && (reportExecutionLog.StatusCode == "W" || reportExecutionLog.StatusCode == "P"))
                            UpdateReportExecutionLog(new ReportExecutionLogArgs() { Exception = new Exception("Report Exc failed - Removed from queue and mark as fail the exc"), DoneDate = DateTime.Now, StartDate = startDate, StatusCode = "F" });
                    }
                }
            }
            catch (Exception ex)
