    ...
    public partial class ExportToExcelHelper
    {
        ...
        public byte[] ExportDataTableToExcelForExportStorage(List<DataTable> dataTable, int tenant, List<BITabularViewSettings> bITabularViewSettings, int numSheet = 1)
        {
            ...
            for (int k = 0; k < numSheet; k++)
            {
                ...
                int count = dataTable[k].Columns.Count;
                List<DataColumn> deletedColumns = new List<DataColumn>();

                var columnNames = bITabularViewSettings[k].Columns.OrderBy(a => a.Index).Select(d => d.Code).ToList();
                int columnIndex = 0;
                foreach (var columnName in columnNames)
                {
                    dataTable[k].Columns[columnName].SetOrdinal(columnIndex);
                    columnIndex++;
                }
                ...
                if (deletedColumns.Count > 0)
                {
                    foreach (var item in deletedColumns)
                    {
                        dataTable[k].Columns.Remove(item);
                    }
                }
                RemoveEqualFromAllColumns(dataTable[k]);//avoid exception "David" is not valid named range due value:"= david 33"

                sheet.ImportDataTable(dataTable[k], true, 1, 1);

                // sheet Format - Width 
                for (var i = 0; i < dataTable[k].Columns.Count; i++)
                {
                    var agColumn = bITabularViewSettings[k].Columns.Where(a => a.Name == dataTable[k].Columns[i].ColumnName).FirstOrDefault();
                    if (agColumn != null)
                    {
                        sheet.Columns[i].ColumnWidth = agColumn.Width / 7.5;
                    }
                ...
                TenantRepository tenantRepoitory = new TenantRepository(tenant);
                var CurTenant = tenantRepoitory.GetSingleByTenant(tenant);
                var rows = dataTable[k].Rows.Count;
                if (rows > 0)
                {
                    for (int j = 1; j <= dataTable[k].Columns.Count; j++)
                    {
                        var agColumn = bITabularViewSettings[k].Columns.Where(a => a.Name == dataTable[k].Columns[j - 1].ColumnName).FirstOrDefault();
                        if (agColumn != null)
                        {
                ...
                sheet.Range[1, 1, 1, dataTable[k].Columns.Count + 1].CellStyle.ColorIndex = ExcelKnownColors.Grey_25_percent;
                sheet.Range[1, 1, 1, dataTable[k].Columns.Count + 1].CellStyle.Font.Bold = true;
                ///workbook.Version = ExcelVersion.Excel2007;
