...
namespace WebFreight.Web.Helpers
{
	...
	public partial class ExportToExcelHelper
	{
										   ...
										   Dictionary<string, dynamic> currentRow, bool withPrefix = false, string prefixProp = null)
		{
			...
			{
				AddRow(table, currentRow);
			}
			
		}

		private static void AddRow(DataTable table, Dictionary<string, dynamic> rowData)
		{
			foreach (var key in rowData.Keys)
			{

				JToken jToken = rowData[key];
				JTokenType jTokenType = jToken == null? JTokenType.Null: jToken.Type;
				Type systemType = ToSystemType(jTokenType);
		...
		private static Type ToSystemType( JTokenType jTokenType)
		{
			switch (jTokenType)
			{
