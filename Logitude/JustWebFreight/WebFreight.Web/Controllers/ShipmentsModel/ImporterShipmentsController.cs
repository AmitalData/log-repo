                        ...
                        if (!string.IsNullOrEmpty(ImporterShipment.CustomerShipmentNumber))
                        {
                            ...
                            if (DocumentFilingPMsIds != null && DocumentFilingPMsIds.Count > 0)
                            {
                                foreach (var DocumentFilingPMId in DocumentFilingPMsIds)
                                {
                                    ...
                                    queueservice.Send(new Dictionary<string, string>() { { "ShipmentId", ImporterShipment.Id }, { "DocumentFilingId", DocumentFilingPMId }, { "Tenant", ImporterShipment.Tenant.ToString() }, { "CorrelationId", Guid.NewGuid().ToString() } }, ImporterShipment.Tenant);
                                }
                            }
                        }
                        else
