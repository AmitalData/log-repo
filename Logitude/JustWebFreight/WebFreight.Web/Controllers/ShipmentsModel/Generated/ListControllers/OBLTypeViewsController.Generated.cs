...
{
    public class OBLTypeViewsController : ApiController
    {
        public HttpResponseMessage GetSingle(string code)
        {
            try
            {
                ...
                OBLTypeList entityList = null;
                OBLType entityPoco = entityRepository.GetSingleOBLType(code);

                if (entityPoco != null)
                {
                    OBLTypeQuery entityQuery = new OBLTypeQuery(entityRepository);
                    entityList = entityQuery.GetSingleOBLTypeList(entityPoco);
                }
        ...
        public HttpResponseMessage GetAll()
        {
            try
            {
                ...
                IShipmentsContext MyContext = ShipmentsContext.GetContext(authToken.Tenant);
                OBLTypeRepository entityRepository = new OBLTypeRepository(MyContext);
                IQueryable<OBLType> entityPocos = entityRepository.GetOBLTypes();

                OBLTypeQuery entityQuery = new OBLTypeQuery(entityRepository);
                IQueryable<OBLTypeList> entityLists = entityQuery.GetIQueryableEntityList(entityPocos);
                entityLists = entityLists.OrderBy(d => d.Name);
                List<OBLTypeList> listResult = entityLists.ToList();
                PerformanceLogger.AddServerExecutionTimeHeader(logKey);
        ...
        public HttpResponseMessage GetByFilters([FromUri] ApiQueryFilters filters)
        {
            try
            {
                ...
                QueryOperations queryOperations = new QueryOperations()
                {
                    ...
                    QuerySection = "OBLTypes",
                    SortByColumnName = filters.SortBy,
                    SortDirectin = filters.SortDirection,
                    GetAll = filters.GetAll,
                };

                List<ObjectField> entityObjectFields = ObjectFieldRepository.GetObjectFieldsByObjectTableName("OBLType", tenant);
                ...
                entityPocos = genericFilter.GetFilteredQuery<OBLType>(nonListQueryOperation, entityPocos);
                int skippedEntities = queryOperations.PageIndex;
                IQueryable<OBLTypeList> entityLists = entityQuery.GetIQueryableEntityList(entityPocos);

                entityLists = genericFilter.GetFilteredQuery<OBLTypeList>(listQueryOperation, entityLists);
                ...
                if (!string.IsNullOrEmpty(queryOperations.SortByColumnName) && !string.IsNullOrEmpty(queryOperations.SortDirectin))
                {
                    ...
                    if (objectField != null)
                    {
                        ...
                        {
                            entityLists = sortClass.GetSorterQuery<OBLTypeList, string>(queryOperations, entityLists);
                        }
                        else
                        {
                            switch (objectField.DataTypeCode.ToLower())
                            {
                                ...
                                case "double":
                                    {
                                        entityLists = sortClass.GetSorterQuery<OBLTypeList, double>(queryOperations, entityLists);
                                        break;
                                    }
                                case "date":
                                case "datetime":
                                    {
                                        entityLists = sortClass.GetSorterQuery<OBLTypeList, DateTime>(queryOperations, entityLists);
                                        break;
                                    }
                                case "unsinteger":
                                case "integer":
                                    {
                                        entityLists = sortClass.GetSorterQuery<OBLTypeList, int>(queryOperations, entityLists);
                                        break;
                                    }
                                case "boolean":
                                    {
                                        entityLists = sortClass.GetSorterQuery<OBLTypeList, bool>(queryOperations, entityLists);
                                        break;
                                    }
                                case "unsdecimal":
                                case "decimal":
                                    {
                                        entityLists = sortClass.GetSorterQuery<OBLTypeList, decimal>(queryOperations, entityLists);
                                        break;
                                    }
                                default:
                                    {
