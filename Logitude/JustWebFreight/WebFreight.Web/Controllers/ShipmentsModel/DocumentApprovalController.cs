            ...
            try
            {
                ...
                new Logitude.Server.Tools.Parameter { Name = "Direction", Value = Data.Direction}
                    ...
                    if (!string.IsNullOrEmpty(commLog.QueueName))
                    {
                        try
                        {
                            Communications.UpdateCommunicationLogStatus(commLog.Id, Data.Tenant, null, commLog.CommunicationStatusTypeCode, "Before adding message to queue DocumentApproval " + DateTime.Now.ToString(), null);
                            SendCommunicationLogMessageToQueue(commLog.QueueName, commLog.Id, Data.Tenant);
                            Communications.UpdateCommunicationLogStatus(commLog.Id, Data.Tenant, null, commLog.CommunicationStatusTypeCode, "after adding message to queue  DocumentApproval " + DateTime.Now.ToString(), null);
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Communications.UpdateCommunicationLogStatus(commLog.Id, Data.Tenant, null, commLog.CommunicationStatusTypeCode, "Exception occured while adding message to queue DocumentApproval " + DateTime.Now.ToString(), errorMessage);
