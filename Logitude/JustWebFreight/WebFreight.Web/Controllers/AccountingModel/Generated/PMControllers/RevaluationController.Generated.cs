...
{ 
    ...
    public partial class RevaluationsController : ApiController
    {
		  ...
		  try
            {
				...
				revaluationQuery.InitializeSettings();
                RevaluationPM revaluationPM = revaluationQuery.GetSingle(id,true,false);

				PerformanceLogger.AddServerExecutionTimeHeader(logKey);
            
                return Request.CreateResponse(HttpStatusCode.OK, revaluationPM);
			 }
            catch (Exception ex)
            {
	   ...
	   public HttpResponseMessage Post(RevaluationPM entityPM)
        {
        ...
        public HttpResponseMessage Put(RevaluationPM entityPM)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    using (TransactionScope scope = TransactionFactory.GetTransaction())
                    {
                        ...
                        AuthenticationToken authToken = AuthenticationTokenRepository.GetSingleTokenFromCache(token);
                        SecurityUtility.AuthenticationOnTenant(authToken.Tenant);
                        SecurityUtility.CheckContactFeature("Revaluation", "UPDATE", authToken.Tenant);
	                        SecurityUtility.AuthenticationOnEntityTenant("Revaluation", entityPM.Tenant, authToken.Tenant);
