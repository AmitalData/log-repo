...
{ 
    ...
    public partial class CashBooksController : ApiController
    {
		  ...
		  try
            {
				...
				cashBookQuery.InitializeSettings();
	   ...
	   public HttpResponseMessage Post(CashBookPM entityPM)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    using (TransactionScope scope = TransactionFactory.GetTransaction())
                    {
                        ...
                        AuthenticationToken authToken = AuthenticationTokenRepository.GetSingleTokenFromCache(token);
                        SecurityUtility.AuthenticationOnTenant(authToken.Tenant);
                        SecurityUtility.CheckContactFeature("CashBook", "NEW", authToken.Tenant);
	                        SecurityUtility.AuthenticationOnEntityTenant("CashBook", entityPM.Tenant, authToken.Tenant);
        ...
        public HttpResponseMessage Put(CashBookPM entityPM)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    using (TransactionScope scope = TransactionFactory.GetTransaction())
                    {
                        ...
                        AuthenticationToken authToken = AuthenticationTokenRepository.GetSingleTokenFromCache(token);
                        SecurityUtility.AuthenticationOnTenant(authToken.Tenant);
                        SecurityUtility.CheckContactFeature("CashBook", "UPDATE", authToken.Tenant);
	                        SecurityUtility.AuthenticationOnEntityTenant("CashBook", entityPM.Tenant, authToken.Tenant);
