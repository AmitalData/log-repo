    ...
    public class ShaamWebServiceController : ApiController
    {
        ...
        public HttpResponseMessage Tokens(int pageSize, int page)
        {
            try
            {
                ...
                int tenant = AuthorizedToken();
                HttpClient client = GetShaamApiHttpClient(tenant);
                string url = $"{AmitalTaxesUrl}taxes/tokens?pageSize={pageSize}&page={page}";
                HttpResponseMessage res = client.GetAsync(url).Result;
                string content = res.Content.ReadAsStringAsync().Result;
                client.Dispose();
        ...
        public HttpResponseMessage NewRefreshToken([FromBody] NewRefreshTokenBody body)
        {
            try
            {
                int tenant = AuthorizedToken();
                HttpClient client = GetShaamApiHttpClient(tenant);
                string url = $"{AmitalTaxesUrl}taxes/createNewRefreshToken?user={body.user}&code={body.code}";
        ...
        public HttpResponseMessage createConfirmationNumber([FromBody] dynamic body, int? confirmationTokenLogId = null)
        {
            try
            {
                ...
                int tenant = AuthorizedToken();
                HttpClient client = GetShaamApiHttpClient(tenant);
                string url = $"{AmitalTaxesUrl}taxes/createConfirmationNumber?confirmationTokenLogId={confirmationTokenLogId}";
                StringContent stringContent = new StringContent(JsonConvert.SerializeObject(invoiceJson), Encoding.UTF8, "application/json");
