...
{ 
    ...
    public partial class ClaimsController : ApiController
    {
		  ...
		  try
            {
                ...
                SecurityUtility.CheckContactFeature("Customs.Claim", "READ", authToken.Tenant);
	                
                ICustomContext MyContext = CustomContext.GetContext(authToken.Tenant);
                ClaimQueryService claimQuery = new ClaimQueryService(MyContext);
				claimQuery.InitializeSettings();
                ClaimPM claimPM = claimQuery.GetSingle(id,true,false);

				PerformanceLogger.AddServerExecutionTimeHeader(logKey);
            
                return Request.CreateResponse(HttpStatusCode.OK, claimPM);
			 }
            catch (Exception ex)
            {
	   ...
	   public HttpResponseMessage Post(ClaimPM entityPM)
        ...
        public HttpResponseMessage Put(ClaimPM entityPM)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    using (TransactionScope scope = TransactionFactory.GetTransaction())
                    {
                        ...
                        AuthenticationToken authToken = AuthenticationTokenRepository.GetSingleTokenFromCache(token);
                        SecurityUtility.AuthenticationOnTenant(authToken.Tenant);
                        SecurityUtility.CheckContactFeature("Customs.Claim", "UPDATE", authToken.Tenant);
	                        SecurityUtility.AuthenticationOnEntityTenant("Claim", entityPM.Tenant, authToken.Tenant);
