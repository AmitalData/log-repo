...
{ 
    ...
    public partial class CustomsAirlinesController : ApiController
    {
		  ...
		  try
            {
                ...
                SecurityUtility.CheckContactFeature("Customs.CustomsAirline", "READ", authToken.Tenant);
	                
                ICustomContext MyContext = CustomContext.GetContext(authToken.Tenant);
                CustomsAirlineQueryService customsAirlineQuery = new CustomsAirlineQueryService(MyContext);
				customsAirlineQuery.InitializeSettings();
                CustomsAirlinePM customsAirlinePM = customsAirlineQuery.GetSingle(id,true,false);
	   ...
	   public HttpResponseMessage Post(CustomsAirlinePM entityPM)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    using (TransactionScope scope = TransactionFactory.GetTransaction())
                    {
                        ...
                        AuthenticationToken authToken = AuthenticationTokenRepository.GetSingleTokenFromCache(token);
                        SecurityUtility.AuthenticationOnTenant(authToken.Tenant);
                        SecurityUtility.CheckContactFeature("Customs.CustomsAirline", "NEW", authToken.Tenant);
	                        SecurityUtility.AuthenticationOnEntityTenant("CustomsAirline", entityPM.Tenant, authToken.Tenant);
        ...
        public HttpResponseMessage Put(CustomsAirlinePM entityPM)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    using (TransactionScope scope = TransactionFactory.GetTransaction())
                    {
                        ...
                        AuthenticationToken authToken = AuthenticationTokenRepository.GetSingleTokenFromCache(token);
                        SecurityUtility.AuthenticationOnTenant(authToken.Tenant);
                        SecurityUtility.CheckContactFeature("Customs.CustomsAirline", "UPDATE", authToken.Tenant);
	                        SecurityUtility.AuthenticationOnEntityTenant("CustomsAirline", entityPM.Tenant, authToken.Tenant);
