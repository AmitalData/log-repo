    ...
    public class FlowCardController : ApiController
    {
        public HttpResponseMessage PostByFilterTree(ApiQueryTreeFilters apiQueryTreeFilters)
        {
            ...
            try
            {
                ...
                IQueryable<CardList> cardQuery = cardsQuery.GetCardWithAddresses(tenant);
                ...
                cardQuery = new TreeFilterQueryService().Apply(cardQuery, treeFilterQueryArgs);
                cardQuery = cardQuery.OrderBy(string.IsNullOrEmpty(apiQueryTreeFilters.OrderBy) ? "Id" : apiQueryTreeFilters.OrderBy);
                cardQuery = cardQuery.Skip(0);
                cardQuery = cardQuery.Take(apiQueryTreeFilters.PageSize);

                var cards = cardQuery.Select("new { " + apiQueryTreeFilters.ReturnedColumns + " }").ToDynamicList();
