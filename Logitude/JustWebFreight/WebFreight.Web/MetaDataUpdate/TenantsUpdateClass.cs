...
namespace WebFreight.Web.MetaDataUpdate
{
    ...
    public class TenantsUpdateClass
    {
                ...
                if (tenant == 0)
                {
                    ...
                    switch (message.ToLower())
                    {
                        ...
                        case "updatetenantzeronew":
                            {
                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Update started ...");
                                ...
                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Tarrif Module ...");
                                UpdateTariffModule(context, false);

                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Time Management Module ...");
                                UpdateTimeManagementModule(context, false);

                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Warehouse Module ...");
                                UpdateWarehouseModule(context, false);

                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Shipment Order Module ...");
                                UpdateShipmentOrderModule(context, false);

                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Social Module ...");
                                UpdateSocialModule(context, false);

                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Booking Module ...");
                                UpdateBookingModule(context, false);

                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating CRM Module ...");
                                UpdateCRMModule(context, false);

                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Workflow Module ...");
                                UpdateWorkflow(context, false);

                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Dashboard Module ...");
                                UpdateDashboardModule(context, false);

                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Menus tables ...");
                                updateClass.LoadMenustables();
                                performanceTimerLogger.LogMessage("Manual" + ",MetaDataUpdateClass.LoadMenustables");

                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Default Reports ...");
                                ...
                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Master Counters ...");
                                updateClass.CreateMasterCounter(0);
                                performanceTimerLogger.LogMessage("Manual" + ",MetaDataUpdateClass.CreateMasterCounter");

                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Email Alerts ...");
                                ...
                                {
                                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Logbox Automation fields ...");
                                ...
                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Rules ...");
                                ...
                                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Reconecting Metadata fields with Ids ...");
        ...
        public static void UpdateTenants()
        {
            try
            {
                ...
                if (globalTenants != null)
                {
                    ...
                    if (upgradableTenants.Count > 0)
                    {
                        ...
                        TenantZeroCreditCardTypes = creditCardTypeRepositoryRepository.GetCreditCardTypes(0).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                        TenantZeroMoveTypes = moveTypeRepository.GetMoveTypesByTenant(0).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                        TenantZeroEmailAlertSettings = emailAlertSettingRepository.GetEmailAlertSettings(0).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                        TenantZeroJournalActionTypes = journalActionTypeRepository.GetAll(0).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                        TenantZeroTaxWithholdingAssessOffices = taxWithholdingAssessOfficeRepository.GetAll(0).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                        TenantZeroAccountingCompanyTypes = accountingCompanyTypeRepository.GetAll(0).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                        TenantZeroWithholdingTaxDeductionTypes = withholdingTaxDeductionTypeRepository.GetAll(0).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                        TenantZeroChargesGroups = chargesGroupRepository.GetChargesGroups(0).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
            ...
            {
                NetCommonHelper.Logger.DevLog.Instance.WriteFatal(ex, "Exiting function with Exception , UpdateTenants");
                throw ex;
            }
        } 
        ...
        static string RunGitCommand(string arguments)
            ...
            private static void UpdateCustomsRelatedModels(IWebFreightContext context)
            {
                ...
                NetCommonHelper.Logger.DevLog.Instance.WriteInfo(message);
                 AzureLog.SaveLogsInStorage(message, "L", DateTime.Now, "", "", 0, null, null, null);

                NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Initializing Infrastructure Module ...");
                 ...
                 try
                {
                    ...
                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Initializing System Logs Module ...");
                    SystemLogsModelUpdateClass systemLogsModelUpdateClass = new SystemLogsModelUpdateClass();

                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Initializing Common Module ...");
                    CommonDataModelUpdateClass commonmodelUpdateClass = new CommonDataModelUpdateClass();

                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Initializing Global Module ...");
                    GlobalModelUpdateClass globalmodelUpdateClass = new GlobalModelUpdateClass();

                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Initializing Business Infrastructure Module ...");
                    InfrastructureUpdateClass businessInfraUpdateClass = new InfrastructureUpdateClass();

                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Initializing Customs Module ...");
            ...
            private static void UpdateTenantData(int tenant)
            {
                try
                {
                    ...
                    {
                        tenantZeroEventTypes = TenantZeroEventTypes.Values.Where(e => e.UpdateDate > globalTenant.LastUpdateDate).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                    }
                    else
                    {
                    ...
                    Dictionary<string, EventType> currentTenantEventTypes = eventTypeRepository.GetEventTypesByTenant(tenant).GroupBy(d => d.Code + d.ObjectTableId).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                    Dictionary<string, Rank> tenantZeroRanks = TenantZeroRanks;
                    Dictionary<string, Rank> currentTenantRanks = rankRepository.GetRanks(tenant).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                    ...
                    Dictionary<string, EmailAlertSetting> currentEmailAlertSettings = emailAlertSettingRepository.GetEmailAlertSettings(tenant).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                    Dictionary<string, JournalActionType> tenantZeroJournalActionTypes = TenantZeroJournalActionTypes;
                    Dictionary<string, JournalActionType> currentJournalActionTypes = journalActionTypeRepository.GetAll(tenant).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                    Dictionary<string, TaxWithholdingAssessOffice> tenantZeroTaxWithholdingAssessOffices = TenantZeroTaxWithholdingAssessOffices;
                    Dictionary<string, TaxWithholdingAssessOffice> currentTaxWithholdingAssessOffices = taxWithholdingAssessOfficeRepository.GetAll(tenant).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                    Dictionary<string, AccountingCompanyType> tenantZeroAccountingCompanyTypes = TenantZeroAccountingCompanyTypes;
                    Dictionary<string, AccountingCompanyType> currentAccountingCompanyTypes = accountingCompanyTypeRepository.GetAll(tenant).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
                    ...
                    Dictionary<string, WithholdingTaxDeductionType> currentWithholdingTaxDeductionTypes = withholdingTaxDeductionTypeRepository.GetAll(tenant).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());

                    Dictionary<string, ChargesGroup> tenantZeroChargesGroups = TenantZeroChargesGroups;
                    Dictionary<string, ChargesGroup> currentTenantChargesGroups = chargesGroupRepository.GetChargesGroups(tenant).GroupBy(d => d.Code).ToDictionary(g => g.Key, a => a.FirstOrDefault());
            ...
            private static void UpdateAllOldModules(MetaDataUpdateClass updateClass, IWebFreightContext context)
            {
                try
                {
                    ...
                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Initializing Shipment Module ...");
                    ShipmentsModelUpdateClass shipmentModelUpdateClass = new ShipmentsModelUpdateClass();

                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Initializing Master Module ...");
                    MasterModelUpdateClass masterModelUpdateClass = new MasterModelUpdateClass();

                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Initializing Quote Module ...");
                    QuoteModelUpdateClass quotemodelUpdateClass = new QuoteModelUpdateClass();

                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Initializing Invoice Module ...");
                    InvoiceModelUpdateClass invoicemodelUpdateClass = new InvoiceModelUpdateClass();
                    ...
                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Entity Status...");
                    updateClass.LoadEntityStatus();
                    ...
                    else
                    {
                        ...
                        NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Shipment Module ...");
                        shipmentModelUpdateClass.LoadObjectTablesMetadata(context, false);
                        performanceTimerLogger.LogMessage("Generated" + ",ShipmentsModelUpdateClass");

                        NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Master Module ...");
                        masterModelUpdateClass.LoadObjectTablesMetadata(context, false);
                        performanceTimerLogger.LogMessage("Generated" + ",MasterModelUpdateClass");

                        NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Quote Module ...");
                        quotemodelUpdateClass.LoadObjectTablesMetadata(context, false);
                        performanceTimerLogger.LogMessage("Generated" + ",QuoteModelUpdateClass");

                        NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Invoice Module ...");
                    ...
                    //
                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Table Counters & Tips ...");
                    ...
                    //updateClass.LoadOtherFields(context);
                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Translation Headers ...");
                    updateClass.LoadTranslationHeaders();
                    performanceTimerLogger.LogMessage("Manual" + ",MetaDataUpdateClass.LoadTranslationHeaders");

                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Measurements ...");
                    updateClass.LoadMeasurements();
                    performanceTimerLogger.LogMessage("Manual" + ",MetaDataUpdateClass.LoadMeasurements");

                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Credit Card Types ...");
                    updateClass.LoadCreditCardTypes();
                    performanceTimerLogger.LogMessage("Manual" + ",MetaDataUpdateClass.LoadCreditCardTypes");

                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Move Types ...");
                    updateClass.LoadMoveTypes();
                    context.SaveChanges();
                    ...
                    //updateClass.LoadRolesAndFeatures(0);
                    //performanceTimerLogger.LogMessage("Manual" + ",MetaDataUpdateClass.LoadRolesAndFeatures");
                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Table Helper Controls ...");
                    updateClass.LoadObjectTableHelperControls();
                    ...
                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Event Types ...");
                    updateClass.LoadEventTypes();
                    performanceTimerLogger.LogMessage("Manual" + ",MetaDataUpdateClass.LoadEventTypes");

                    NetCommonHelper.Logger.DevLog.Instance.WriteInfo("Updating Ranks ...");
                    updateClass.LoadRanks();
            ...
            public static void BuildObjectTablesZipFilesData(bool savetodisk = false, bool includeCustoms = false)
            {
                try
                {
                    ...
                    {
                        if (objectTable.DBTableName == "CustomerTenantAccessStatusTypes" || objectTable.DBTableName == "ShipmentStoragePricings")
                        {
