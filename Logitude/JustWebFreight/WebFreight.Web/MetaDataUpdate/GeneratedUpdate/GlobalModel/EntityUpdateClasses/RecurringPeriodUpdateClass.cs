...
namespace WebFreight.Web.MetaDataUpdate.GeneratedUpdate.GlobalModel.EntityUpdateClasses
{
   ...
   {  
	    ...
	    public void FillRecurringPeriod()
        { 
            var repo = new RecurringPeriodRepository(0);
            var dic =repo.GetAll().ToDictionary(rec => rec.Code, rec => rec);
            new FillCloseTables().FillCloseTable<
                                RecurringPeriod,
                                Logitude.BL.GlobalModel.RecurringPeriodDetails,
                                RecurringPeriodRepository>(repo, dic);
        }
