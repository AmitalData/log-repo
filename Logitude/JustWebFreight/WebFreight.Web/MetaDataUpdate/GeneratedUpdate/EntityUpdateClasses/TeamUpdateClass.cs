   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn AllTeamsQueryColumn_0 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllTeamsQuery.Id,QueryCode = AllTeamsQuery.Code, IndexOrder = 0, ObjectFieldId = TeamObjectFields.Where(d => d.FieldName == "Name" && d.ObjectTableId == TeamObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = TeamObjectFields.Where(d => d.FieldName == "Name" && d.ObjectTableId == TeamObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllTeamsQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllTeamsQuery.Id,QueryCode = AllTeamsQuery.Code, IndexOrder = 1, ObjectFieldId = TeamObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == TeamObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = TeamObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == TeamObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllTeamsQueryColumn_2 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllTeamsQuery.Id,QueryCode = AllTeamsQuery.Code, IndexOrder = 2, ObjectFieldId = TeamObjectFields.Where(d => d.FieldName == "ManagerUserName" && d.ObjectTableId == TeamObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = TeamObjectFields.Where(d => d.FieldName == "ManagerUserName" && d.ObjectTableId == TeamObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllTeamsQueryColumn_3 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllTeamsQuery.Id,QueryCode = AllTeamsQuery.Code, IndexOrder = 3, ObjectFieldId = TeamObjectFields.Where(d => d.FieldName == "InActive" && d.ObjectTableId == TeamObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = TeamObjectFields.Where(d => d.FieldName == "InActive" && d.ObjectTableId == TeamObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 100 }, queryColumnsRepository, tenantQueryColumns);
