   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn AllTicketSeveritiesQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllTicketSeveritiesQuery.Id, IndexOrder = 1, ObjectFieldId = TicketSeverityObjectFields.Where(d => d.FieldName == "Inactive" && d.ObjectTableId == TicketSeverityObjectTable.Id).FirstOrDefault().Id, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);
	    ...
	    {    
            ...
            AddObjectTableTabs.AddObjectTableTab(new ObjectTableTabDetails() { Code = "TSGE",HtmlComponentName = "",HtmlComponentUrl = "", FeatureId = tenantFeatures.Where(d => d.Code == "TicketSeverity.Tab.General" && d.ObjectTableId == TicketSeverityObjectTable.Id).FirstOrDefault().Id, ControlPath = "Simplog.Infrastructure.GeneralControls.GeneralTabControl", ObjectTableId = TicketSeverityObjectTable.Id, TabNameTextCodeId = tenantTextCodes.Where(d => d.Code == "TicketSeverity.TH.General" && d.Tenant == 0).FirstOrDefault().Id, Tenant = 0, IndexOrder = 0 }, objectTableTabsRepository, TenantObjectTableTabs);
   
            AddObjectTableTabs.AddObjectTableTab(new ObjectTableTabDetails() { Code = "TSEV",HtmlComponentName = "",HtmlComponentUrl = "", FeatureId = tenantFeatures.Where(d => d.Code == "TicketSeverity.Tab.Events" && d.ObjectTableId == TicketSeverityObjectTable.Id).FirstOrDefault().Id, ControlPath = "Simplog.Infrastructure.Views.Events.EventsControl", ObjectTableId = TicketSeverityObjectTable.Id, TabNameTextCodeId = tenantTextCodes.Where(d => d.Code == "TicketSeverity.TH.Events" && d.Tenant == 0).FirstOrDefault().Id, Tenant = 0, IndexOrder = 1 }, objectTableTabsRepository, TenantObjectTableTabs);
