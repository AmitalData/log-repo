   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn AllSprintsQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllSprintsQuery.Id, IndexOrder = 1, ObjectFieldId = SprintObjectFields.Where(d => d.FieldName == "FromDate" && d.ObjectTableId == SprintObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = SprintObjectFields.Where(d => d.FieldName == "FromDate" && d.ObjectTableId == SprintObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllSprintsQueryColumn_2 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllSprintsQuery.Id, IndexOrder = 2, ObjectFieldId = SprintObjectFields.Where(d => d.FieldName == "ToDate" && d.ObjectTableId == SprintObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = SprintObjectFields.Where(d => d.FieldName == "ToDate" && d.ObjectTableId == SprintObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);
