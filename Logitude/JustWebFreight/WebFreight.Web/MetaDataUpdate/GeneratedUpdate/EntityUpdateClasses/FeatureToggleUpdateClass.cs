   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn AllFeatureTogglesQueryColumn_0 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllFeatureTogglesQuery.Id,QueryCode = AllFeatureTogglesQuery.Code, IndexOrder = 0, ObjectFieldId = FeatureToggleObjectFields.Where(d => d.FieldName == "CreateDate" && d.ObjectTableId == FeatureToggleObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = FeatureToggleObjectFields.Where(d => d.FieldName == "CreateDate" && d.ObjectTableId == FeatureToggleObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllFeatureTogglesQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllFeatureTogglesQuery.Id,QueryCode = AllFeatureTogglesQuery.Code, IndexOrder = 1, ObjectFieldId = FeatureToggleObjectFields.Where(d => d.FieldName == "CreatedByUser" && d.ObjectTableId == FeatureToggleObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = FeatureToggleObjectFields.Where(d => d.FieldName == "CreatedByUser" && d.ObjectTableId == FeatureToggleObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllFeatureTogglesQueryColumn_2 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllFeatureTogglesQuery.Id,QueryCode = AllFeatureTogglesQuery.Code, IndexOrder = 2, ObjectFieldId = FeatureToggleObjectFields.Where(d => d.FieldName == "ToggleName" && d.ObjectTableId == FeatureToggleObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = FeatureToggleObjectFields.Where(d => d.FieldName == "ToggleName" && d.ObjectTableId == FeatureToggleObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllFeatureTogglesQueryColumn_3 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllFeatureTogglesQuery.Id,QueryCode = AllFeatureTogglesQuery.Code, IndexOrder = 3, ObjectFieldId = FeatureToggleObjectFields.Where(d => d.FieldName == "TenantNumber" && d.ObjectTableId == FeatureToggleObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = FeatureToggleObjectFields.Where(d => d.FieldName == "TenantNumber" && d.ObjectTableId == FeatureToggleObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);
	    ...
	    {                
            ...
            AddObjectTableTabs.AddObjectTableTab(new ObjectTableTabDetails() { Code = "FTEV",HtmlComponentName = "",HtmlComponentUrl = "", FeatureId = FeatureToggleEventsFeature_TH1.Id, ControlPath = "Simplog.Infrastructure.Views.Events.EventsControl", ObjectTableId = FeatureToggleObjectTable.Id, TabNameTextCodeId = FeatureToggleEventsTextCode_TH1.Id, TabNameTextCodeCode = FeatureToggleEventsTextCode_TH1.Code, Tenant = 0, IndexOrder = 1 }, objectTableTabsRepository, TenantObjectTableTabs);
