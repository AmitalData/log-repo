   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn ALLTAXREPORTSQueryColumn_0 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = ALLTAXREPORTSQuery.Id,QueryCode = ALLTAXREPORTSQuery.Code, IndexOrder = 0, ObjectFieldId = TaxReportObjectFields.Where(d => d.FieldName == "TaxReportMonth" && d.ObjectTableId == TaxReportObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = TaxReportObjectFields.Where(d => d.FieldName == "TaxReportMonth" && d.ObjectTableId == TaxReportObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 100 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn ALLTAXREPORTSQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = ALLTAXREPORTSQuery.Id,QueryCode = ALLTAXREPORTSQuery.Code, IndexOrder = 1, ObjectFieldId = TaxReportObjectFields.Where(d => d.FieldName == "StatusEnglishName" && d.ObjectTableId == TaxReportObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = TaxReportObjectFields.Where(d => d.FieldName == "StatusEnglishName" && d.ObjectTableId == TaxReportObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 120 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn ALLTAXREPORTSQueryColumn_2 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = ALLTAXREPORTSQuery.Id,QueryCode = ALLTAXREPORTSQuery.Code, IndexOrder = 2, ObjectFieldId = TaxReportObjectFields.Where(d => d.FieldName == "CreateDate" && d.ObjectTableId == TaxReportObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = TaxReportObjectFields.Where(d => d.FieldName == "CreateDate" && d.ObjectTableId == TaxReportObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 100 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn ALLTAXREPORTSQueryColumn_3 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = ALLTAXREPORTSQuery.Id,QueryCode = ALLTAXREPORTSQuery.Code, IndexOrder = 3, ObjectFieldId = TaxReportObjectFields.Where(d => d.FieldName == "CreatedByUserName" && d.ObjectTableId == TaxReportObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = TaxReportObjectFields.Where(d => d.FieldName == "CreatedByUserName" && d.ObjectTableId == TaxReportObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 120 }, queryColumnsRepository, tenantQueryColumns);
