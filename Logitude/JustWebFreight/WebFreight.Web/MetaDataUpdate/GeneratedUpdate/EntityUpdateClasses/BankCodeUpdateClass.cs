   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn BankCodesQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = BankCodesQuery.Id, IndexOrder = 1, ObjectFieldId = BankCodeObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == BankCodeObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = BankCodeObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == BankCodeObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 250 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn BankCodesQueryColumn_2 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = BankCodesQuery.Id, IndexOrder = 2, ObjectFieldId = BankCodeObjectFields.Where(d => d.FieldName == "EnglishName" && d.ObjectTableId == BankCodeObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = BankCodeObjectFields.Where(d => d.FieldName == "EnglishName" && d.ObjectTableId == BankCodeObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 250 }, queryColumnsRepository, tenantQueryColumns);
