   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			  ...
			  Query AllQueuesQuery = AddQueries.AddQuery(new QueryDetails() { NameTextCodeId = BusinessProcessQueueTextCode_0.Id, NameTextCodeCode = BusinessProcessQueueTextCode_0.Code, Code = "All Queues",  QueryGroupCode = "aa2e", IndexOrder = 0, Tenant = 0, ObjectTableId = BusinessProcessQueueObjectTable.Id, QuerySection = "BusinessProcessQueue", SystemLevel = true, IsAddNewEntityEnabled = true, FeatureId = BusinessProcessQueueFeature_0.Id, DefaultSortName = "CreateDate", DefaultSortDirection = "Desending", Perspective = null }, queriesRepository, tenantQueries);
	
			 QueryColumn AllQueuesQueryColumn_0 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllQueuesQuery.Id,QueryCode = AllQueuesQuery.Code, IndexOrder = 0, ObjectFieldId = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "Name" && d.ObjectTableId == BusinessProcessQueueObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "Name" && d.ObjectTableId == BusinessProcessQueueObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllQueuesQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllQueuesQuery.Id,QueryCode = AllQueuesQuery.Code, IndexOrder = 1, ObjectFieldId = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == BusinessProcessQueueObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == BusinessProcessQueueObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllQueuesQueryColumn_2 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllQueuesQuery.Id,QueryCode = AllQueuesQuery.Code, IndexOrder = 2, ObjectFieldId = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "InActive" && d.ObjectTableId == BusinessProcessQueueObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "InActive" && d.ObjectTableId == BusinessProcessQueueObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 100 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllQueuesQueryColumn_3 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllQueuesQuery.Id,QueryCode = AllQueuesQuery.Code, IndexOrder = 3, ObjectFieldId = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "EntityName" && d.ObjectTableId == BusinessProcessQueueObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "EntityName" && d.ObjectTableId == BusinessProcessQueueObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllQueuesQueryColumn_4 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllQueuesQuery.Id,QueryCode = AllQueuesQuery.Code, IndexOrder = 4, ObjectFieldId = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "BusinessRoleName" && d.ObjectTableId == BusinessProcessQueueObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "BusinessRoleName" && d.ObjectTableId == BusinessProcessQueueObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);
	    ...
	    {   
		   ...
		   List<ObjectField> BusinessProcessQueueObjectFields = ObjectContext.ObjectFields.Where(d => d.ObjectTable.Name == "BusinessProcessQueue").ToList();
            ...
            ScreenField BusinessProcessQueueHeaderScreenScreenField0 = AddScreensAndScreenFields.AddScreenField(new ScreenFieldDetails() { Column = 0, Row = 0, ObjectFieldId = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "Name").FirstOrDefault().Id, ScreenId = BusinessProcessQueueHeaderScreenScreen1.Id, ObjectFieldCode = BusinessProcessQueueObjectFields.Where(d => d.FieldName == "Name").FirstOrDefault().FieldCode, Tenant = 0, }, screenFieldsRepository, tenantScreenFields);
