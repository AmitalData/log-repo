   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			  ...
			  Query JournalActionTypesQuery = AddQueries.AddQuery(new QueryDetails() { NameTextCodeId = JournalActionTypeTextCode_0.Id, Code = "Journal Action Types",  QueryGroupCode = "JACT", IndexOrder = 0, Tenant = 0, ObjectTableId = JournalActionTypeObjectTable.Id, QuerySection = "JournalActionType", SystemLevel = true, IsAddNewEntityEnabled = true, FeatureId = JournalActionTypeFeature_0.Id, DefaultSortName = null, DefaultSortDirection = null, Perspective = null }, queriesRepository, tenantQueries);
	
			 QueryColumn JournalActionTypesQueryColumn_0 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = JournalActionTypesQuery.Id, IndexOrder = 0, ObjectFieldId = JournalActionTypeObjectFields.Where(d => d.FieldName == "Code" && d.ObjectTableId == JournalActionTypeObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = JournalActionTypeObjectFields.Where(d => d.FieldName == "Code" && d.ObjectTableId == JournalActionTypeObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 73 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn JournalActionTypesQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = JournalActionTypesQuery.Id, IndexOrder = 1, ObjectFieldId = JournalActionTypeObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == JournalActionTypeObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = JournalActionTypeObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == JournalActionTypeObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 120 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn JournalActionTypesQueryColumn_2 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = JournalActionTypesQuery.Id, IndexOrder = 2, ObjectFieldId = JournalActionTypeObjectFields.Where(d => d.FieldName == "EnglishName" && d.ObjectTableId == JournalActionTypeObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = JournalActionTypeObjectFields.Where(d => d.FieldName == "EnglishName" && d.ObjectTableId == JournalActionTypeObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 120 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn JournalActionTypesQueryColumn_3 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = JournalActionTypesQuery.Id, IndexOrder = 3, ObjectFieldId = JournalActionTypeObjectFields.Where(d => d.FieldName == "Inactive" && d.ObjectTableId == JournalActionTypeObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = JournalActionTypeObjectFields.Where(d => d.FieldName == "Inactive" && d.ObjectTableId == JournalActionTypeObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 56 }, queryColumnsRepository, tenantQueryColumns);
	    ...
	    {                
            ...
            AddObjectTableTabs.AddObjectTableTab(new ObjectTableTabDetails() { Code = "JAEV",HtmlComponentName = "",HtmlComponentUrl = "", FeatureId = JournalActionTypeEventsFeature_TH1.Id, ControlPath = "Simplog.Infrastructure.Views.Events.EventsControl", ObjectTableId = JournalActionTypeObjectTable.Id, TabNameTextCodeId = JournalActionTypeEventsTextCode_TH1.Id, Tenant = 0, IndexOrder = 1 }, objectTableTabsRepository, TenantObjectTableTabs);
