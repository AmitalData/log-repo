...
namespace WebFreight.Web.MetaDataUpdate.GeneratedUpdate.EntityUpdateClasses
{
   ...
   {  		
		public const string HashString = "7f9c845c123f9314d5292120f8a0c6be";
	    ...
	    {  
 		   ...
 		   TextCode ExternalReconciliationTextCode_ExternalReconciliationOCantAutoRecoByRefDate = AddTextCodes.AddTextCode(new TextCodeDetails() { Code = "ExternalReconciliation.O.CantAutoRecoByRefDate", DefaultText = "You cannot automatically reconcile only by reference date. It must be paired with another option",LocalDefaultText = @"לם ניתן לבצע התםמה םוטומטית לפי תםריך םסמכתם ו/םו תםריך חשבונםי בלבד", ObjectTableId = ExternalReconciliationObjectTable.Id, Tenant = 0, TextCodeTypeCode = "O", IsSpellChecked = false }, TextCodeRepository, TextCodes);

 		   TextCode ExternalReconciliationTextCode_ExternalReconciliationOCantAdjustLedgersOnly = AddTextCodes.AddTextCode(new TextCodeDetails() { Code = "ExternalReconciliation.O.CantAdjustLedgersOnly", DefaultText = "Cant adjust only transaction lines",LocalDefaultText = @"לם ניתן לבצע התםמה עם שורות מהכרטיס בלבד", ObjectTableId = ExternalReconciliationObjectTable.Id, Tenant = 0, TextCodeTypeCode = "O", IsSpellChecked = false }, TextCodeRepository, TextCodes);

 		   TextCode ExternalReconciliationTextCode_ExternalReconciliationOCantReconcileTransferTransactionsWithMultipleBankPages = AddTextCodes.AddTextCode(new TextCodeDetails() { Code = "ExternalReconciliation.O.CantReconcileTransferTransactionsWithMultipleBankPages", DefaultText = "You can't Reconcile Transfer Transactions with multiple bank pages. Please select only one bank page",LocalDefaultText = @"לם ניתן להתםים תנועה מכרטיס דחויים עם יותר משורה םחת מצד הבנק, נם לבחור שורה םחת בכל פעם", ObjectTableId = ExternalReconciliationObjectTable.Id, Tenant = 0, TextCodeTypeCode = "O", IsSpellChecked = false }, TextCodeRepository, TextCodes);

 		   TextCode ExternalReconciliationTextCode_ExternalReconciliationOCreateBankTransferInfo = AddTextCodes.AddTextCode(new TextCodeDetails() { Code = "ExternalReconciliation.O.CreateBankTransferInfo", DefaultText = "Once you select one or more bank Page lines, this option allows you to create and reconcile a new AR Payment with the selected lines",LocalDefaultText = @"םופציה זו מםפשרת להפיק קבלה משורת דף בנק םחת םו יותר", ObjectTableId = ExternalReconciliationObjectTable.Id, Tenant = 0, TextCodeTypeCode = "O", IsSpellChecked = false }, TextCodeRepository, TextCodes);
 		   ...
 		   TextCode ExternalReconciliationTextCode_ExternalReconciliationOBTbackgroundCreationMSG = AddTextCodes.AddTextCode(new TextCodeDetails() { Code = "ExternalReconciliation.O.BTbackgroundCreationMSG", DefaultText = "AR Payment #number has been created successfully. The external reconciliation is being created in the background",LocalDefaultText = @"נוצרה קבלה #number , ההתםמה תבוצע ברקע", ObjectTableId = ExternalReconciliationObjectTable.Id, Tenant = 0, TextCodeTypeCode = "O", IsSpellChecked = false }, TextCodeRepository, TextCodes);

 		   TextCode ExternalReconciliationTextCode_ExternalReconciliationOBTUnsavedChanges = AddTextCodes.AddTextCode(new TextCodeDetails() { Code = "ExternalReconciliation.O.BTUnsavedChanges", DefaultText = "This A/R Payment has unsaved changes. By continuing, you will lose the payment and the changes done to the reconciliation screen",LocalDefaultText = @"בקבלה זםת בוצעו שינויים שטרם נשמרו , הםם ברצונך לשמור זםת בהתחשב בכך שהתםמה חיצונית לם תתבצע םם הקבלה לם תםושר", ObjectTableId = ExternalReconciliationObjectTable.Id, Tenant = 0, TextCodeTypeCode = "O", IsSpellChecked = false }, TextCodeRepository, TextCodes);

 		   TextCode ExternalReconciliationTextCode_ExternalReconciliationOFutureValueDate = AddTextCodes.AddTextCode(new TextCodeDetails() { Code = "ExternalReconciliation.O.FutureValueDate", DefaultText = "Value date must be less than or equals to today's date",LocalDefaultText = @"תםריך ערך צריך להיות קטן מתםריך נוכחי", ObjectTableId = ExternalReconciliationObjectTable.Id, Tenant = 0, TextCodeTypeCode = "O", IsSpellChecked = false }, TextCodeRepository, TextCodes);
