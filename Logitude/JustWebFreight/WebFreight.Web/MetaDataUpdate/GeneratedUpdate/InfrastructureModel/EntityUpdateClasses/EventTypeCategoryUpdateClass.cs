...
namespace WebFreight.Web.MetaDataUpdate.GeneratedUpdate.InfrastructureModel.EntityUpdateClasses
{
   ...
   {  		
	    ...
	    public void FillEventTypeCategory()
        { 
            var repo = new EventTypeCategoryRepository(0);
            var dic =repo.GetAll().ToDictionary(rec => rec.Code, rec => rec);
            new FillCloseTables().FillCloseTable<
                                EventTypeCategory,
                                Logitude.BL.InfrastructureModel.EventTypeCategoryDetails,
                                EventTypeCategoryRepository>(repo, dic);
        }
