   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn InboundEmailsQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = InboundEmailsQuery.Id, IndexOrder = 1, ObjectFieldId = InboundEmailObjectFields.Where(d => d.FieldName == "CreateDate" && d.ObjectTableId == InboundEmailObjectTable.Id).FirstOrDefault().Id, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn InboundEmailsQueryColumn_2 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = InboundEmailsQuery.Id, IndexOrder = 2, ObjectFieldId = InboundEmailObjectFields.Where(d => d.FieldName == "UpdateDate" && d.ObjectTableId == InboundEmailObjectTable.Id).FirstOrDefault().Id, ColumnWidth = 300 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn InboundEmailsQueryColumn_3 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = InboundEmailsQuery.Id, IndexOrder = 3, ObjectFieldId = InboundEmailObjectFields.Where(d => d.FieldName == "ObjectTableName" && d.ObjectTableId == InboundEmailObjectTable.Id).FirstOrDefault().Id, ColumnWidth = 300 }, queryColumnsRepository, tenantQueryColumns);
