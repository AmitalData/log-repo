   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn AllDistributorsQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllDistributorsQuery.Id,QueryCode = AllDistributorsQuery.Code, IndexOrder = 1, ObjectFieldId = DistributorObjectFields.Where(d => d.FieldName == "EnglishName" && d.ObjectTableId == DistributorObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = DistributorObjectFields.Where(d => d.FieldName == "EnglishName" && d.ObjectTableId == DistributorObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllDistributorsQueryColumn_2 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllDistributorsQuery.Id,QueryCode = AllDistributorsQuery.Code, IndexOrder = 2, ObjectFieldId = DistributorObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == DistributorObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = DistributorObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == DistributorObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);
