   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn CustomerTenantAccessRequestsQueryColumn_0 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = CustomerTenantAccessRequestsQuery.Id,QueryCode = CustomerTenantAccessRequestsQuery.Code, IndexOrder = 0, ObjectFieldId = CustomerTenantAccessRequestObjectFields.Where(d => d.FieldName == "RequestDateTime" && d.ObjectTableId == CustomerTenantAccessRequestObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = CustomerTenantAccessRequestObjectFields.Where(d => d.FieldName == "RequestDateTime" && d.ObjectTableId == CustomerTenantAccessRequestObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 100 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn CustomerTenantAccessRequestsQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = CustomerTenantAccessRequestsQuery.Id,QueryCode = CustomerTenantAccessRequestsQuery.Code, IndexOrder = 1, ObjectFieldId = CustomerTenantAccessRequestObjectFields.Where(d => d.FieldName == "RequestStatus" && d.ObjectTableId == CustomerTenantAccessRequestObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = CustomerTenantAccessRequestObjectFields.Where(d => d.FieldName == "RequestStatus" && d.ObjectTableId == CustomerTenantAccessRequestObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 100 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn CustomerTenantAccessRequestsQueryColumn_2 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = CustomerTenantAccessRequestsQuery.Id,QueryCode = CustomerTenantAccessRequestsQuery.Code, IndexOrder = 2, ObjectFieldId = CustomerTenantAccessRequestObjectFields.Where(d => d.FieldName == "ForwarderId" && d.ObjectTableId == CustomerTenantAccessRequestObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = CustomerTenantAccessRequestObjectFields.Where(d => d.FieldName == "ForwarderId" && d.ObjectTableId == CustomerTenantAccessRequestObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 100 }, queryColumnsRepository, tenantQueryColumns);
