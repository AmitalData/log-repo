   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn AllLeadSourcesQueryColumn_0 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllLeadSourcesQuery.Id,QueryCode = AllLeadSourcesQuery.Code, IndexOrder = 0, ObjectFieldId = LeadSourceObjectFields.Where(d => d.FieldName == "Name" && d.ObjectTableId == LeadSourceObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = LeadSourceObjectFields.Where(d => d.FieldName == "Name" && d.ObjectTableId == LeadSourceObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllLeadSourcesQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllLeadSourcesQuery.Id,QueryCode = AllLeadSourcesQuery.Code, IndexOrder = 1, ObjectFieldId = LeadSourceObjectFields.Where(d => d.FieldName == "InActive" && d.ObjectTableId == LeadSourceObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = LeadSourceObjectFields.Where(d => d.FieldName == "InActive" && d.ObjectTableId == LeadSourceObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);
