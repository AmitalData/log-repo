   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn AllRegionsQueryColumn_0 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllRegionsQuery.Id,QueryCode = AllRegionsQuery.Code, IndexOrder = 0, ObjectFieldId = RegionObjectFields.Where(d => d.FieldName == "Name" && d.ObjectTableId == RegionObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = RegionObjectFields.Where(d => d.FieldName == "Name" && d.ObjectTableId == RegionObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllRegionsQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllRegionsQuery.Id,QueryCode = AllRegionsQuery.Code, IndexOrder = 1, ObjectFieldId = RegionObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == RegionObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = RegionObjectFields.Where(d => d.FieldName == "LocalName" && d.ObjectTableId == RegionObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);
	    ...
	    {                
            ...
            AddObjectTableTabs.AddObjectTableTab(new ObjectTableTabDetails() { Code = "RGEV",HtmlComponentName = "",HtmlComponentUrl = "", FeatureId = RegionEventsFeature_TH1.Id, ControlPath = "Simplog.Infrastructure.Views.Events.EventsControl", ObjectTableId = RegionObjectTable.Id, TabNameTextCodeId = RegionEventsTextCode_TH1.Id, TabNameTextCodeCode = RegionEventsTextCode_TH1.Code, Tenant = 0, IndexOrder = 1 }, objectTableTabsRepository, TenantObjectTableTabs);
