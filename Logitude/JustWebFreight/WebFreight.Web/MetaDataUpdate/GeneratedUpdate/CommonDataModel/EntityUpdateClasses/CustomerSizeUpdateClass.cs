   ...
   {  
	    ...
	    {  
	        ...
	        FeaturesRepository.SubmitChanges();    
			 ...
			 QueryColumn AllCustomerSizesQueryColumn_0 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllCustomerSizesQuery.Id, IndexOrder = 0, ObjectFieldId = CustomerSizeObjectFields.Where(d => d.FieldName == "Name" && d.ObjectTableId == CustomerSizeObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = CustomerSizeObjectFields.Where(d => d.FieldName == "Name" && d.ObjectTableId == CustomerSizeObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);

			 QueryColumn AllCustomerSizesQueryColumn_1 = AddQueries.AddQueryColumn(new QueryColumnDetails { Tenant = 0, QueryId = AllCustomerSizesQuery.Id, IndexOrder = 1, ObjectFieldId = CustomerSizeObjectFields.Where(d => d.FieldName == "Order" && d.ObjectTableId == CustomerSizeObjectTable.Id).FirstOrDefault().Id, ObjectFieldCode = CustomerSizeObjectFields.Where(d => d.FieldName == "Order" && d.ObjectTableId == CustomerSizeObjectTable.Id).FirstOrDefault().FieldCode, ColumnWidth = 150 }, queryColumnsRepository, tenantQueryColumns);
