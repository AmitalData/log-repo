    ...
    public class AccountingUpdate
    {
        ...
        public void UpgradeClosedTablesForTenantZero()
        {
            ...
            string enviroment = ConfigurationManager.AppSettings.Get("ENVIROMENT");
                ...
                using (TransactionScope scop = TransactionFactory.GetNewTransaction(new TimeSpan(0, 5, 0)))
                {
                    GlobalDBRepository globalDbRep = new GlobalDBRepository();
        ...
        private void LoadTextCodes_General(Dictionary<string, TextCode> textcodes)
        {
            ...
            AddTextCodes.AddTextCode(new TextCodeDetails() { Code = "Accounting.O.ARP.invoiceAmount2reconcileMSGLower", DefaultText = "The amount to reconcile in the invoice is lower than the invoice open amount", LocalDefaultText = "הסכום להתאמה קטן מהסכום הפתוח בחשבונית", ObjectTableId = objectTable.Id, Tenant = 0, TextCodeTypeCode = "O", }, textCodeRepository, textcodes);
