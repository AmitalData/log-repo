...
<DAT name=""TEMPLATE_SQL"" xml:space='preserve'>SELECT PaymentOrderNumber FROM Declarations WHERE Declarations.CUSTOMFILENO = @CUSTOMFILENO AND Declarations.AmendmentDontDisplayInList = 0 AND TENANT=@Tenant</DAT>
...
<DAT name=""TEMPLATE_SQL"" xml:space='preserve'>SELECT email FROM Customs.CONTACTS WHERE ID =(SELECT id FROM Customs.USERS WHERE code ='AMITAL' AND TENANT=@Tenant) AND TENANT=@Tenant</DAT>
