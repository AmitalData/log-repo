...
{
    public partial class ChampStatus : System.Web.UI.Page
    {
        ...
        private bool AnyFailedStatus()
        {
            ...
            using (TransactionScope scope = TransactionFactory.GetNewTransaction())//TransactionFactory.GetNewTransaction())
            {
                ...
                {
                    ExceptionHandler.HandleException(errorInfo, DateTime.Now, 0, "", "ChampStatus", "Bug in AnyFailedStatus Method : globaldbRep.All()",null);
                }
                scope.Complete();
            }
            ...
            foreach (GlobalDB db in GlobalDatabases)
            {
                ...
                try
                {
                    IsFaild = (from a in Context.CommunicationLogs
                               where a.CommunicationStatusTypeCode == "f" && (a.CreateDateUTC > twoDaysBefore) && a.To == "Champ"
                               //where a.CommunicationStatusTypeCode == "f" && (a.CreateDate > twoDaysBefore) && (a.To == "Champ" || a.From == "Champ")
                               select a).Any();
                }

                catch (Exception errorInfo)
                {
                    ExceptionHandler.HandleException(errorInfo, DateTime.Now, 0, "", "ChampStatus", "Bug in AnyFailedStatus Method : IsFaild = (from a in Context.CommunicationLogs ...",null);
                }
