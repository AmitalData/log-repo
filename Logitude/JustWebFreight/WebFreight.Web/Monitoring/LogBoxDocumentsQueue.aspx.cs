...
{
    public partial class LogBoxDocumentsQueue : System.Web.UI.Page
    {
        ...
        private bool AnyFailedStatus()
        {
            ...
            try
            {
                IsFaildWaiting = (from a in Context.QueueMessages
                           where (a.QueueDefinitionCode.Contains("ImportersShipmentDocumentsQueue") || a.QueueDefinitionCode.Contains("ImporterShipmentDocumentsBatch")) && a.Status == 0
                           select a).Count() > 500;

                IsFaild = (from a in Context.QueueMessages
                                  where (a.QueueDefinitionCode.Contains("ImportersShipmentDocumentsQueue") || a.QueueDefinitionCode.Contains("ImporterShipmentDocumentsBatch")) && a.Status == -1 
                                  && a.CreateDateTime >= DateTime.Now.Date select a).Count() > 50;
            }

            catch (Exception errorInfo)
            {
                ExceptionHandler.HandleException(errorInfo, DateTime.Now, 0, "", "LogBoxShipmentQueue", "Bug in AnyFailedStatus Method : IsFaild = (from a in Context.QueueMessages ...", null);
            }
