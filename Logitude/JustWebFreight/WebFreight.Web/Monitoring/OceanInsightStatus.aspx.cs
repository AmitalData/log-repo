...
{
    public partial class OceanInsightStatus : System.Web.UI.Page
    {
        ...
        private bool AnyFailedStatus()
        {
            ...
            using (TransactionScope scope = TransactionFactory.GetNewTransaction())//TransactionFactory.GetNewTransaction())
            {
                ...
                {
                    ExceptionHandler.HandleException(errorInfo, DateTime.Now, 0, "", "OceanInsightStatus", "Bug in AnyFailedStatus Method : globaldbRep.All()",null);
                }
                scope.Complete();
            }
            ...
            foreach (GlobalDB db in GlobalDatabases)
            {
                ...
                try
                {
                    IsFaild = (from a in Context.CommunicationLogs
                               where a.CommunicationStatusTypeCode.ToLower() == "w" && (a.CreateDateUTC > twoDaysBefore) && a.Subject == "Ocean Insights Status"
                               select a).Any();
                }

                catch (Exception errorInfo)
                {
                    ExceptionHandler.HandleException(errorInfo, DateTime.Now, 0, "", "OceanInsightStatus", "Bug in AnyFailedStatus Method : IsFaild = (from a in Context.CommunicationLogs ...",null);
                }
