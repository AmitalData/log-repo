...
{
    public partial class SATCommunicationWaitingStatus : System.Web.UI.Page
    {
        ...
        private bool AnyFailedStatus()
        {
            ...
            using (TransactionScope scope = TransactionFactory.GetNewTransaction())//TransactionFactory.GetNewTransaction())
            {
                ...
                {
                    ExceptionHandler.HandleException(errorInfo, DateTime.Now, 0, "", "SATCommunicationLogs", "Bug in AnyFailedStatus Method : globaldbRep.All()", null);
                }
                scope.Complete();
            }
            ...
            foreach (GlobalDB db in GlobalDatabases)
            {
                ...
                {
                    ExceptionHandler.HandleException(errorInfo, DateTime.Now, 0, "", "SATCommunicationLogs", "Bug in AnyFailedStatus Method : IsFaild = (from a in Context.CommunicationLogs ...", null);
                }
