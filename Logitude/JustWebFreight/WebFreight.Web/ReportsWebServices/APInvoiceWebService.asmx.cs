    ...
    public class APInvoiceWebService : System.Web.Services.WebService
    {
        ...
        private APInvoiceMultipleEntity CalculateAPInvoiceMultipileShipment(APInvoiceMultipleShipmentPM item, List<string> frieghtChargesIds, List<string> otherChargesIds)
        {
            ...
            if (FeatureToggleHelper.HasFeatureToggle("MAP", currentTenant))
            {
                ...
                singleRecord.PayablesAccounted = item.AccountedAmount;
                singleRecord.LongMaster = item.LongMaster;

                var freightAccounted = (from d in shipmentsContext.ShipmentPayables
                                        where d.ShipmentId == item.ShipmentId
                                        && frieghtChargesIds.Contains(d.ChargesTypeId)
                                        select d.AccountedAmountInProfitCurrency).Sum();


                var otherAccounted = (from d in shipmentsContext.ShipmentPayables
                                      where d.ShipmentId == item.ShipmentId
                                      && otherChargesIds.Contains(d.ChargesTypeId)
                                      select d.AccountedAmountInProfitCurrency).Sum();
