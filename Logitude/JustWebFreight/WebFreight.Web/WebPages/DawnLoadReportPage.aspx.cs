    ...
    public partial class DawnLoadReportPage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if ((isAppServiceENV || isAppService) && !string.IsNullOrEmpty(LogitudeSettings.LogitudeIISURL))
                {
                    string URI = LogitudeSettings.LogitudeIISURL.TrimEnd('/') + "/api/Report/" + "GetMemoryStreamFromStorageForPrintExcelOrPdf";

                    using (var client = new HttpClient())
                    {
                        var memoryStreamForExcelOrPdfRequest = new { FileName = fileName, Tenant = tenant, Type = type, UseOnePageHeaderAndFooter = useOnePageHeaderAndFooter, ExportDataOnly = exportDataOnly, ExportObjectFormatting = exportObjectFormatting };
                        ...
                        {
                            throw new Exception("Dwonload page failed to get file from IISWR");
                        }
                ...
                else
                {
                    ...
                    if (result == null || result.Length == 0) throw new Exception(string.Format("Dwonload page failed to get file from storage. file name {0}", fileName));

                    NetCommonHelper.Logger.DevLog.Instance.WriteDebug("step 1 this is not appService  get file from storage {0} file size {1}", null, fileName, result?.Length);
                ...
                if (memoryStream != null)
                {
                    ...
                    if (_DatainByte.Length == 0) { throw new Exception(string.Format("Dwonloadpage failed to get file from IISWR - 0 bytes {0} ", fileName)); }
