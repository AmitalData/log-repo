...
{
    public partial class BookingValidationClass : IBookingValidateContext
    {
        bool ready;
        int tenant = 0;
        ...
        public BookingValidationClass(string objectTableName, int tenant)
        {
            this.tenant = tenant;
        ...
        public string GetErrorMessage(object value, object instance, string propertyName)
        {
            ...
            if (field.IsRequiered)
            {
                ...
                if (value == null)
                {
                    error = BookingTranslateTextsClass.GetTranslation("General.M.FieldIsRequired", field.FullNameTextCode.Code, null, null, field.Tenant);
                }
            ...
            if (field.DataTypeCode == "Text")
            {
                ...
                if (FieldValueValidator.IsNotValidMinMaxValue(field, valueString))
                {
                    error = BookingTranslateTextsClass.GetTranslation("General.M.MinMax", field.FullNameTextCode.Code, field.MinLength.ToString(), field.MaxLength.ToString(), field.Tenant);
                }
            }
