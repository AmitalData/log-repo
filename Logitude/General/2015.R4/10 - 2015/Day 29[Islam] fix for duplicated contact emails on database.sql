...
--  having count(*) > 1
--  OPEN eventsCursor FETCH NEXT FROM eventsCursor INTO @Email,@GlobalTenantId,@Count
...
--            set @delimiter = '@'
--			if @Email <> ''  and @Email is not null and len(@Email) > 1 and CHARINDEX('@',@Email) > 0---and @Email like('@') and len(@Email) > 1
--			begin
--			--
--			 --print @Email
...
--		update [Global].[dbo].[GlobalContacts] set Email = @NewEmail where id = @CurrentContactId and GlobalTenantId = @GlobalTenantId
--		update [Main].[dbo].[Contacts] set Email = @NewEmail where id = @CurrentContactId and Tenant = @GlobalTenantId
		
--		print  @CurrentContactId+' - '+  @NewEmail + '------ IsUser:' + CAST(@IsUser as varchar)
...
--		   FETCH NEXT FROM eventsCursor INTO @Email,@GlobalTenantId,@Count
