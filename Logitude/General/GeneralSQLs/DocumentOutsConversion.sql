...
Create PROCEDURE [dbo].[usp_DocumentOutsConversion]
(
...
declare @Issued as bit
  ...
  set @createdBy=(select id from Contacts where Email=@email and Tenant=@Tenant)
   ...
   begin
   insert into DocumentsFilings (Id,Tenant,ChildEntityId,ChildEntityReference,EntityId,UpdatedByUserId,UpdateDate,DocumentTypeId,Notes,ObjectTableId,CreateDate,CreatedByUserId,DirectionCode,OwnerId,HasCopies,Code,Received) 
   values (@Id,@Tenant,@ChildEntityId,@ChildEntityReference,@EntityId,@createdBy,@createdDate,@DocumentTypeId,@Notes,@ObjectTableId,@createdDate,@createdBy,'O',@createdBy,0,@codeCounter,0)
   print @email
   end
