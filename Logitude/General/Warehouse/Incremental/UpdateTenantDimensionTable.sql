 ...
 begin
   declare @Key as  int
	...
	set @Key = (select TenantNumber from Dim_Tenants where TenantNumber = @Id)
	if(@Key is  null) begin insert into Dim_Tenants values(@Id,@Company,@Country); end
	else begin update   Dim_Tenants set TenantName =@Company,  Country =@Country  Where TenantNumber = @Id end
