...
IF OBJECT_ID ('Fact_Shipments', 'U')  IS NOT NULL  drop table Fact_Shipments
...
CREATE TABLE #Fact_ShipmentsTable (
	...
	Customer int,
	Incoterm int,
	TotalGrossWeightInKG float,
	TotalChargeableWeightInKG float,
	TotalVolumeInCBM float,
	NumberOfPackages int ,
	NumberOfContainers int ,
	Salesman int,
	AccountManager int,
	TotalReceivablesInLocalCurrency float,
	TotalPayablesInLocalCurrency float,
    TotalProfitInLocalCurrency float,
	TotalReceivablesInProfitCurrency float,
	TotalPayablesInProfitCurrency float,
	TotalProfitInProfitCurrency float,
	LocalCurrency  int,
    ProfitCurrency  int,
	NumberOfInvoices  int,
	OperationallyClosed  bit,
	AccountingClosed bit ,
	Status int , 
  ...
  declare @TotalProfitInProfitCurrency as float
   ...
   declare @ToPortId as int
	...
	BEGIN
		 ...
		 --set @FinalDestination = (select Id_Number from DIM_Ports where  Id = @ToPortId and SourceTenant =@Tenant )
...
ADD CONSTRAINT PK_Fact_Shipments_Id_Number PRIMARY KEY CLUSTERED (Id_Number); 
