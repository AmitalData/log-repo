...
go
	...
	FOR
	SELECT Id, FromTypeCode, ToTypeCode, FromCity, ToCity, FromAddressCity, ToAddressCity, FromPortName, ToPortName, ToPortId
	FROM #temp	
	OPEN DataCursor FETCH NEXT FROM DataCursor INTO @Id, @FromTypeCode, @ToTypeCode, @FromCity, @ToCity, @FromAddressCity, @ToAddressCity, @FromPortName, @ToPortName, @ToPortId
	...
	BEGIN
		...
		insert into #temp_ShipmentComputedFields(Id, [From], [To], ToPortId) values(@Id, @From, @To, @ToPortId)
		...
		begin		
			...
			set
			DeliveryFrom = #temp_ShipmentComputedFields.[From],
			DeliveryTo = #temp_ShipmentComputedFields.[To],
			DeliveryToPortId = #temp_ShipmentComputedFields.ToPortId
	...
	FETCH NEXT FROM DataCursor INTO @Id, @FromTypeCode, @ToTypeCode, @FromCity, @ToCity, @FromAddressCity, @ToAddressCity, @FromPortName, @ToPortName, @ToPortId
	END
	CLOSE DataCursor
