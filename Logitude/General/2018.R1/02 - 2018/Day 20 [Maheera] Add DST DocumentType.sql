...
BEGIN
		set @Tenant = 0
		set @ObjectTableId = (select Id from ObjectTables where Name = 'Shipment' and Tenant = @Tenant)

		if not exists (select * from DocumentTypes where Code = 'DST' and Tenant = @Tenant)
		BEGIN
