...
BEGIN
	...
	begin
	set @value= @value+ 'Tel:' +  Space(1) +  @Phone+  Space(1) + 'Fax:'+  Space(1) +   @Fax + @NewLineChar
	end
	
	if (@Phone is not NULL and @Fax is  null )
	begin
	set @value=@value+'Tel:'+  Space(1) +  @Phone  + Space(1) +@NewLineChar
	end

	if (@Phone is NULL and @Fax is not null )
	begin
	set @value=@value+ 'Fax:' + Space(1) +  @Fax +@NewLineChar
	end
	print @value
