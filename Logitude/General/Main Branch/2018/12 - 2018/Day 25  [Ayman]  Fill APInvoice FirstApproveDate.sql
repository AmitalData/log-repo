...
BEGIN 
	...
	where
	TraceEvents.ObjectTableId = (select Id from ObjectTables where Name = 'APInvoice')
	AND EventTypes.Code = 'APIA'
	...
	BEGIN
		...
		set @EntityApproveDate = (select ApprovedDate from APInvoices where Id = @EntityId)
		...
		begin			
			update APInvoices set FirstApproveDate = ApprovedDate where Id = @EntityId
		end

		else
		begin
			update APInvoices set FirstApproveDate = @FirstDateTime where Id = @EntityId
		end
