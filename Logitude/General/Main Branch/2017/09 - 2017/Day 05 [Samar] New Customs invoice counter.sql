...
BEGIN
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				if not exists (select * from CounterDefinitions where Tenant = @Tenant AND CounterId = @ARInvoiceCounterId AND Parameter1 = 'CA')
				begin
					...
					values (@NewEntityId, @Tenant, 'CA', 1000, @ARInvoiceCounterId, 'CA', 0)
				end

				if not exists (select * from CounterDefinitions where Tenant = @Tenant AND CounterId = @ARInvoiceCounterId AND Parameter1 = 'CC')
				begin
					EXECUTE usp_GetNextTableIdValue @NewEntityId OUTPUT,'CounterDefinition'
					insert into CounterDefinitions (Id, Tenant, Parameter1, StartNumber, CounterId, Prefix, UniquePerPrefix)
					values (@NewEntityId, @Tenant, 'CC', 1000, @ARInvoiceCounterId, 'CC', 0)
				end
