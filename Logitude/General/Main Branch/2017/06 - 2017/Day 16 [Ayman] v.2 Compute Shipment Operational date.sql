...
BEGIN
	...
	FOR
	SELECT Id, Tenant, DirectionId, CreateDateTime, ShipmentLevelCode, MasterShipmentDataId
	FROM Shipments
	OPEN DataCursor FETCH NEXT FROM DataCursor INTO @ShipmentId, @Tenant, @DirectionId, @CreateDateTime, @ShipmentLevelCode, @MasterShipmentDataId
	...
	BEGIN
		...
		BEGIN
			...
			if(@ShipmentLevelCode = 'C' AND @OperationalDate != @CreateDateTime)
			BEGIN
				...
				BEGIN
					...
					update Shipments set OperationalDate = @OperationalDate where Id = @HouseId AND Tenant = @Tenant
	...
	FETCH NEXT FROM DataCursor INTO @ShipmentId, @Tenant, @DirectionId, @CreateDateTime, @ShipmentLevelCode, @MasterShipmentDataId
	END
	CLOSE DataCursor
