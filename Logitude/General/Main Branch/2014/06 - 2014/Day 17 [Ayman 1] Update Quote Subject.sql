		...
		BEGIN
			...
			begin
				...
				begin
					...
					begin
						set @FromAddressZipCode = (select ZipCode from Addresses where Id = @FromPartnerAddressId AND Tenant = @Tenant)
						if (@FromAddressZipCode is not null)
						begin
						...
						set @FromAddressCity = (select City from Addresses where Id = @FromPartnerAddressId AND Tenant = @Tenant)
					...
					begin
						set @ToAddressZipCode = (select ZipCode from Addresses where Id = @ToPartnerAddressId AND Tenant = @Tenant)
						if (@ToAddressZipCode is not null)
						begin
						...
						set @ToAddressCity = (select City from Addresses where Id = @ToPartnerAddressId AND Tenant = @Tenant)
				...
				begin
					...
					begin
						...
						begin
							...
							set @FromAddressZipCode = (select ZipCode from Addresses where Id = @FromAddressId AND Tenant = @Tenant)
							if (@FromAddressZipCode is not null)
							begin
								if (@MySubjectField = '') set @MySubjectField = @FromAddressZipCode
								else set @MySubjectField = @MySubjectField + ' ' + @FromAddressZipCode
							...
							set @FromAddressCity = (select City from Addresses where Id = @FromAddressId AND Tenant = @Tenant)
							if (@FromAddressCity is not null)
							begin
								if (@MySubjectField = '') set @MySubjectField = @FromAddressCity
								else set @MySubjectField = @MySubjectField + ' ' + @FromAddressCity							
					...
					begin
						...
						begin
							...
							set @ToAddressZipCode = (select ZipCode from Addresses where Id = @ToAddressId AND Tenant = @Tenant)
							if (@ToAddressZipCode is not null)
							begin
								if (@MySubjectField = '') set @MySubjectField = @ToAddressZipCode
								else set @MySubjectField = @MySubjectField + ' > ' + @ToAddressZipCode
							...
							set @ToAddressCity = (select City from Addresses where Id = @ToAddressId AND Tenant = @Tenant)
							if (@ToAddressCity is not null)
							begin
								if (@MySubjectField = '') set @MySubjectField = @ToAddressCity
								else set @MySubjectField = @MySubjectField + ' > ' + @ToAddressCity							
