...
BEGIN
		...
		FOR
		SELECT Id, Tenant, QuoteNumber, ShipperName, ShipperReference1, ShipperReference2, ConsigneeName, ConsigneeReference1, ConsigneeReference2, CustomerName, CustomerReference1, CustomerReference2, FromPortId, ToPortId, MainCarriageCarrierId, Subject
		FROM Quotes
		OPEN QuotesCursor FETCH NEXT FROM QuotesCursor INTO @QuoteId, @Tenant, @QuoteNumber, @ShipperName, @ShipperReference1, @ShipperReference2, @ConsigneeName, @ConsigneeReference1, @ConsigneeReference2, @CustomerName, @CustomerReference1, @CustomerReference2, @FromPortId, @ToPortId, @MainCarriageCarrierId, @Subject
		...
		set @MySearchFields = ''
			...
			begin
				set @MySearchFields = @QuoteNumber
			end
		...
		FETCH NEXT FROM QuotesCursor INTO @QuoteId, @Tenant, @QuoteNumber, @ShipperName, @ShipperReference1, @ShipperReference2, @ConsigneeName, @ConsigneeReference1, @ConsigneeReference2, @CustomerName, @CustomerReference1, @CustomerReference2, @FromPortId, @ToPortId, @MainCarriageCarrierId, @Subject
		END				
		CLOSE QuotesCursor
