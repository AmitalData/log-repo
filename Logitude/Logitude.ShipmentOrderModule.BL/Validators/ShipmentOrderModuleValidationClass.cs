   ...
   public partial class ShipmentOrderModuleValidationClass : IShipmentOrderValidateContext
    {
        ...
        public ShipmentOrderModuleValidationClass(string objectTableName, int tenant)
        {
            this.tenant = tenant;
        ...
        public string GetErrorMessage(object value, object instance, string propertyName)
        {
            ...
            if (field.IsRequiered)
            {
                ...
                if (value == null)
                {
                    error = ShipmentOrderModuleTranslateTextsClass.GetTranslation("General.M.FieldIsRequired", field.FullNameTextCode.Code, null, null, field.Tenant);
                }
            ...
            if (field.DataTypeCode == "Text")
            {
                ...
                if (!string.IsNullOrEmpty(valueString))
                {
                    if (!field.IsMaxLength)
                    {
                        if (valueString.Length > field.MaxLength || valueString.Length < field.MinLength)
                        {
                            error = ShipmentOrderModuleTranslateTextsClass.GetTranslation("General.M.MinMax", field.FullNameTextCode.Code, field.MinLength.ToString(), field.MaxLength.ToString(), field.Tenant);
