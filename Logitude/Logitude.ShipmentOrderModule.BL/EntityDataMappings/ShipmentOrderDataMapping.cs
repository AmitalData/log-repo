...
namespace Logitude.ShipmentOrderModule.BL.EntityDataMappings
{
    ...
    public partial class ShipmentOrderDataMapping : IMapping<ShipmentOrderPM, ShipmentOrder>
    {
        ...
        public void CustomLocalPOCOToPM(ShipmentOrderPM entityPM, ShipmentOrder entityPOCO)
        {
            ...
            List<PortList> ports = GetPortList(entityPOCO.Tenant, entityPOCO);
            incoterm = GetIncotermById(entityPM.IncotermId, entityPM.Tenant);
            entityPM.ConsigneeName = cards.Where(d => d.Id == entityPOCO.ConsigneeId).Select(d => GetCardName(d)).FirstOrDefault();
            entityPM.ShipperName = cards.Where(d => d.Id == entityPOCO.ShipperId).Select(d => GetCardName(d)).FirstOrDefault();
            entityPM.AgentName = cards.Where(d => d.Id == entityPOCO.AgentId).Select(d => GetCardName(d)).FirstOrDefault();
            entityPM.CustomsAgentName = cards.Where(d => d.Id == entityPOCO.CustomsAgentId).Select(d => !string.IsNullOrEmpty(d.EnglishName) ? d.EnglishName : d.LocalName).FirstOrDefault();
            entityPM.ForwarderName = cards.Where(d => d.Id == entityPOCO.ForwarderId).Select(d => !string.IsNullOrEmpty(d.EnglishName) ? d.EnglishName: d.LocalName).FirstOrDefault();
            entityPM.CarrierName = cards.Where(d => d.Id == entityPOCO.CarrierId).Select(d => GetCardName(d)).FirstOrDefault();
            entityPM.CustomerName = cards.Where(d => d.Id == entityPOCO.CustomerId).Select(d => GetCardName(d)).FirstOrDefault();
            entityPM.OriginPortName = ports.Where(d => d.Id == entityPOCO.OriginPortId).Select(d => d.EnglishName).FirstOrDefault();
            entityPM.DestinationPortName = ports.Where(d => d.Id == entityPOCO.DestinationPortId).Select(d => d.EnglishName).FirstOrDefault();
            entityPM.GatewayName = ports.Where(d => d.Id == entityPOCO.GatewayId).Select(d => d.EnglishName).FirstOrDefault();
            entityPM.AccountManagerName = GetAccountManagerNameById(entityPOCO.AccountManagerId, entityPOCO.Tenant);
        ...
        private string GetCardName(CardList cardObject)
        {
